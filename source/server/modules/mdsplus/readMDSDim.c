...
// Length of Dimension Data
        ...
        IDAM_LOGF(LOG_DEBUG, "Length of Dimension (%d) = %d\n", ndim, size);
...
// Dimension Units
        ...
        if (lunits > 1) {
            ...
            IDAM_LOGF(LOG_DEBUG, "ReadMDSDim: TDI =  %s \n", sdim);
        ...
        IDAM_LOGF(LOG_DEBUG, "ReadMDSDim: Dimension Units =[%s] %d\n", units,
                  strlen(units));
