...
// Extract Requested DataType Info
    ...
    if(dtid >= 0) {
        ...
        err = 999;
        addIdamError(&idamerrorstack, CODEERRORTYPE, "readPPF", err, "Unable to Identify the PPF DDA Data-Type Requested");
        PPFUID("JETPPF","R", 7, 1);
        return err;
    }
...
// Subsetting indicies begin with value 1
    if(xsubset > 0 && xsubset > nx ) {
        err = 995;
        addIdamError(&idamerrorstack, CODEERRORTYPE, "readPPF", err, "The Requested X-Dimension Subset Exceeds the Valid X-Dimensions");
        PPFUID("JETPPF","R", 7, 1);
        return err;
    }
