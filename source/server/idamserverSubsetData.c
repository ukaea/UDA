...
// Process all sets of subsetting operations
    ...
    for (i = 0; i < nsubsets; i++) {                        // the number of sets of Subset Operations
        ...
        for (j = 0; j < nbound; j++) {                        // Process each operation separately
            ...
            IDAM_LOGF(LOG_DEBUG, "[%d][%d]Value = %e, Operation = %s, DIM id = %d, Reform = %d\n",
                    i, j, value, operation, dimid, subset.reform);

            if (dimid < 0 || dimid >= data_block->rank) {
                ierr = 9999;
                IDAM_LOGF(LOG_ERROR, "Error ***    DIM id = %d,  Rank = %d, Test = %d \n",
...
// Decompress the dimensional data if necessary & free Heap Associated with Compression
            ...
            if (dim->compressed) {
                ...
                if (dim->sams != NULL) free((void*) dim->sams);
                if (dim->offs != NULL) free((void*) dim->offs);
                if (dim->ints != NULL) free((void*) dim->ints);

                dim->udoms = 0;
                dim->sams = NULL;        // Avoid double freeing of Heap
                dim->offs = NULL;
                dim->ints = NULL;
            }
...
// Free Heap associated with the original Dimensional Structure Array
            ...
            if (dim->errhi != NULL) free((void*) dim->errhi);
    ...
    if (ierr == 0 && subset.reform) {
        ...
        for (j = 0; j < rank; j++) {
            if (data_block->dims[j].dim_n <= 1) {
                IDAM_LOGF(LOG_DEBUG, "Reforming Dimension %d\n", j);

                data_block->dims[j].compressed = 0;
                data_block->dims[j].method = 0;

                if (data_block->dims[j].dim != NULL) free((void*) data_block->dims[j].dim);
                if (data_block->dims[j].errlo != NULL) free((void*) data_block->dims[j].errlo);
                if (data_block->dims[j].errhi != NULL) free((void*) data_block->dims[j].errhi);
                if (data_block->dims[j].sams != NULL) free((void*) data_block->dims[j].sams);
                ...
                if (data_block->order == j) {
                    ...
                    if (data_block->order > j)
                        data_block->order = data_block->order - 1;        // Time Dimension ID reduced by 1
                }
...
// Apply simple functions to the subset data: XML syntax function="name(dimid=9)"
    ...
    if (ierr == 0 && subset.function[0] != '\0') {
        ...
        if (!strncasecmp(subset.function, "minimum(", 8)) {        // Single scalar result
            ...
            if (dimid < 0 || dimid >= data_block->rank) {
                ierr = 999;
                IDAM_LOGF(LOG_ERROR, "Function Syntax Error -  dimid = %d,  Rank = %d\n", dimid,
                        data_block->rank);
                addIdamError(&idamerrorstack, CODEERRORTYPE, "idamserverSubsetData", ierr,
                ...
                case (TYPE_FLOAT): {
                    ...
                    switch (data_block->rank) {
                        case (0): {            // Ignore function dimid argument
                            ...
                            dp[0] = min;
                            dp = realloc((void*) dp, sizeof(float));        // Reduce array size
                            data_block->data_n = 1;
                            break;
                        }
                        case (1): {
                ...
                case (TYPE_DOUBLE): {
                    ...
                    switch (data_block->rank) {
                        case (0): {            // Ignore function dimid argument
                            ...
                            dp[0] = min;
                            dp = realloc((void*) dp, sizeof(double));        // Reduce array size
                            data_block->data_n = 1;
                            break;
                        }
                        case (1): {
        ...
        if (!strncasecmp(subset.function, "const", 5)) {        // Constant value substitution
            ...
            if (p1 != NULL) {
                ...
                TrimString(p2);
                LeftTrimString(p2);
                IDAM_LOGF(LOG_DEBUG, "p2 = [%s]\n", p2);
                if (IsFloat(p2)) {
                    value = atof(p2);
                } else {
                    IDAM_LOG(LOG_DEBUG, "IsFloat FALSE!\n");
                    // ERROR
                }
            }
        ...
        if (!strncasecmp(subset.function, "rotateRZ", 8)) {        // Rotate R,Z coordinates in rank 3 array
            ...
            if (order == 0) {        // array[nz][nr][nt] -> [nr][nz][nt]
                ...
                double*** data = (double***) malloc(nt * sizeof(double**));
                for (k = 0; k < nt; k++) {
...
// Seek specific options
    ...
    if ((p = strstr(options, "member=")) != NULL) {        // Extract a Structure member
...
// Parse the Operation String for Value and Operation
    ...
    if ((p = strtok(opcopy, ",")) != NULL) {        // Tokenise into Individual Operations on each Dimension
...
//
    ...
    for (i = 0; i < nbound; i++) {
        ...
        if ((p = strstr(opcopy, ":")) != NULL) {        // Integer Type Array Index Bounds
            t2 = p + 1;
            ...
            if (t1[0] == '#')
                subsets[nsubsets - 1].lbindex[i] = -2;        // Reverse the data as # => Final array value
                                ...
                                if (!strcasecmp(operation, "ne") || !strncmp(operation, "!=", 2) ||
                                    ...
                                    if (strncmp(operation, "!~=", 3) != 0) {
                                        ...
                                        if (index >= 0) {
                                            for (k = 0; k < dim->dim_n; k++) {
                                                if (k != index)
                                                    subsetindices[count++] = k;            // Drop the single nearest value
                                            }
                            ...
                            int start, int end, int start1, int end1, int* n, void** newdata)
{
    ...
    IDAM_LOGF(LOG_DEBUG, "Data Type: %d    Rank: %d\n", data_type, rank);
...
// Reshape the Data
        ...
        default:
            ...
            IDAM_LOGF(LOG_ERROR, "Data Type: %d    Rank: %d\n", data_type, rank);
