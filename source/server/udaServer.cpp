...
int uda::server::uda_server(uda::config::Config& config, uda::client_server::ClientBlock client_block)
{
    ...
    err = handshake_client(config, &client_block, &server_block, &server_closedown, &log_struct_list, server_input,
                           server_output);
#endif
    ...
    if (!err && !server_closedown) {
        int fatal = 0;
        do_server_loop(config, &request_block, &data_block_list, &client_block, &server_block, &metadata_block, &actions_desc,
                     ...
                     unsigned int* total_datablock_size)
{
    ...
    *total_datablock_size = count_data_block_list_size(data_block_list, client_block);
    ...
    if ((err = protocol2(server_output, protocol_id, XDRStreamDirection::Send, nullptr, log_malloc_list, user_defined_type_list,
    ...
    if ((err = protocol2(server_output, ProtocolId::DataBlockList, XDRStreamDirection::Send, nullptr, log_malloc_list,
                   ...
                   int server_tot_block_time, int* server_timeout)
{
    ...
    if ((err = protocol2(server_input, protocol_id, XDRStreamDirection::Receive, nullptr, log_malloc_list, user_defined_type_list,
    ...
    *server_timeout = client_block->timeout;                 // User specified Server Lifetime
    ...
    if ((err = protocol2(server_input, ProtocolId::RequestBlock, XDRStreamDirection::Receive, nullptr, log_malloc_list,
    ...
    for (int i = 0; i < request_block->num_requests; ++i) {
        ...
        if (request->put) {
            if ((err = protocol2(server_input, ProtocolId::PutdataBlockList, XDRStreamDirection::Receive, nullptr, log_malloc_list,
                   ...
                   int* server_timeout)
{
    ...
    do {
        ...
        int server_closedown = 0;
        err = handle_request(config, request_block, client_block, server_block, metadata_block, actions_desc, actions_sig,
    ...
    } while (err == 0 && next_protocol == ProtocolId::Sleep && !*fatal);
