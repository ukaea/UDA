...
void tokenize1DArcadeParameters(const char* s, char** diagnostic, char** object_name, int* extractionIndex, char** normalizationAttributes)
{
	...
	*object_name = strdup(strtok(NULL, delim2));
	...
	char* token = strdup(strtok(NULL, delim2));
...
void tokenize1DArcadeParameters2(const char* s, char** diagnostic, char** object_name, int* extractionIndex, char** diagnostic2, char** object_name2, int* extractionIndex2)
{
	...
	strdup(strtok(s_copy, delim)); //function name
	*diagnostic = strdup(strtok(NULL, delim2));
	UDA_LOG(UDA_LOG_DEBUG, "in tokenize1DArcadeParameters2, diagnostic=%s\n", *diagnostic);
	RemoveSpaces(*diagnostic);
	*object_name = strdup(strtok(NULL, delim2));
	UDA_LOG(UDA_LOG_DEBUG, "in tokenize1DArcadeParameters2, object_name=%s\n", *object_name);
	RemoveSpaces(*object_name);
	char* token = strtok(NULL, delim2);
	*extractionIndex = atoi(token);
	UDA_LOG(UDA_LOG_DEBUG, "in tokenize1DArcadeParameters2, extractionIndex1=%d\n", *extractionIndex);
	*diagnostic2 = strdup(strtok(NULL, delim2));
	UDA_LOG(UDA_LOG_DEBUG, "in tokenize1DArcadeParameters2, diagnostic2=%s\n", *diagnostic2);
	RemoveSpaces(*diagnostic2);
	*object_name2 = strdup(strtok(NULL, delim2));
	UDA_LOG(UDA_LOG_DEBUG, "in tokenize1DArcadeParameters2, object_name2=%s\n", *object_name2);
	RemoveSpaces(*object_name2);
	token = strtok(NULL, delim2);
	*extractionIndex2 = atoi(token);
	UDA_LOG(UDA_LOG_DEBUG, "in tokenize1DArcadeParameters2, extractionIndex2=%d\n", *extractionIndex2);
	free(s_copy);
}
...
void multiplyFloat(float* p, float factor, int val_nb)
{
	...
	//DAM_LOGF(UDA_LOG_DEBUG, "val_nb : %d\n", val_nb);
...
int getReturnType(char* attributes, int* dataType)
{
	...
	if (STR_EQUALS(token, "vecstring_type") || STR_EQUALS(token, "string") || STR_EQUALS(token, "STR_0D")) {
		...
		addIdamError(CODEERRORTYPE, "WEST:ERROR: convertToInt(): Unsupported data type: %s", err, token);
		return -1;
	}
	*dataType = i;
	free(s_copy);
