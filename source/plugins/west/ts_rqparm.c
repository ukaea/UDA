...
int readStaticParameters(char** pt_char, int *nb_val, int num_choc, char* nom_prod, char* nom_objet, char* nom_param, int  val_nb)
{
	...
	cr = TSRqParm (num_choc, nom_prod, nom_objet, nom_param, val_nb,
			pt_char, nb_val, &format);
	return cr;
}
		...
		int rang[], float **X, float **Y, int *maxreel)
{
...
#endif
	if (cr || (nbcoord[0] == 0))
	{				/* ncoord[0]= 0 si certif <= -2 */
		if (cr)
		{			/* verification de l'existence du choc */
			cr1 = TSExist ("num_choc", ptEntree, nbcoord, &numv, certif, date, heure);
			if (cr1)
			{			/* le choc n'existe pas */

				switch(cr1) {
				case 103 : printf("Tslib Server down !!! \n");
				printf("N'en fau tasta per lou saupre \n");
				break;
				default: printf ("TSExist - %s Choc absent cr1 = %d\n",
						nomsig, cr1);
				}
				free (ptEntree);
	...
	if (rgrp)
		type = extgType;		/* extraction d'un signal demandee */

	if (!(ptDescDon->type_donnee >> SG_FTMPS & 1))
	{				/* non fonction du temps */
