...
int execute(const char* mapfun, int shotNumber, DATA_BLOCK* data_block, int* nodeIndices) {
	...
	if (strcmp(fun_name,"tsmat_collect") == 0) { //returns a static parameter (rank = 0) from a collection of static data (Top objects).
		//Given the list of all static data in the collection, the element returned in the data_block is list(idam index)
		fun = 0;
	}
	else if (strcmp(fun_name,"shape_of_tsmat_collect") == 0) { //Returns the list size of all static data in the collection

		fun = 1;
	}
	else if (strcmp(fun_name,"set_value") == 0) { //returns a static value (rank = 0)
		fun = 2;
	}
	else if (strcmp(fun_name,"tsmat") == 0) { //returns a static parameter (rank = 0 or 1) from a Top object
		fun = 3;
	}
	else if (strcmp(fun_name,"set_value_collect") == 0) { //returns a static value according to the position of the element in the collection (rank = 0)
		fun = 4;
	}
	...
	case 0:
	{
		...
		execute_tsmat_collect(TOP_collections_parameters, attributes, shotNumber, data_block, nodeIndices, normalizationAttributes);
	...
	case 3:
	{
		...
		idamLog(LOG_DEBUG, "Calling tokenizeFunParameters() from WEST plugin\n");
		tokenizeFunParameters(mapfun, &TOP_collections_parameters, &attributes, &normalizationAttributes);
		execute_tsmat_without_idam_index(TOP_collections_parameters, attributes, shotNumber, data_block, normalizationAttributes);
		break;
	}
	...
	case 4:
	{
		...
		idamLog(LOG_DEBUG, "Calling tokenizeFunParameters() from WEST plugin\n");
		tokenizeFunParameters(mapfun, &TOP_collections_parameters, &attributes, &normalizationAttributes);
		execute_setvalue_collect(TOP_collections_parameters, attributes, shotNumber, data_block, nodeIndices, normalizationAttributes);
		break;
	}
...
int GetDynamicData(int shotNumber, const char* mapfun, DATA_BLOCK* data_block, int* nodeIndices) {
	...
	if (signalType == 2) { //signal is a group of signals, so we append extraction chars to signal name
		...
		char result[50];
		addExtractionChars(result, objectName, searchedArrayIndex + 1); //Concatenate signalName avec %searchedArrayIndex + 1
...
int convertToInt(char* value)
{
	...
	if (strcmp(value, "vecstring_type") == 0 || strcmp(value, "string") == 0 || strcmp(value, "STR_0D") == 0) {
		i = TYPE_STRING;
	} else if (strcmp(value, "vecflt_type") == 0 || strcmp(value, "float") == 0 || strcmp(value, "FLT_0D") == 0) {
		i = TYPE_FLOAT;
	} /*else if (strcmp(value, "double") == 0) {
		i = TYPE_DOUBLE;
	}*/else if (strcmp(value, "vecint_type") == 0 || strcmp(value, "integer") == 0 || strcmp(value, "int") == 0 || strcmp(value, "INT_0D") == 0) {
		i = TYPE_INT;
	}
