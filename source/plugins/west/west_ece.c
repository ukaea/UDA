...
int ece_harmonic_data(int shotNumber, DATA_BLOCK* data_block, int* nodeIndices) {
	...
	if (shotNumber >= ARCADE_GECEMODE_EXISTS_FROM_SHOT) {
		...
		IDAM_LOGF(UDA_LOG_DEBUG, "mode: %d\n", mode);
		if (mode == -1) { //ERROR
			free(time);
			free(data);
			return -1;
...
int ece_harmonic_time(int shotNumber, DATA_BLOCK* data_block, int* nodeIndices) {
	...
	IDAM_LOGF(UDA_LOG_DEBUG, "harmonic time array length: %d\n", len);
...
int getECEModeFromNPZFile(int shotNumber) {
	...
	if (shotNumber >= ARCADE_GECEMODE_EXISTS_FROM_SHOT) {
		int err = 901;
		addIdamError(&idamerrorstack, CODEERRORTYPE, "getECEModeFromNPZFile() should not be called for this shot number (from WEST)", err, "");
		return -1;
	}
...
int getECEModeHarmonicTime(int shotNumber, int channel, float** time, int* len,
	...
	if (status != 0) {
		int err = 901;
		addIdamError(&idamerrorstack, CODEERRORTYPE, "Unable to get time for O1 mode", err, "");
	}
...
int getECEModeObject(char* TOP_collections_parameters, char** objectName, int GVSHArray, int mode) {
	...
	*objectName = strdup(result);
	IDAM_LOGF(UDA_LOG_DEBUG, "Object name for mode %d: %s\n", mode, *objectName);
	free(command);
	return 0;
}
