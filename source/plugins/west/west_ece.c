...
int ece_harmonic_time(int shotNumber, DATA_BLOCK* data_block, int* nodeIndices)
{
	...
	IDAM_LOG(UDA_LOG_DEBUG, "Calling getECEModeHarmonicTime() \n");
	int status = getECEModeHarmonicTime(shotNumber, channel, &time, &len, TOP_collections_parameters);
	if (status != 0) return status;
	IDAM_LOGF(UDA_LOG_DEBUG, "ECE harmonic mode time array length: %d\n", len);
	...
	IDAM_LOG(UDA_LOG_DEBUG, "reaching end of function of ece_harmonic_time()\n");
	return 0;
}
...
int ece_frequencies(int shotNumber, DATA_BLOCK* data_block, int* nodeIndices)
{
	...
	if (shotNumber >= ARCADE_GECEMODE_EXISTS_FROM_SHOT) {
		...
		int len;
		IDAM_LOG(UDA_LOG_DEBUG, "Calling getECEModeHarmonicTime\n");
		int status = getECEModeHarmonicTime(shotNumber, channel, &frequencies_time, &len, TOP_collections_parameters);
		if (status != 0) {
			int err = 901;
			addIdamError(CODEERRORTYPE, "WEST:ERROR: error calling getECEModeHarmonicTime()", err, "");
			free(frequencies_time);
			return status;
		}
		IDAM_LOG(UDA_LOG_DEBUG, "After calling getECEModeHarmonicTime\n");
		frequencies_data = malloc(sizeof(float) * len);
		for (i = 0; i < len; i++)
			frequencies_data[i] = GSH[channel - 1] * 1e9; //result converted in Hertz
		IDAM_LOG(UDA_LOG_DEBUG, "setting dynamic data\n");
		SetDynamicData(data_block, len, frequencies_time, frequencies_data);
		IDAM_LOG(UDA_LOG_DEBUG, "freeing GSH\n");
		free(GSH);
		free(TOP_collections_parameters);
		IDAM_LOG(UDA_LOG_DEBUG, "end of function\n");
		return 0;
	}
	return -1;
}
...
int ece_identifiers(int shotNumber, DATA_BLOCK* data_block, int* nodeIndices)
{
	...
	IDAM_LOG(UDA_LOG_DEBUG, "Calling ece_identifiers\n");
...
int getECEModeFromNPZFile(int shotNumber)
{
	...
	if (pFile == NULL) {
		...
		IDAM_LOG(UDA_LOG_ERROR, "Error opening ECE mode file\n");
		return -1;
	} else {
	...
	if (s == NULL) {
		...
		IDAM_LOGF(UDA_LOG_ERROR, "ECE mode not found for shot: %d\n", shotNumber);
	} else {
		...
		char* TOP_collections_parameters)
{
	...
	if (status != 0) {
		int err = 901;
		addIdamError(CODEERRORTYPE, "WEST:ERROR: error calling getECEModeObject()", err, "");
		free(O_ModeData);
		free(O_ObjectName);
		return status;
	}

	IDAM_LOGF(UDA_LOG_DEBUG, "O_ObjectName: %s\n", O_ObjectName);
	...
	free(O_ModeData);
	free(O_ObjectName);
	IDAM_LOG(UDA_LOG_DEBUG, "returning from getECEModeHarmonicTime()");
	return status;
		...
		char* TOP_collections_parameters)
{
	...
	if (status != 0) {
		int err = 901;
		IDAM_LOG(UDA_LOG_ERROR, "WEST:ERROR: unable to get ECE harmonic mode 1 signal\n");
		addIdamError(CODEERRORTYPE, "WEST:ERROR: unable to get signal", err, "");
		free(O_ModeTime);
		free(O_ModeData);
		free(O_ObjectName);
		IDAM_LOG(UDA_LOG_DEBUG, "after freeing...");
		return status;
	}
...
int getECEModeObject(char* TOP_collections_parameters, char** objectName, int GVSHArray, int mode)
{
	...
	if (status != 0) {
		int err = 901;
		IDAM_LOG(UDA_LOG_ERROR, "unable to get command for ECE\n");
	...
	*objectName = strdup(result);
	IDAM_LOGF(UDA_LOG_DEBUG, "ECE object name for mode %d: %s\n", mode, *objectName);
	free(command);
	IDAM_LOG(UDA_LOG_DEBUG, "returning from getECEModeObject...\n");
	return 0;
}
