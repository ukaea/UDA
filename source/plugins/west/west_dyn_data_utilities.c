		...
		char* TOP_collections_parameters, char* attributes, char* normalizationAttributes, int setTime)
{
	...
	if (status != 0) {
		...
		char msg[1000];
		strcpy(msg, errorMsg);
		sprintf(msg, "%s,shot:%d\n", TOP_collections_parameters, shotNumber);
		...
		char* TOP_collections_parameters, char* attributes)
{
	...
	if (status != 0) {
		int err = 901;
		char errorMsg[1000];
		strcpy(errorMsg, "WEST:ERROR: unable to get command for TOP parameters:");
	...
	if (status != 0) {
		int err = 901;
		UDA_LOG(UDA_LOG_DEBUG, "WEST:ERROR: error reading signal: %s\n", objectName);
		char* errorMsg = "WEST:ERROR (GetDynData): error reading signal";
		char msg[1000];
		sprintf(msg, "%s,objectName:%s,shot:%d\n", errorMsg, objectName, shotNumber);
		UDA_LOG(UDA_LOG_ERROR, "%s", msg);
		addIdamError(CODEERRORTYPE, msg, err, "");
		free(command);
		free(objectName);
	...
	if (*len == 0) {
		int err = 901;
		char msg[1000];
		char* errorMsg = "WEST:ERROR (GetDynData): dynamic data empty";
		sprintf(msg, "%s,objectName:%s,shot:%d\n", errorMsg, objectName, shotNumber);
		UDA_LOG(UDA_LOG_ERROR, "%s", msg);
		addIdamError(CODEERRORTYPE, msg, err, "");
		status = -1;
...
int getArcadeSignal(char* nomsigp, int shotNumber, int extractionIndex, float** data_time, float** data, int* len, float normalizationFactor)
{
	...
	/*if (status != 0) {
		int err = 901;
		char* errorMsg = "WEST:ERROR (getArcadeSignal): error reading signal";
		char msg[1000];
		sprintf(msg, "%s,objectName:%s,shot:%d\n", errorMsg, nomsigp_to_extract, shotNumber);
		UDA_LOG(UDA_LOG_ERROR, "%s", msg);
		addIdamError(CODEERRORTYPE, msg, err, "");
	}*/
	if ((status == 0) && (*len != 0))
...
int averageArcadeSignal(char* sigName, int shotNumber, int extractions[], int extractions_length, float** data_time, float** averaged_data, int* len) {
