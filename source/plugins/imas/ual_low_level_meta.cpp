...
void putInfoWithData(int expIdx, char* cpoPath, char* path, int exists, int nDims, int* dims, int itemSize, char type,
...
int getInfoWithDataNoLock(int expIdx, char* cpoPath, char* path, int* exists, int* nDims, int* dims, int* itemSize,
...
void internalFlush(int expIdx, char* cpoPath, char* path, char* timeBasePath, int isSliced, int isObject, int nDims,
                   int* dims, int itemSize, int dtype, int dataSize, char* data, int numSlices, int* sliceOffsets);
...
//Gabriele June 2012Handle deletion of object node containing timed and non timed children
    ...
    virtual bool
    getInfo(char* path, bool* exists, int* nDims, int* dims, int* itemSize, char* type, int* dataSize, char** data)
    {
        int idx;
        char* name1;
        idx = getFirstSlashOccurrence(path);
...
//Added to keep track of the original path name for time information
    ...
    void putInfo(char* name, bool exists, int nDims, int* dims, int itemSize, char type, int dataSize, char* data,
                 bool isObject)
    {
...
*/
    ...
    virtual bool
    getInfo(char* name, bool* exists, int* nDims, int* dims, int* itemSize, char* type, int* dataSize, char** data)
    {

        //printf("virtual GET INFO FOR %s EXISTS: %d dataSIze: %d NUM CHILDREN: %d\n", this->name, this->exists, this->dataSize, numChildren);
        if (numChildren >
            0) //Handle object components which may be created as leaf even when having timed and nontimed children
        {
    ...
    virtual void
    appendSlice(char* name, char* timeBasePath, int nDims, int* dims, int itemSize, char type, int sliceSize,
                char* slice)
    {
/*	if(strcmp(this->name, name)|| getFirstSlashOccurrence(name) != -1)
	{
...
//////////////////
    ...
    virtual void
    appendSliceSet(char* name, char* timeBasePath, int nDims, int* dims, int itemSize, char type, int sliceSize,
                   char* slice, int numSlices)
    {
/*	if(strcmp(this->name, name)|| getFirstSlashOccurrence(name) != -1)
	{
...
//////////////////
    ...
    virtual void appendInfoObjectSlice(char* path, char* buf, int size)
    {
        ...
        if (bufferSize < dataSize + size) {//Make room for more STEP_SLICES slices assuming the passed size
...
//    	    internalFlush(expIdx, cpoPath, path, "dummyTimeBasePath", isSliced, isObject, nDims, dims, itemSize, type, dataSize, data, numSlices, sliceOffsets);
            internalFlush(expIdx, cpoPath, path, timeBasePath, isSliced, isObject, nDims, dims, itemSize, type,
                          dataSize, data, numSlices, sliceOffsets);
            delete[] path;
        }
    }
...
static void putInfoLocal(int expIdx, char* cpoPath, char* path, int exists, int nDims, int* dims, bool isObject = false,
                         int itemSize = 0, char type = 0, int dataSize = 0, char* data = 0)
{
    int idx;
...
static int getInfoLocal(int expIdx, char* cpoPath, char* path, int* exists, int* nDims, int* dims, int* itemSize = 0,
                        char* type = 0, int* dataSize = 0, char** data = 0, bool lock = true)
{
