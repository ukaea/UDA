...
class PathLeaf;
class PathNode;
static PathLeaf *createPathLeaf(PathNode *parent, const char *name, bool exists, int nDims, int *dims, int itemSize, char type, int dataSize, char *data);
...
public:
    ...
    virtual void putInfo(const char *path, bool exists, int nDims, int *dims, int itemSize, char type, int dataSize, char *data, bool isObject)
    {
...
//Gabriele June 2012Handle deletion of object node containing timed and non timed children
    ...
    virtual void appendSliceSet(const char *path, char *timeBasePath, int nDims, int *dims, int itemSize, char type, int sliceSize, char *slice, int numSlices)
    {
        PathNode *leaf = getLeaf(path);
	leaf->appendSliceSet("", timeBasePath, nDims, dims, itemSize, type, sliceSize, slice, numSlices);
    }

    virtual void appendSlice(const char *path, char *timeBasePath, int nDims, int *dims, int itemSize, char type, int sliceSize, char *slice)
    {
        PathNode *leaf = getLeaf(path);
...
//Added to keep track of the original path name for time information
    ...
    PathLeaf(PathNode *parent, const char *name, bool exists, int nDims, int *dims, int itemSize = 0, char type = 0, int dataSize = 0, char *data = 0):PathNode(parent, name)
    {
