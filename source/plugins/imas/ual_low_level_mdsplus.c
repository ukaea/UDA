...
static void updateInfo(int expIdx, char* cpoPath, char* path, struct descriptor* dataD, int isObject)
{
    ...
    if (dataD->class == CLASS_S) {
        ...
        putInfoWithData(expIdx, cpoPath, path, 1, 0, dims, dataD->length, dataD->dtype, dataD->length, dataD->pointer,
                        0); //Cannot be a serialized object
        return;
    }
    ...
    putInfoWithData(expIdx, cpoPath, path, 1, nDims, dims, arrayDPtr->length, arrayDPtr->dtype, arrayDPtr->arsize,
                    arrayDPtr->pointer, isObject);
...
static void updateInfoObjectSlice(int expIdx, char* cpoPath, char* path, void* obj)
{
    ...
    if (numSamples ==
        1) { //The object has been passed by put/replaceObjectSlice and therefore it is an array with 1 elements
...
static int getInfoObject(int expIdx, char* cpoPath, char* path, int* exists, void** retObj, int isTimed)
{
    ...
    if (!isTimed) {
        infoExists = getInfoWithData(expIdx, cpoPath, fullPath, exists, &nDims, dims, &itemSize, &dtype, &dataSize,
                                     &data);
        if (!infoExists) {
            free(fullPath);
            return 0;
...
void internalFlush(int expIdx, char* cpoPath, char* path, char* timeBasePath, int isSliced, int isObject, int nDims,
                   int* dims, int itemSize, int dtype, int dataSize,
...
// Remove previous data, if any
            if (isExpRemote(expIdx)) {
                status = putDataRemote(getExpConnectionId(expIdx), getExpRemoteIdx(expIdx), cpoPath, path,
                                       (struct descriptor*)&emptyXd);
            } else {
...
//Non Object management follows
    ...
    } else //Array
    {
        ...
        if (isSliced) {
            ...
            getInfoWithDataNoLock(expIdx, cpoPath, timeBasePath, &timeExists, &timeNDims, timeDims, &timeItemSize,
                                  &timeType, &timeDataSize,
                                  (char**)&times); // This is getting the timebase from the structure in memory, should be fine now
            if (!timeExists) {
...
//Write ALL slices as a unique segment
            if (isExpRemote(expIdx)) {
                status = putSegmentRemote(getExpConnectionId(expIdx), getExpRemoteIdx(expIdx), cpoPath, path,
                                          timeBasePath, (void*)&arrayD, times, timeDims[0]);
            } else {
...
static void appendSliceData(int expIdx, char* cpoPath, char* path, char* timeBasePath, struct descriptor* dataD)
{
    ...
    if (dataD->class == CLASS_S) {
        appendSlice(expIdx, cpoPath, path, timeBasePath, 0, dims, dataD->length, dataD->dtype, dataD->length,
                    dataD->pointer);
        return;
    }
    arrayDPtr = (void*)dataD;
    ...
    appendSlice(expIdx, cpoPath, path, timeBasePath, nDims, dims, arrayDPtr->length, arrayDPtr->dtype,
                arrayDPtr->arsize, arrayDPtr->pointer);
}
...
static void appendSliceSetData(int expIdx, char* cpoPath, char* path, char* timeBasePath, struct descriptor_a* dataD)
{
    ...
    appendSliceSet(expIdx, cpoPath, path, timeBasePath, nDims, dims, arrayDPtr->length, arrayDPtr->dtype,
                   arrayDPtr->arsize, arrayDPtr->pointer, dims[nDims - 1]);
}
...
int getDataNoDescr(int expIdx, char* cpoPath, char* path, void** retData, int* retDims, int* retDimsCt, char* retType,
                   char* retClass, int* retLength, int64_t* retArsize, int evaluate)
{
    int status, i;
...
//Argument time is no more used in getSlicedData()
    ...
    if (isExpRemote(expIdx)) {
        return putSegmentRemote(getExpConnectionId(expIdx), getExpRemoteIdx(expIdx), cpoPath, path, timeBasePath, dataD,
                                times, nTimes);
    } else {
...
//First get Data, if any so that it is correctly posted in the memory cache
    ...
    if (isExpRemote(expIdx)) {
        return putSliceRemote(getExpConnectionId(expIdx), getExpRemoteIdx(expIdx), cpoPath, path, timeBasePath, dataD,
                              time);
    } else {
...
int
getSlicedData(int expIdx, char* cpoPath, char* path, char* timeBasePath, double time, struct descriptor_xd* retDataXd,
...
int mdsgetSlicedData(int expIdx, char* cpoPath, char* path, char* timeBasePath, double time,
                     struct descriptor_xd* retDataXd,
                     struct descriptor_xd* retTimesXd, int expandObject)
{
    int status;
    ...
    if (!status) {
        status = mdsgetData(expIdx, cpoPath, timeBasePath, retTimesXd,
                            0); // BEWARE: internal compacted name for timeBasePath
    }
    return status;
}
...
static void setDataEnv(char* user, char* tokamak, char* version)
{
    ...
    if (!strcmp(user, "public")) {
        ...
        sprintf(treeBase1,
                "/pfs/imasdb/imas_trees/public/%s/%s/mdsplus/1;/pfs/imasdb/imas_trees/public/models/%s/mdsplus",
                tokamak, version, version);
        sprintf(treeBase2,
                "/pfs/imasdb/imas_trees/public/%s/%s/mdsplus/2;/pfs/imasdb/imas_trees/public/models/%s/mdsplus",
                tokamak, version, version);
        sprintf(treeBase3,
                "/pfs/imasdb/imas_trees/public/%s/%s/mdsplus/3;/pfs/imasdb/imas_trees/public/models/%s/mdsplus",
                tokamak, version, version);
        sprintf(treeBase4,
                "/pfs/imasdb/imas_trees/public/%s/%s/mdsplus/4;/pfs/imasdb/imas_trees/public/models/%s/mdsplus",
                tokamak, version, version);
        sprintf(treeBase5,
                "/pfs/imasdb/imas_trees/public/%s/%s/mdsplus/5;/pfs/imasdb/imas_trees/public/models/%s/mdsplus",
                tokamak, version, version);
        sprintf(treeBase6,
                "/pfs/imasdb/imas_trees/public/%s/%s/mdsplus/6;/pfs/imasdb/imas_trees/public/models/%s/mdsplus",
                tokamak, version, version);
        sprintf(treeBase7,
                "/pfs/imasdb/imas_trees/public/%s/%s/mdsplus/7;/pfs/imasdb/imas_trees/public/models/%s/mdsplus",
                tokamak, version, version);
        sprintf(treeBase8,
                "/pfs/imasdb/imas_trees/public/%s/%s/mdsplus/8;/pfs/imasdb/imas_trees/public/models/%s/mdsplus",
                tokamak, version, version);
        sprintf(treeBase9,
...
int mdsimasCreateEnv(char* name, int shot, int run, int refShot, int refRun, int* retIdx, char* user, char* tokamak,
                     char* version)
{
    int status;
    int remoteIdx;
                    ...
                    char* retClass, int* retLength, int64_t* retArsize, int evaluate)
{
    ...
    if (numSegments > 0) {
        EMPTYXD(retTimesXd);
        unlock();
        status = getTimedDataNoDescr(expIdx, cpoPath, path, 0, 0, retData, retDims, retDimsCt, retType, retClass,
                                     retLength, retArsize, &retTimesXd, 1);
        if (!status) {
            MdsFree1Dx(&retTimesXd, 0);
        }
        return status;
...
static int
getNonSegmentedTimedDataNoDescr(int expIdx, char* cpoPath, char* path, void** retData, int* retDims, int* retDimsCt,
                                char* retType, char* retClass, int* retLength, int64_t* retArsize,
                                struct descriptor_xd* retTimesXd)
{
    struct descriptor_signal* signalD;
    struct descriptor_a* longTimesD;
                 ...
                 struct descriptor_xd* retTimesXd, int all)
{
    ...
    status = TreeGetNumSegments(nid, &numSegments);
    if (!(status & 1)) {
        sprintf(errmsg, "Error getting number of segments for IDS: %s, field: %s - %s", cpoPath, path,
                MdsGetMsg(status));
        unlock();
        return -1;
    }
                    ...
                    struct descriptor_xd* retTimesXd, int all)
{
    ...
    if (numSegments == 0) {
        unlock();
        status = getNonSegmentedTimedDataNoDescr(expIdx, cpoPath, path, retData, retDims, retDimsCt, retType, retClass,
                                                 retLength, retArsize, retTimesXd);
        return status;
    }
...
int
getDataAndSliceIdxs(int expIdx, char* cpoPath, char* path, char* timeBasePath, struct descriptor_xd* xd, double time,
                    int* sliceId1, int* sliceId2,
...
//        printf("NID = %d\n", nid);
    ...
    if (!(status & 1)) {
        sprintf(errmsg, "Cannot get num segments segment at path %s, IDS path %s: %s", path, cpoPath,
                MdsGetMsg(status));
        unlock();
        return -1;
    }
//    printf("ual_low_level_mdsplus : putSegmentLocal converted paths before CheckedSegment (cpopath, timebasepath) : %s , %s \n", convertPath(cpoPath),convertAnotherPath(timeBasePath));
    sprintf(segmentExpr, "data(GetCheckedSegment(build_path(\"%s:%s\"),%d))", convertPath(cpoPath),
            mdsconvertPath(timeBasePath), currSegment);
...
// segmentExpr = data(GetCheckedSegment(build_path("pf0:coils0/1/current0/timebase0"),0))
    ...
    if (!(status & 1)) {
        sprintf(errmsg, "Internal error at path %s, IDS path %s: %s: cannot compile %s", path, cpoPath,
                MdsGetMsg(status), segmentExpr);
        unlock();
        return -1;
    }
    if (currSegment == 0) {
    ...
    if (leftItems > 0) {
        ...
        if (prevArsize > leftBytes) //Write remaining segment
        {
            ...
            if (!(status & 1)) {
                ...
                sprintf(errmsg, "Cannot write data segment at path %s, IDS path %s: %s", path, cpoPath,
                        MdsGetMsg(status));
                unlock();
                return -1;
            }
...
//        sprintf(segmentExpr, "data(GetCheckedSegment(%s:time,%d))", convertPath(cpoPath), currSegment);
    ...
    if (leftItems > 0) {
        ...
        if (leftBytes < dataLen) {
            sprintf(errmsg, "Internal error in putSlice at path %s, IDS path %s: %s: wrong segment size", path, cpoPath,
                    MdsGetMsg(status));
            unlock();
            return -1;
        }
...
*/
    ...
    for (i = 0; i < dim; i++) { //printf("mdsPutVect1DString. strlen %d, max: %d\n",strlen(data[i]), maxLen);
                                ...
                                int nTimes)
{
    ...
    if (nTimes > 0) { //printf("ual_low_level_mdsplus : putTimedVect1DDouble: just before putSegment\n");
        status = putSegment(expIdx, cpoPath, path, timeBasePath, (struct descriptor_a*)&dataD, times, nTimes);
...
int
mdsPutVect2DInt(int expIdx, char* cpoPath, char* path, char* timeBasePath, int* data, int dim1, int dim2, int isTimed)
{
    EMPTYXD(emptyXd);
...
int mdsPutVect2DFloat(int expIdx, char* cpoPath, char* path, char* timeBasePath, float* data, int dim1, int dim2,
                      int isTimed)
{
    EMPTYXD(emptyXd);
...
int mdsPutVect2DDouble(int expIdx, char* cpoPath, char* path, char* timeBasePath, double* data, int dim1, int dim2,
                       int isTimed)
{
    EMPTYXD(emptyXd);
...
int mdsPutVect3DInt(int expIdx, char* cpoPath, char* path, char* timeBasePath, int* data, int dim1, int dim2, int dim3,
                    int isTimed)
{
    EMPTYXD(emptyXd);
...
int
mdsPutVect3DFloat(int expIdx, char* cpoPath, char* path, char* timeBasePath, float* data, int dim1, int dim2, int dim3,
                  int isTimed)
{
    EMPTYXD(emptyXd);
...
int mdsPutVect3DDouble(int expIdx, char* cpoPath, char* path, char* timeBasePath, double* data, int dim1, int dim2,
                       int dim3, int isTimed)
{
    EMPTYXD(emptyXd);
...
int
mdsPutVect4DFloat(int expIdx, char* cpoPath, char* path, char* timeBasePath, float* data, int dim1, int dim2, int dim3,
                  int dim4, int isTimed)
{
    EMPTYXD(emptyXd);
...
int mdsPutVect4DDouble(int expIdx, char* cpoPath, char* path, char* timeBasePath, double* data, int dim1, int dim2,
                       int dim3, int dim4, int isTimed)
{
    EMPTYXD(emptyXd);
...
int mdsPutVect4DInt(int expIdx, char* cpoPath, char* path, char* timeBasePath, int* data, int dim1, int dim2, int dim3,
                    int dim4, int isTimed)
{
    EMPTYXD(emptyXd);
...
int
mdsPutVect5DFloat(int expIdx, char* cpoPath, char* path, char* timeBasePath, float* data, int dim1, int dim2, int dim3,
                  int dim4, int dim5, int isTimed)
{
    EMPTYXD(emptyXd);
    DESCRIPTOR_A_COEFF(dataD, sizeof(float), DTYPE_FLOAT, 0, 5, 0);
    int status;
    if (isTimed) {
        return putTimedVect5DFloat(expIdx, cpoPath, path, timeBasePath, data, putTimes, nPutTimes, dim1, dim2, dim3,
                                   dim4);
    }
...
int mdsPutVect5DDouble(int expIdx, char* cpoPath, char* path, char* timeBasePath, double* data, int dim1, int dim2,
                       int dim3, int dim4, int dim5, int isTimed)
{
    EMPTYXD(emptyXd);
    DESCRIPTOR_A_COEFF(dataD, sizeof(double), DTYPE_DOUBLE, 0, 5, 0);
    int status;
    if (isTimed) {
        return putTimedVect5DDouble(expIdx, cpoPath, path, timeBasePath, data, putTimes, nPutTimes, dim1, dim2, dim3,
                                    dim4);
    }
...
int mdsPutVect5DInt(int expIdx, char* cpoPath, char* path, char* timeBasePath, int* data, int dim1, int dim2, int dim3,
                    int dim4, int dim5, int isTimed)
{
    EMPTYXD(emptyXd);
    ...
    int status;
    if (isTimed) {
        return putTimedVect5DInt(expIdx, cpoPath, path, timeBasePath, data, putTimes, nPutTimes, dim1, dim2, dim3,
                                 dim4);
    }
    dataD.arsize = dim1 * dim2 * dim3 * dim4 * dim5 * sizeof(double);
    dataD.m[0] = dim1;
...
int
mdsPutVect6DFloat(int expIdx, char* cpoPath, char* path, char* timeBasePath, float* data, int dim1, int dim2, int dim3,
                  int dim4, int dim5, int dim6, int isTimed)
{
    ...
    int status;
    if (isTimed) {
        return putTimedVect6DFloat(expIdx, cpoPath, path, timeBasePath, data, putTimes, nPutTimes, dim1, dim2, dim3,
                                   dim4, dim5);
    }
...
int mdsPutVect6DDouble(int expIdx, char* cpoPath, char* path, char* timeBasePath, double* data, int dim1, int dim2,
                       int dim3,
                       int dim4, int dim5, int dim6, int isTimed)
{
    EMPTYXD(emptyXd);
    ...
    int status;
    if (isTimed) {
        return putTimedVect6DDouble(expIdx, cpoPath, path, timeBasePath, data, putTimes, nPutTimes, dim1, dim2, dim3,
                                    dim4, dim5);
    }
...
int mdsPutVect6DInt(int expIdx, char* cpoPath, char* path, char* timeBasePath, int* data, int dim1, int dim2, int dim3,
                    int dim4,
                    int dim5, int dim6, int isTimed)
{
    ...
    int status;
    if (isTimed) {
        return putTimedVect6DInt(expIdx, cpoPath, path, timeBasePath, data, putTimes, nPutTimes, dim1, dim2, dim3, dim4,
                                 dim5);
    }
    dataD.arsize = dim1 * dim2 * dim3 * dim4 * dim5 * dim6 * sizeof(double);
    dataD.m[0] = dim1;
...
int
mdsPutVect7DFloat(int expIdx, char* cpoPath, char* path, char* timeBasePath, float* data, int dim1, int dim2, int dim3,
                  int dim4, int dim5, int dim6, int dim7, int isTimed)
{
    ...
    int status;
    if (isTimed) {
        return putTimedVect7DFloat(expIdx, cpoPath, path, timeBasePath, data, putTimes, nPutTimes, dim1, dim2, dim3,
                                   dim4, dim5, dim6);
    }
...
int mdsPutVect7DDouble(int expIdx, char* cpoPath, char* path, char* timeBasePath, double* data, int dim1, int dim2,
                       int dim3,
                       int dim4, int dim5, int dim6, int dim7, int isTimed)
{
    EMPTYXD(emptyXd);
    ...
    int status;
    if (isTimed) {
        return putTimedVect7DDouble(expIdx, cpoPath, path, timeBasePath, data, putTimes, nPutTimes, dim1, dim2, dim3,
                                    dim4, dim5, dim6);
    }
...
int mdsPutVect7DInt(int expIdx, char* cpoPath, char* path, char* timeBasePath, int* data, int dim1, int dim2, int dim3,
                    int dim4,
                    int dim5, int dim6, int dim7, int isTimed)
{
    ...
    int status;
    if (isTimed) {
        return putTimedVect7DInt(expIdx, cpoPath, path, timeBasePath, data, putTimes, nPutTimes, dim1, dim2, dim3, dim4,
                                 dim5, dim6);
    }
    dataD.arsize = dim1 * dim2 * dim3 * dim4 * dim5 * dim6 * dim7 * sizeof(double);
    dataD.m[0] = dim1;
...
int
mdsPutVect1DDoubleSlice(int expIdx, char* cpoPath, char* path, char* timeBasePath, double* data, int dim, double time)
{
    EMPTYXD(emptyXd);
    DESCRIPTOR_A(dataD, sizeof(double), DTYPE_DOUBLE, 0, 0);
    int status;
...
int mdsPutVect2DIntSlice(int expIdx, char* cpoPath, char* path, char* timeBasePath, int* data, int dim1, int dim2,
                         double time)
{
    EMPTYXD(emptyXd);
    DESCRIPTOR_A_COEFF(dataD, sizeof(int), DTYPE_L, 0, 2, 0);
...
int mdsPutVect2DFloatSlice(int expIdx, char* cpoPath, char* path, char* timeBasePath, float* data, int dim1, int dim2,
                           double time)
{
    EMPTYXD(emptyXd);
    DESCRIPTOR_A_COEFF(dataD, sizeof(float), DTYPE_FLOAT, 0, 2, 0);
...
int mdsPutVect2DDoubleSlice(int expIdx, char* cpoPath, char* path, char* timeBasePath, double* data, int dim1, int dim2,
                            double time)
{
    EMPTYXD(emptyXd);
    DESCRIPTOR_A_COEFF(dataD, sizeof(double), DTYPE_DOUBLE, 0, 2, 0);
...
int
mdsPutVect3DIntSlice(int expIdx, char* cpoPath, char* path, char* timeBasePath, int* data, int dim1, int dim2, int dim3,
                     double time)
{
    EMPTYXD(emptyXd);
    DESCRIPTOR_A_COEFF(dataD, sizeof(int), DTYPE_L, 0, 3, 0);
...
int mdsPutVect3DFloatSlice(int expIdx, char* cpoPath, char* path, char* timeBasePath, float* data, int dim1, int dim2,
                           int dim3, double time)
{
    EMPTYXD(emptyXd);
    DESCRIPTOR_A_COEFF(dataD, sizeof(float), DTYPE_FLOAT, 0, 3, 0);
    int status;
...
int mdsPutVect3DDoubleSlice(int expIdx, char* cpoPath, char* path, char* timeBasePath, double* data, int dim1, int dim2,
                            int dim3, double time)
{
    EMPTYXD(emptyXd);
    DESCRIPTOR_A_COEFF(dataD, sizeof(double), DTYPE_DOUBLE, 0, 3, 0);
...
int mdsPutVect4DFloatSlice(int expIdx, char* cpoPath, char* path, char* timeBasePath, float* data, int dim1, int dim2,
                           int dim3, int dim4, double time)
{
    EMPTYXD(emptyXd);
...
int mdsPutVect4DDoubleSlice(int expIdx, char* cpoPath, char* path, char* timeBasePath, double* data, int dim1, int dim2,
                            int dim3, int dim4, double time)
{
    EMPTYXD(emptyXd);
    DESCRIPTOR_A_COEFF(dataD, sizeof(double), DTYPE_DOUBLE, 0, 4, 0);
...
int
mdsPutVect4DIntSlice(int expIdx, char* cpoPath, char* path, char* timeBasePath, int* data, int dim1, int dim2, int dim3,
                     int dim4, double time)
{
    EMPTYXD(emptyXd);
    DESCRIPTOR_A_COEFF(dataD, sizeof(double), DTYPE_L, 0, 4, 0);
    int status;
...
int mdsPutVect5DFloatSlice(int expIdx, char* cpoPath, char* path, char* timeBasePath, float* data, int dim1, int dim2,
                           int dim3,
...
int mdsPutVect5DDoubleSlice(int expIdx, char* cpoPath, char* path, char* timeBasePath, double* data, int dim1, int dim2,
                            int dim3,
                            int dim4, int dim5, double time)
{
    EMPTYXD(emptyXd);
...
int
mdsPutVect5DIntSlice(int expIdx, char* cpoPath, char* path, char* timeBasePath, int* data, int dim1, int dim2, int dim3,
                     int dim4, int dim5, double time)
{
    EMPTYXD(emptyXd);
    DESCRIPTOR_A_COEFF(dataD, sizeof(double), DTYPE_L, 0, 5, 0);
...
/////6D Slice
int mdsPutVect6DFloatSlice(int expIdx, char* cpoPath, char* path, char* timeBasePath, float* data, int dim1, int dim2,
                           int dim3,
...
int mdsPutVect6DDoubleSlice(int expIdx, char* cpoPath, char* path, char* timeBasePath, double* data, int dim1, int dim2,
                            int dim3,
                            int dim4, int dim5, int dim6, double time)
{
    EMPTYXD(emptyXd);
...
int
mdsPutVect6DIntSlice(int expIdx, char* cpoPath, char* path, char* timeBasePath, int* data, int dim1, int dim2, int dim3,
                     int dim4, int dim5, int dim6, double time)
{
    EMPTYXD(emptyXd);
    DESCRIPTOR_A_COEFF(dataD, sizeof(double), DTYPE_L, 0, 6, 0);
...
int mdsReplaceLastVect4DFloatSlice(int expIdx, char* cpoPath, char* path, float* data, int dim1, int dim2, int dim3,
                                   int dim4)
{
    EMPTYXD(emptyXd);
    DESCRIPTOR_A_COEFF(dataD, sizeof(double), DTYPE_FLOAT, 0, 4, 0);
...
int mdsReplaceLastVect4DDoubleSlice(int expIdx, char* cpoPath, char* path, double* data, int dim1, int dim2, int dim3,
                                    int dim4)
{
    EMPTYXD(emptyXd);
    DESCRIPTOR_A_COEFF(dataD, sizeof(double), DTYPE_DOUBLE, 0, 4, 0);
...
int
mdsReplaceLastVect4DIntSlice(int expIdx, char* cpoPath, char* path, int* data, int dim1, int dim2, int dim3, int dim4)
{
    EMPTYXD(emptyXd);
    DESCRIPTOR_A_COEFF(dataD, sizeof(double), DTYPE_L, 0, 4, 0);
    int status;
...
static int
mdsGetLocalDimension(int expIdx, char* cpoPath, char* path, int* numDims, int* dim1, int* dim2, int* dim3, int* dim4,
...
int mdsGetDimension(int expIdx, char* cpoPath, char* path, int* numDims, int* dim1, int* dim2, int* dim3, int* dim4,
                    int* dim5, int* dim6, int* dim7)
{
    EMPTYXD(xd);
    int status;
...
//////////////Memory-Based dimension / existence info management///////////////
	...
	*/
        ...
        *dim1 = dims[0];
    }   /* m array not declared if not a multidimensional array */
    if (dimCt > 1)
        *dim2 = dims[1];
    if (dimCt > 2)
        *dim3 = dims[2];
    ...
    if (!isTimed) {
        *retSamples = 1;
        ...
        if (status || !xd.l_length || xd.pointer->class != CLASS_A) {
            ...
            sprintf(errmsg, "Internal error: Time not found for beginIdsGet at path %s: %s", path,
                    MdsGetMsg(mdsStatus));
            mdsendIdsGet(expIdx, path);
            return -1;
        }
...
int mdsGetIntSlice(int expIdx, char* cpoPath, char* path, char* timeBasePath, int* data, double time, double* retTime,
                   int interpolMode)
{
    struct descriptor_a* arrayD;
    int y1, y2;
    ...
    EMPTYXD(xd);
    status = getDataAndSliceIdxs(expIdx, cpoPath, path, timeBasePath, &xd, time, &sliceIdx1, &sliceIdx2, &sliceTime1,
                                 &sliceTime2, &nTimes);
    if (status) {
    ...
    switch (interpolMode) {
        case INTERPOLATION:
            *data = (sliceTime1 == sliceTime2) ? y1 : y1 + ((double)y2 - (double)y1) * (time - sliceTime1) /
                                                           (sliceTime2 - sliceTime1);
...
int
mdsGetFloatSlice(int expIdx, char* cpoPath, char* path, char* timeBasePath, float* data, double time, double* retTime,
                 int interpolMode)
{
    struct descriptor_a* arrayD;
    float y1, y2;
...
int
mdsGetDoubleSlice(int expIdx, char* cpoPath, char* path, char* timeBasePath, double* data, double time, double* retTime,
                  int interpolMode)
{
    struct descriptor_a* arrayD;
    double y1, y2;
    ...
    if (status || !xd.l_length || xd.pointer->class != CLASS_A ||
        (xd.pointer->dtype != DTYPE_DOUBLE && xd.pointer->dtype != DTYPE_FT)) {
...
int mdsGetVect1DIntSlice(int expIdx, char* cpoPath, char* path, char* timeBasePath, int** data, int* dim, double time,
                         double* retTime, int interpolMode)
{
    ...
    *dim = nItems = arrayD->m[0];
    ...
    for (i = 0; i < nItems; i++) {
        switch (interpolMode) {
            case INTERPOLATION:
                retData[i] = (sliceTime1 == sliceTime2) ? y1[i] : y1[i] + ((double)y2[i] - (double)y1[i]) *
                                                                          (time - sliceTime1) /
                                                                          (sliceTime2 - sliceTime1);
                *retTime = (sliceTime1 == sliceTime2) ? sliceTime1 : time;
                break;
...
int
mdsGetVect1DFloatSlice(int expIdx, char* cpoPath, char* path, char* timeBasePath, float** data, int* dim, double time,
                       double* retTime, int interpolMode)
{
    ARRAY_BOUNDS(int, 2) * arrayD;
    float* y1, * y2;
...
int
mdsGetStringSlice(int expIdx, char* cpoPath, char* path, char* timeBasePath, char** data, double time, double* retTime,
                  int interpolMode)
{
    ARRAY_BOUNDS(int, 2) * arrayD;
    char* y1, * y2;
...
int
mdsGetVect1DDoubleSlice(int expIdx, char* cpoPath, char* path, char* timeBasePath, double** data, int* dim, double time,
                        double* retTime, int interpolMode)
{
    ARRAY_BOUNDS(int, 2) * arrayD;
    double* y1, * y2;
    ...
    if (!xd.l_length || xd.pointer->class != CLASS_A ||
        (xd.pointer->dtype != DTYPE_DOUBLE && xd.pointer->dtype != DTYPE_FT)) {
        sprintf(errmsg, "Internal error: not a int array at IDS path %s, path %s", cpoPath, path);
        return -1;
    }
    ...
    for (i = 0; i < nItems; i++) {
        switch (interpolMode) {
            case INTERPOLATION:
                ...
                retData[i] = (sliceTime1 == sliceTime2) ? y1[i] : y1[i] + (y2[i] - y1[i]) * (time - sliceTime1) /
                                                                          (sliceTime2 - sliceTime1);
                *retTime = (sliceTime1 == sliceTime2) ? sliceTime1 : time;
                break;
            case CLOSEST_SAMPLE:
...
int mdsGetVect2DIntSlice(int expIdx, char* cpoPath, char* path, char* timeBasePath, int** data, int* dim1, int* dim2,
...
int
mdsGetVect2DFloatSlice(int expIdx, char* cpoPath, char* path, char* timeBasePath, float** data, int* dim1, int* dim2,
                       double time, double* retTime, int interpolMode)
{
    ARRAY_BOUNDS(int, 2) * arrayD;
    float* y1, * y2;
...
int
mdsGetVect2DDoubleSlice(int expIdx, char* cpoPath, char* path, char* timeBasePath, double** data, int* dim1, int* dim2,
                        double time, double* retTime, int interpolMode)
{
    ARRAY_BOUNDS(int, 2) * arrayD;
    double* y1, * y2;
...
int mdsGetVect3DIntSlice(int expIdx, char* cpoPath, char* path, char* timeBasePath, int** data, int* dim1, int* dim2,
                         int* dim3, double time, double* retTime, int interpolMode)
{
...
int
mdsGetVect3DFloatSlice(int expIdx, char* cpoPath, char* path, char* timeBasePath, float** data, int* dim1, int* dim2,
                       int* dim3, double time, double* retTime, int interpolMode)
{
    ARRAY_BOUNDS(int, 3) * arrayD;
    float* y1, * y2;
...
int
mdsGetVect3DDoubleSlice(int expIdx, char* cpoPath, char* path, char* timeBasePath, double** data, int* dim1, int* dim2,
                        int* dim3, double time, double* retTime, int interpolMode)
{
    ARRAY_BOUNDS(int, 3) * arrayD;
    double* y1, * y2;
...
//////////
int mdsGetVect4DIntSlice(int expIdx, char* cpoPath, char* path, char* timeBasePath, int** data, int* dim1, int* dim2,
...
int
mdsGetVect4DFloatSlice(int expIdx, char* cpoPath, char* path, char* timeBasePath, float** data, int* dim1, int* dim2,
                       int* dim3, int* dim4, double time, double* retTime, int interpolMode)
{
    ARRAY_BOUNDS(int, 4) * arrayD;
    float* y1, * y2;
...
int
mdsGetVect4DDoubleSlice(int expIdx, char* cpoPath, char* path, char* timeBasePath, double** data, int* dim1, int* dim2,
                        int* dim3, int* dim4, double time, double* retTime, int interpolMode)
{
    ARRAY_BOUNDS(int, 4) * arrayD;
    double* y1, * y2;
...
///////////
int mdsGetVect5DIntSlice(int expIdx, char* cpoPath, char* path, char* timeBasePath, int** data, int* dim1, int* dim2,
                         int* dim3,
...
int
mdsGetVect5DFloatSlice(int expIdx, char* cpoPath, char* path, char* timeBasePath, float** data, int* dim1, int* dim2,
...
int
mdsGetVect5DDoubleSlice(int expIdx, char* cpoPath, char* path, char* timeBasePath, double** data, int* dim1, int* dim2,
                        int* dim3, int* dim4, int* dim5, double time, double* retTime, int interpolMode)
{
    ARRAY_BOUNDS(int, 5) * arrayD;
    double* y1, * y2;
...
//////////6D Slice
int mdsGetVect6DIntSlice(int expIdx, char* cpoPath, char* path, char* timeBasePath, int** data, int* dim1, int* dim2,
                         int* dim3,
...
int
mdsGetVect6DFloatSlice(int expIdx, char* cpoPath, char* path, char* timeBasePath, float** data, int* dim1, int* dim2,
...
int
mdsGetVect6DDoubleSlice(int expIdx, char* cpoPath, char* path, char* timeBasePath, double** data, int* dim1, int* dim2,
                        int* dim3, int* dim4, int* dim5, int* dim6, double time, double* retTime, int interpolMode)
{
    ARRAY_BOUNDS(int, 6) * arrayD;
    double* y1, * y2;
...
int mdsIsSliced(int expIdx, char* cpoPath, char* path)
{
    ...
    status = TreeGetNumSegments(nid, &numSegments);
    if (!(status & 1)) {
        sprintf(errmsg, "Internal error: cannot get num segments for %s, field: %s - %s", cpoPath, path,
                MdsGetMsg(status));
...
{
    if (isExpRemote(expIdx)) {
        return putObjectSegmentRemote(getExpConnectionId(expIdx), getExpRemoteIdx(expIdx), cpoPath, path, objSegment,
                                      segIdx);
    } else {
    ...
    while (sliceIdx < numSlices) {
        if (sliceSizes[sliceIdx] >= SEGMENT_OBJECT_TRESHOLD) {
            ...
            if (totSize >= SEGMENT_OBJECT_SIZE) { //A whole segment is filled
                status = TreeMakeSegment(nid, &startSegIdxD, &endSegIdxD, &endSegIdxD,
                                         (struct descriptor_a*)&segmentDataDsc, -1, totSize);
            } else {
                ...
                status = TreeBeginSegment(nid, &startSegIdxD, &endSegIdxD, (struct descriptor*)&segmentTimesDsc,
                                          (struct descriptor_a*)&segmentTemplateDsc, -1);
...
//ReplaceLastObject case: decrease number of slices for this segment and write a new straight segment for this slice
    {
        ...
        if (endSegIdx != -1 && endSegIdx ==
                               startSegIdx) //If only one slice for this segment is left, change its content in the "traditional" way, i.e. without 4 byte length in front of serialized
        {
    ...
    if (leftBytes > arrD->arsize + 4)  //In this case the (length + serialized object) is put in the current segment
    {
        ...
        if (status & 1) {
            endSegIdx++;
            status = TreeUpdateSegment(nid, &startSegIdxD, &endSegIdxD, &endSegIdxD,
                                       numSegments - 1);//Dimension field not useful
        }
...
//Create a new segment large SEGMENT_OBJECT_SIZE
            ...
            startSegIdx = endSegIdx = endSegIdx + 1;
            status = TreeBeginSegment(nid, &startSegIdxD, &endSegIdxD, &endSegIdxD,
                                      (struct descriptor_a*)&extSerializedDsc, -1);
            free(extSerialized);
            if (status & 1) {
                extSerialized = malloc(arrD->arsize + sizeof(int));
                memcpy(extSerialized, &arrD->arsize, sizeof(int));
...
static char* getApdName(struct descriptor_a* apd)
{
    ...
    char* retName;
    if (apd->class != CLASS_APD || apd->arsize / apd->length < 2 ||
        ((struct descriptor**)apd->pointer)[0]->dtype != DTYPE_T) {
...
static void*
allocateDescr(int type, void* data, int size, int totSize, int nDims, int dim1, int dim2, int dim3, int dim4, int dim5,
              int dim6, int dim7)
{
...
void* mdsPutFloatInObject(void* obj, char* path, int idx, float data)
{
    putInObject(obj, path, idx,
                allocateDescr(DTYPE_FLOAT, &data, sizeof(float), sizeof(float), 0, 0, 0, 0, 0, 0, 0, 0));
    return obj;
}
...
{
    putInObject(obj, path, idx,
                allocateDescr(DTYPE_DOUBLE, &data, sizeof(double), sizeof(double), 0, 0, 0, 0, 0, 0, 0, 0));
    return obj;
}
...
{
    putInObject(obj, path, idx,
                allocateDescr(DTYPE_FLOAT, data, sizeof(float), dim * sizeof(float), 1, dim, 0, 0, 0, 0, 0, 0));
    return obj;
}
...
{
    putInObject(obj, path, idx,
                allocateDescr(DTYPE_DOUBLE, data, sizeof(double), dim * sizeof(double), 1, dim, 0, 0, 0, 0, 0, 0));
    return obj;
}
...
{
    putInObject(obj, path, idx,
                allocateDescr(DTYPE_L, data, sizeof(int), dim1 * dim2 * sizeof(int), 2, dim1, dim2, 0, 0, 0, 0, 0));
    return obj;
}
...
{
    putInObject(obj, path, idx,
                allocateDescr(DTYPE_FLOAT, data, sizeof(float), dim1 * dim2 * sizeof(float), 2, dim1, dim2, 0, 0, 0, 0,
                              0));
    return obj;
}
...
{
    putInObject(obj, path, idx,
                allocateDescr(DTYPE_DOUBLE, data, sizeof(double), dim1 * dim2 * sizeof(double), 2, dim1, dim2, 0, 0, 0,
                              0, 0));
    return obj;
}
...
{
    putInObject(obj, path, idx,
                allocateDescr(DTYPE_L, data, sizeof(int), dim1 * dim2 * dim3 * sizeof(int), 3, dim1, dim2, dim3, 0, 0,
                              0, 0));
    return obj;
}
...
{
    putInObject(obj, path, idx,
                allocateDescr(DTYPE_FLOAT, data, sizeof(float), dim1 * dim2 * dim3 * sizeof(float), 3, dim1, dim2, dim3,
                              0, 0, 0, 0));
    return obj;
}
...
{
    putInObject(obj, path, idx,
                allocateDescr(DTYPE_DOUBLE, data, sizeof(double), dim1 * dim2 * dim3 * sizeof(double), 3, dim1, dim2,
                              dim3, 0, 0, 0, 0));
    return obj;
}
...
{
    putInObject(obj, path, idx,
                allocateDescr(DTYPE_L, data, sizeof(int), dim1 * dim2 * dim3 * dim4 * sizeof(int), 4, dim1, dim2, dim3,
                              dim4, 0, 0, 0));
    return obj;
}
...
{
    putInObject(obj, path, idx,
                allocateDescr(DTYPE_FLOAT, data, sizeof(float), dim1 * dim2 * dim3 * dim4 * sizeof(float), 4, dim1,
                              dim2, dim3, dim4, 0, 0, 0));
    return obj;
}
...
{
    putInObject(obj, path, idx,
                allocateDescr(DTYPE_DOUBLE, data, sizeof(double), dim1 * dim2 * dim3 * dim4 * sizeof(double), 4, dim1,
                              dim2, dim3, dim4, 0, 0, 0));
    return obj;
}

void*
mdsPutVect5DIntInObject(void* obj, char* path, int idx, int* data, int dim1, int dim2, int dim3, int dim4, int dim5)
{
    putInObject(obj, path, idx,
                allocateDescr(DTYPE_L, data, sizeof(int), dim1 * dim2 * dim3 * dim4 * dim5 * sizeof(int), 5, dim1, dim2,
                              dim3, dim4, dim5, 0, 0));
    return obj;
}
...
void* mdsPutVect5DDoubleInObject(void* obj, char* path, int idx, double* data, int dim1, int dim2, int dim3, int dim4,
                                 int dim5)
{
    putInObject(obj, path, idx,
                allocateDescr(DTYPE_DOUBLE, data, sizeof(double), dim1 * dim2 * dim3 * dim4 * dim5 * sizeof(double), 5,
                              dim1, dim2, dim3, dim4, dim5, 0, 0));
    return obj;
}

void*
mdsPutVect6DIntInObject(void* obj, char* path, int idx, int* data, int dim1, int dim2, int dim3, int dim4, int dim5,
                        int dim6)
{
    putInObject(obj, path, idx,
                allocateDescr(DTYPE_L, data, sizeof(int), dim1 * dim2 * dim3 * dim4 * dim5 * dim6 * sizeof(int), 6,
                              dim1, dim2, dim3, dim4, dim5, dim6, 0));
    return obj;
}

void*
mdsPutVect6DFloatInObject(void* obj, char* path, int idx, float* data, int dim1, int dim2, int dim3, int dim4, int dim5,
                          int dim6)
{
    putInObject(obj, path, idx,
                allocateDescr(DTYPE_FLOAT, data, sizeof(float), dim1 * dim2 * dim3 * dim4 * dim5 * dim6 * sizeof(float),
                              6, dim1, dim2, dim3, dim4, dim5, dim6, 0));
    return obj;
}

void* mdsPutVect6DDoubleInObject(void* obj, char* path, int idx, double* data, int dim1, int dim2, int dim3, int dim4,
                                 int dim5, int dim6)
{
    putInObject(obj, path, idx, allocateDescr(DTYPE_DOUBLE, data, sizeof(double),
                                              dim1 * dim2 * dim3 * dim4 * dim5 * dim6 * sizeof(double), 6, dim1, dim2,
                                              dim3, dim4, dim5, dim6, 0));
    return obj;
}

void*
mdsPutVect7DIntInObject(void* obj, char* path, int idx, int* data, int dim1, int dim2, int dim3, int dim4, int dim5,
                        int dim6, int dim7)
{
    putInObject(obj, path, idx,
                allocateDescr(DTYPE_L, data, sizeof(int), dim1 * dim2 * dim3 * dim4 * dim5 * dim6 * dim7 * sizeof(int),
                              7, dim1, dim2, dim3, dim4, dim5, dim6, dim7));
    return obj;
}

void*
mdsPutVect7DFloatInObject(void* obj, char* path, int idx, float* data, int dim1, int dim2, int dim3, int dim4, int dim5,
                          int dim6, int dim7)
{
    putInObject(obj, path, idx, allocateDescr(DTYPE_FLOAT, data, sizeof(float),
                                              dim1 * dim2 * dim3 * dim4 * dim5 * dim6 * dim7 * sizeof(float), 7, dim1,
                                              dim2, dim3, dim4, dim5, dim6, dim7));
    return obj;
}

void* mdsPutVect7DDoubleInObject(void* obj, char* path, int idx, double* data, int dim1, int dim2, int dim3, int dim4,
                                 int dim5, int dim6, int dim7)
{
    putInObject(obj, path, idx, allocateDescr(DTYPE_DOUBLE, data, sizeof(double),
                                              dim1 * dim2 * dim3 * dim4 * dim5 * dim6 * dim7 * sizeof(double), 7, dim1,
                                              dim2, dim3, dim4, dim5, dim6, dim7));
    return obj;
}
...
//Count first actual number of slices
    ...
    for (segIdx = 0; segIdx < numSegments; segIdx++) {
        ...
        if (!(status & 1)) {
            sprintf(errmsg, "Error reading egment limits for object at path %s/%s: %s ", cpoPath, path,
                    MdsGetMsg(status));
            unlock();
            free((char*)retXd);
            return -1;
...
int
mdsGetVect4DFloatFromObject(void* obj, char* path, int idx, float** data, int* dim1, int* dim2, int* dim3, int* dim4)
{
    ARRAY_COEFF(char *, 7) * arrD = (void*)getDataFromObject(obj, path, idx);
...
int mdsGetVect5DIntFromObject(void* obj, char* path, int idx, int** data, int* dim1, int* dim2, int* dim3, int* dim4,
                              int* dim5)
{
    ARRAY_COEFF(char *, 7) * arrD = (void*)getDataFromObject(obj, path, idx);
...
int
mdsGetVect5DFloatFromObject(void* obj, char* path, int idx, float** data, int* dim1, int* dim2, int* dim3, int* dim4,
                            int* dim5)
{
    ARRAY_COEFF(char *, 7) * arrD = (void*)getDataFromObject(obj, path, idx);
...
int
mdsGetVect5DDoubleFromObject(void* obj, char* path, int idx, double** data, int* dim1, int* dim2, int* dim3, int* dim4,
                             int* dim5)
{
    ARRAY_COEFF(char *, 7) * arrD = (void*)getDataFromObject(obj, path, idx);
...
int mdsGetVect6DIntFromObject(void* obj, char* path, int idx, int** data, int* dim1, int* dim2, int* dim3, int* dim4,
                              int* dim5, int* dim6)
{
    ARRAY_COEFF(char *, 7) * arrD = (void*)getDataFromObject(obj, path, idx);
...
int
mdsGetVect6DFloatFromObject(void* obj, char* path, int idx, float** data, int* dim1, int* dim2, int* dim3, int* dim4,
                            int* dim5, int* dim6)
{
    ARRAY_COEFF(char *, 7) * arrD = (void*)getDataFromObject(obj, path, idx);
...
int
mdsGetVect6DDoubleFromObject(void* obj, char* path, int idx, double** data, int* dim1, int* dim2, int* dim3, int* dim4,
                             int* dim5, int* dim6)
{
    ARRAY_COEFF(char *, 7) * arrD = (void*)getDataFromObject(obj, path, idx);
    if (!arrD) return -1;
...
int mdsGetVect7DIntFromObject(void* obj, char* path, int idx, int** data, int* dim1, int* dim2, int* dim3, int* dim4,
                              int* dim5, int* dim6, int* dim7)
{
    ARRAY_COEFF(char *, 7) * arrD = (void*)getDataFromObject(obj, path, idx);
...
int
mdsGetVect7DFloatFromObject(void* obj, char* path, int idx, float** data, int* dim1, int* dim2, int* dim3, int* dim4,
                            int* dim5, int* dim6, int* dim7)
{
    ARRAY_COEFF(char *, 7) * arrD = (void*)getDataFromObject(obj, path, idx);
...
int
mdsGetVect7DDoubleFromObject(void* obj, char* path, int idx, double** data, int* dim1, int* dim2, int* dim3, int* dim4,
                             int* dim5, int* dim6, int* dim7)
{
    ARRAY_COEFF(char *, 7) * arrD = (void*)getDataFromObject(obj, path, idx);
...
int mdsGetDimensionFromObject(int expIdx, void* obj, char* path, int idx, int* numDims, int* dim1, int* dim2, int* dim3,
                              int* dim4, int* dim5, int* dim6, int* dim7)
{
...
static int getObjectSliceLocal(int expIdx, char* cpoPath, char* path, double time, void** obj, int expand)
{
    ...
    status = mdsGetVect1DDouble(expIdx, cpoPath, fullPathTime, &times,
                                &nTimes); // Changed w.r.t. to ITM: read the (user hidden) time array of the type 3 AoS
    if (status) return status;
//Find Idx
    if (time <= times[0]) {
        sliceIdx = 0;
...
*******************************************************************************************/
    ...
    for (actSegmentIdx = numSegments - 1; actSegmentIdx >= 0; actSegmentIdx--) {
        ...
        if (segEndIdx <
            segStartIdx) { //This happens only for a replace last slice in case a segmment with one struct array is dismissed
                continue;
        }
...
//If it is the last segment, and not containing bytes (serialized objects) make sure the new segment contains DEFAULT_SEGMENT_ROW rows
                if (currSegment == numSegments - 1 && dtype != DTYPE_B && dtype != DTYPE_BU &&
                ...
                status = _TreeBeginSegment(toCtx, toNid, (struct descriptor*)&startXd, (struct descriptor*)&endXd,
                                           (struct descriptor*)&dimXd, (void*)&initArrDsc, -1);
                free(initArrDsc.pointer);
                if (status & 1) {
                    if (currSegment == numSegments -
                                       1) //If it is the last segment, it is necessary to store only actual data, not the whole segment
                    {
                        currSegmentDPtr = (void*)segmentXd.pointer;
                        currSegmentDPtr->m[currSegmentDPtr->dimct - 1] = next_row;
                        for (currSize = 1, i = 0;
                             i < currSegmentDPtr->dimct - 1; currSize *= currSegmentDPtr->m[i], i++);
