...
static int setIdamIdx(int shot, int run, int idx)
{
    int i;
    for(i = 0; i < MAX_OPEN_EXP && expInfo[i].mode != IS_NONE; i++);
    if(i == MAX_OPEN_EXP) return -1;
    expInfo[i].mode = IS_IDAM;
    expInfo[i].shot = shot;
    expInfo[i].run = run;
    expInfo[i].idx = idx;
    return i;
...
EXPORT int imas_open(char *name, int shot, int run, int *retIdx)
{
    ...
    *retIdx = setIdamIdx(shot, run, idx);
#else
...
#ifdef IDAM
    status = idamimasOpenEnv(name, shot, run, &idx, user, tokamak, version);
    if(status) return status;
    *retIdx = setIdamIdx(shot, run, idx);
#else
...
#ifdef IDAM
    status = idamimasCreateEnv(name, shot, run, refShot, refRun, &idx, user, tokamak, version);
    if(status) return status;
    *retIdx = setIdamIdx(shot, run, idx);
#else
...
#ifdef IDAM
    currIdx = expInfo[idx].idx;
    return idamimasClose(currIdx, "ids", getShot(idx), getRun(idx));
#else
...
int deleteData(int expIdx, char *cpoPath, char *path)
{
    ...
    return idamDeleteData(currIdx, cpoPath, path);
#else
...
#ifdef IDAM
    currIdx = expInfo[expIdx].idx;
    return idambeginIdsPutSlice(currIdx, path);
#else
    if(isMds(expIdx, &currIdx))
...
#ifdef IDAM
    currIdx = expInfo[expIdx].idx;
    return idamendIdsPutSlice(currIdx, path);
#else
    if(isMds(expIdx, &currIdx))
...
#ifdef IDAM
    currIdx = expInfo[expIdx].idx;
    return idambeginIdsReplaceLastSlice(currIdx, path);
#else
    if(isMds(expIdx, &currIdx))
...
#ifdef IDAM
    currIdx = expInfo[expIdx].idx;
    return idamendIdsReplaceLastSlice(currIdx, path);
#else
    if(isMds(expIdx, &currIdx))
...
#ifdef IDAM
    currIdx = expInfo[expIdx].idx;
    return idambeginIdsPut(currIdx, path);
#else
    if(isMds(expIdx, &currIdx))
...
#ifdef IDAM
    currIdx = expInfo[expIdx].idx;
    return idamendIdsPut(currIdx, path);
#else
    if(isMds(expIdx, &currIdx))
...
#ifdef IDAM
    currIdx = expInfo[expIdx].idx;
    return idambeginIdsPutTimed(currIdx, path, samples, inTimes);
#else
    if(isMds(expIdx, &currIdx))
...
#ifdef IDAM
    currIdx = expInfo[expIdx].idx;
    return idambeginIdsPutNonTimed(currIdx, path);
#else
    if(isMds(expIdx, &currIdx))
...
#ifdef IDAM
    currIdx = expInfo[expIdx].idx;
    return idamendIdsPutNonTimed(currIdx, path);
#else
    if(isMds(expIdx, &currIdx))
...
#ifdef IDAM
    currIdx = expInfo[expIdx].idx;
    return idamPutString(currIdx, cpoPath, path, data, strlen);
#else
    if(isMds(expIdx, &currIdx))
...
#ifdef IDAM
    currIdx = expInfo[expIdx].idx;
    return idamPutFloat(currIdx, cpoPath, path, data);
#else
    if(isMds(expIdx, &currIdx))
...
#ifdef IDAM
    currIdx = expInfo[expIdx].idx;
    return idamPutInt(currIdx, cpoPath, path, data);
#else
    if(isMds(expIdx, &currIdx))
...
#ifdef IDAM
    currIdx = expInfo[expIdx].idx;
    return idamPutDouble(currIdx, cpoPath, path, data);
#else
    if(isMds(expIdx, &currIdx))
...
#ifdef IDAM
    currIdx = expInfo[expIdx].idx;
    return idamPutVect1DInt(currIdx, cpoPath, path, data, dim, isTimed);
#else
    if(isMds(expIdx, &currIdx))
...
#ifdef IDAM
    currIdx = expInfo[expIdx].idx;
    return idamPutVect1DString(currIdx, cpoPath, path, data, dim, isTimed);
#else
    if(isMds(expIdx, &currIdx))
...
#ifdef IDAM
    currIdx = expInfo[expIdx].idx;
    return idamPutVect1DDouble(currIdx, cpoPath, path, data, dim, isTimed);
#else
    if(isMds(expIdx, &currIdx))
...
#ifdef IDAM
    currIdx = expInfo[expIdx].idx;
    return idamPutVect1DFloat(currIdx, cpoPath, path, data, dim, isTimed);
#else
    if(isMds(expIdx, &currIdx))
...
#ifdef IDAM
    currIdx = expInfo[expIdx].idx;
    return idamPutVect2DInt(currIdx, cpoPath, path, data, dim1, dim2, isTimed);
#else
    if(isMds(expIdx, &currIdx))
...
#ifdef IDAM
    currIdx = expInfo[expIdx].idx;
    return idamPutVect2DFloat(currIdx, cpoPath, path, data, dim1, dim2, isTimed);
#else
    if(isMds(expIdx, &currIdx))
...
#ifdef IDAM
    currIdx = expInfo[expIdx].idx;
    return idamPutVect2DDouble(currIdx, cpoPath, path, data, dim1, dim2, isTimed);
#else
    if(isMds(expIdx, &currIdx))
...
#ifdef IDAM
    currIdx = expInfo[expIdx].idx;
    return idamPutVect3DInt(currIdx, cpoPath, path, data, dim1, dim2, dim3, isTimed);
#else
    if(isMds(expIdx, &currIdx))
...
#ifdef IDAM
    currIdx = expInfo[expIdx].idx;
    return idamPutVect3DFloat(currIdx, cpoPath, path, data, dim1, dim2, dim3, isTimed);
#else
    if(isMds(expIdx, &currIdx))
...
#ifdef IDAM
    currIdx = expInfo[expIdx].idx;
    return idamPutVect3DDouble(currIdx, cpoPath, path, data, dim1, dim2, dim3, isTimed);
#else
    if(isMds(expIdx, &currIdx))
    {
...
#ifdef IDAM
    currIdx = expInfo[expIdx].idx;
    return idamPutVect4DInt(currIdx, cpoPath, path, data, dim1, dim2, dim3, dim4, isTimed);
#else
    if(isMds(expIdx, &currIdx))
...
#ifdef IDAM
    currIdx = expInfo[expIdx].idx;
    return idamPutVect4DFloat(currIdx, cpoPath, path, data, dim1, dim2, dim3, dim4, isTimed);
#else
    if(isMds(expIdx, &currIdx))
...
#ifdef IDAM
    currIdx = expInfo[expIdx].idx;
    return idamPutVect4DDouble(currIdx, cpoPath, path, data, dim1, dim2, dim3, dim4, isTimed);
#else
    if(isMds(expIdx, &currIdx))
...
#ifdef IDAM
    currIdx = expInfo[expIdx].idx;
    return idamPutVect5DInt(currIdx, cpoPath, path, data, dim1, dim2, dim3, dim4, dim5, isTimed);
#else
    if(isMds(expIdx, &currIdx))
...
#ifdef IDAM
    currIdx = expInfo[expIdx].idx;
    return idamPutVect5DFloat(currIdx, cpoPath, path, data, dim1, dim2, dim3, dim4, dim5, isTimed);
#else
    if(isMds(expIdx, &currIdx))
...
#ifdef IDAM
    currIdx = expInfo[expIdx].idx;
    return idamPutVect5DDouble(currIdx, cpoPath, path, data, dim1, dim2, dim3, dim4, dim5, isTimed);
#else
    if(isMds(expIdx, &currIdx))
...
#ifdef IDAM
    currIdx = expInfo[expIdx].idx;
    return idamPutVect6DInt(currIdx, cpoPath, path, data, dim1, dim2, dim3, dim4, dim5, dim6, isTimed);
#else
    if(isMds(expIdx, &currIdx))
...
#ifdef IDAM
    currIdx = expInfo[expIdx].idx;
    return idamPutVect6DFloat(currIdx, cpoPath, path, data, dim1, dim2, dim3, dim4, dim5, dim6, isTimed);
#else
    if(isMds(expIdx, &currIdx))
...
#ifdef IDAM
    currIdx = expInfo[expIdx].idx;
    return idamPutVect6DDouble(currIdx, cpoPath, path, data, dim1, dim2, dim3, dim4, dim5, dim6, isTimed);
#else
    if(isMds(expIdx, &currIdx))
...
#ifdef IDAM
    currIdx = expInfo[expIdx].idx;
    return idamPutVect7DInt(currIdx, cpoPath, path, data, dim1, dim2, dim3, dim4, dim5, dim6, dim7, isTimed);
#else
    if(isMds(expIdx, &currIdx))
...
#ifdef IDAM
    currIdx = expInfo[expIdx].idx;
    return idamPutVect7DFloat(currIdx, cpoPath, path, data, dim1, dim2, dim3, dim4, dim5, dim6, dim7, isTimed);
#else
    if(isMds(expIdx, &currIdx))
...
#ifdef IDAM
    currIdx = expInfo[expIdx].idx;
    return idamPutVect7DDouble(currIdx, cpoPath, path, data, dim1, dim2, dim3, dim4, dim5, dim6, dim7, isTimed);
#else
    if(isMds(expIdx, &currIdx))
...
#ifdef IDAM
    currIdx = expInfo[expIdx].idx;
    return idamPutStringSlice(currIdx, cpoPath, path, data, time);
#else
    if(isMds(expIdx, &currIdx))
...
#ifdef IDAM
    currIdx = expInfo[expIdx].idx;
    return idamPutFloatSlice(currIdx, cpoPath, path, data, time);
#else
    if(isMds(expIdx, &currIdx))
...
#ifdef IDAM
    currIdx = expInfo[expIdx].idx;
    return idamPutIntSlice(currIdx, cpoPath, path, data, time);
#else
    if(data == EMPTY_INT)
	return 0;
    if(isMds(expIdx, &currIdx))
...
#ifdef IDAM
    currIdx = expInfo[expIdx].idx;
    return idamPutDoubleSlice(currIdx, cpoPath, path, data, time);
#else
    if(data == EMPTY_DOUBLE)
	return 0;
    if(isMds(expIdx, &currIdx))
...
#ifdef IDAM
    currIdx = expInfo[expIdx].idx;
    return idamPutVect1DIntSlice(currIdx, cpoPath, path, data, dim, time);
#else
    if(isMds(expIdx, &currIdx))
...
#ifdef IDAM
    currIdx = expInfo[expIdx].idx;
    return idamPutVect1DDoubleSlice(currIdx, cpoPath, path, data, dim, time);
#else
    if(isMds(expIdx, &currIdx))
...
#ifdef IDAM
    currIdx = expInfo[expIdx].idx;
    return idamPutVect1DFloatSlice(currIdx, cpoPath, path, data, dim, time);
#else
    if(isMds(expIdx, &currIdx))
...
#ifdef IDAM
    currIdx = expInfo[expIdx].idx;
    return idamPutVect2DIntSlice(currIdx, cpoPath, path, data, dim1, dim2, time);
#else
    if(isMds(expIdx, &currIdx))
...
#ifdef IDAM
    currIdx = expInfo[expIdx].idx;
    return idamPutVect2DFloatSlice(currIdx, cpoPath, path, data, dim1, dim2, time);
#else
    if(isMds(expIdx, &currIdx))
...
#ifdef IDAM
    currIdx = expInfo[expIdx].idx;
    return idamPutVect2DDoubleSlice(currIdx, cpoPath, path, data, dim1, dim2, time);
#else
    if(isMds(expIdx, &currIdx))
...
#ifdef IDAM
    currIdx = expInfo[expIdx].idx;
    return idamPutVect3DIntSlice(currIdx, cpoPath, path, data, dim1, dim2, dim3, time);
#else
    if(isMds(expIdx, &currIdx))
...
#ifdef IDAM
    currIdx = expInfo[expIdx].idx;
    return idamPutVect3DDoubleSlice(currIdx, cpoPath, path, data, dim1, dim2, dim3, time);
#else
    if(isMds(expIdx, &currIdx))
...
#ifdef IDAM
    currIdx = expInfo[expIdx].idx;
    return idamPutVect4DFloatSlice(currIdx, cpoPath, path, data, dim1, dim2, dim3, dim4, time);
#else
    if(isMds(expIdx, &currIdx))
...
#ifdef IDAM
    currIdx = expInfo[expIdx].idx;
    return idamPutVect4DDoubleSlice(currIdx, cpoPath, path, data, dim1, dim2, dim3, dim4, time);
#else
    if(isMds(expIdx, &currIdx))
...
#ifdef IDAM
    currIdx = expInfo[expIdx].idx;
    return idamPutVect5DFloatSlice(currIdx, cpoPath, path, data, dim1, dim2, dim3, dim4, dim5, time);
#else
    if(isMds(expIdx, &currIdx))
...
#ifdef IDAM
    currIdx = expInfo[expIdx].idx;
    return idamPutVect5DDoubleSlice(currIdx, cpoPath, path, data, dim1, dim2, dim3, dim4, dim5, time);
#else
    if(isMds(expIdx, &currIdx))
...
#ifdef IDAM
    currIdx = expInfo[expIdx].idx;
    return idamPutVect5DIntSlice(currIdx, cpoPath, path, data, dim1, dim2, dim3, dim4, dim5, time);
#else
    if(isMds(expIdx, &currIdx))
...
#ifdef IDAM
    currIdx = expInfo[expIdx].idx;
    return idamPutVect6DFloatSlice(currIdx, cpoPath, path, data, dim1, dim2, dim3, dim4, dim5, dim6, time);
#else
    if(isMds(expIdx, &currIdx))
...
#ifdef IDAM
    currIdx = expInfo[expIdx].idx;
    return idamPutVect6DDoubleSlice(currIdx, cpoPath, path, data, dim1, dim2, dim3, dim4, dim5, dim6, time);
#else
    if(isMds(expIdx, &currIdx))
...
#ifdef IDAM
    currIdx = expInfo[expIdx].idx;
    return idamPutVect6DIntSlice(currIdx, cpoPath, path, data, dim1, dim2, dim3, dim4, dim5, dim6, time);
#else
    if(isMds(expIdx, &currIdx))
...
#ifdef IDAM
    currIdx = expInfo[expIdx].idx;
    return idamReplaceLastStringSlice(currIdx, cpoPath, path, data);
#else
    if(isMds(expIdx, &currIdx))
...
#ifdef IDAM
    currIdx = expInfo[expIdx].idx;
    return idamReplaceLastFloatSlice(currIdx, cpoPath, path, data);
#else
    if(isMds(expIdx, &currIdx))
...
#ifdef IDAM
    currIdx = expInfo[expIdx].idx;
    return idamReplaceLastIntSlice(currIdx, cpoPath, path, data);
#else
    if(isMds(expIdx, &currIdx))
...
#ifdef IDAM
    currIdx = expInfo[expIdx].idx;
    return idamReplaceLastDoubleSlice(currIdx, cpoPath, path, data);
#else
    if(isMds(expIdx, &currIdx))
...
#ifdef IDAM
    currIdx = expInfo[expIdx].idx;
    return idamReplaceLastVect1DIntSlice(currIdx, cpoPath, path, data, dim);
#else
    if(isMds(expIdx, &currIdx))
...
#ifdef IDAM
    currIdx = expInfo[expIdx].idx;
    return idamReplaceLastVect1DDoubleSlice(currIdx, cpoPath, path, data, dim);
#else
    if(isMds(expIdx, &currIdx))
...
#ifdef IDAM
    currIdx = expInfo[expIdx].idx;
    return idamReplaceLastVect1DFloatSlice(currIdx, cpoPath, path, data, dim);
#else
    if(isMds(expIdx, &currIdx))
...
#ifdef IDAM
    currIdx = expInfo[expIdx].idx;
    return idamReplaceLastVect2DIntSlice(currIdx, cpoPath, path, data, dim1, dim2);
#else
    if(isMds(expIdx, &currIdx))
...
#ifdef IDAM
    currIdx = expInfo[expIdx].idx;
    return idamReplaceLastVect2DFloatSlice(currIdx, cpoPath, path, data, dim1, dim2);
#else
    if(isMds(expIdx, &currIdx))
...
#ifdef IDAM
    currIdx = expInfo[expIdx].idx;
    return idamReplaceLastVect2DDoubleSlice(currIdx, cpoPath, path, data, dim1, dim2);
#else
    if(isMds(expIdx, &currIdx))
...
#ifdef IDAM
    currIdx = expInfo[expIdx].idx;
    return idamReplaceLastVect3DIntSlice(currIdx, cpoPath, path, data, dim1, dim2, dim3);
#else
    if(isMds(expIdx, &currIdx))
...
#ifdef IDAM
    currIdx = expInfo[expIdx].idx;
    return idamReplaceLastVect3DFloatSlice(currIdx, cpoPath, path, data, dim1, dim2, dim3);
#else
    if(isMds(expIdx, &currIdx))
...
#ifdef IDAM
    currIdx = expInfo[expIdx].idx;
    return idamReplaceLastVect3DDoubleSlice(currIdx, cpoPath, path, data, dim1, dim2, dim3);
#else
    if(isMds(expIdx, &currIdx))
...
#ifdef IDAM
    currIdx = expInfo[expIdx].idx;
    return idamReplaceLastVect4DFloatSlice(currIdx, cpoPath, path, data, dim1, dim2, dim3, dim4);
#else
    if(isMds(expIdx, &currIdx))
...
#ifdef IDAM
    currIdx = expInfo[expIdx].idx;
    return idamReplaceLastVect4DDoubleSlice(currIdx, cpoPath, path, data, dim1, dim2, dim3, dim4);
#else
    if(isMds(expIdx, &currIdx))
...
#ifdef IDAM
    currIdx = expInfo[expIdx].idx;
    return idamReplaceLastVect4DIntSlice(currIdx, cpoPath, path, data, dim1, dim2, dim3, dim4);
#else
    if(isMds(expIdx, &currIdx))
...
#ifdef IDAM
    currIdx = expInfo[expIdx].idx;
    return idamReplaceLastVect5DFloatSlice(currIdx, cpoPath, path, data, dim1, dim2, dim3, dim4, dim5);
#else
    if(isMds(expIdx, &currIdx))
...
#ifdef IDAM
    currIdx = expInfo[expIdx].idx;
    return idamReplaceLastVect5DDoubleSlice(currIdx, cpoPath, path, data, dim1, dim2, dim3, dim4, dim5);
#else
    if(isMds(expIdx, &currIdx))
...
#ifdef IDAM
    currIdx = expInfo[expIdx].idx;
    return idamReplaceLastVect5DIntSlice(currIdx, cpoPath, path, data, dim1, dim2, dim3, dim4, dim5);
#else
    if(isMds(expIdx, &currIdx))
...
#ifdef IDAM
    currIdx = expInfo[expIdx].idx;
    return idamReplaceLastVect6DFloatSlice(currIdx, cpoPath, path, data, dim1, dim2, dim3, dim4, dim5, dim6);
#else
    if(isMds(expIdx, &currIdx))
...
#ifdef IDAM
    currIdx = expInfo[expIdx].idx;
    return idamReplaceLastVect6DDoubleSlice(currIdx, cpoPath, path, data, dim1, dim2, dim3, dim4, dim5, dim6);
#else
    if(isMds(expIdx, &currIdx))
...
#ifdef IDAM
    currIdx = expInfo[expIdx].idx;
    return idamReplaceLastVect6DIntSlice(currIdx, cpoPath, path, data, dim1, dim2, dim3, dim4, dim5, dim6);
#else
    if(isMds(expIdx, &currIdx))
...
#ifdef IDAM
    currIdx = expInfo[expIdx].idx;
    return idamGetDimension(currIdx, cpoPath, path, numDims, dim1, dim2, dim3, dim4, dim5, dim6, dim7);
#else
    if(isMds(expIdx, &currIdx))
...
#ifdef IDAM
    currIdx = expInfo[expIdx].idx;
    return idamGetString(currIdx, cpoPath, path, data);
#else
    if(isMds(expIdx, &currIdx))
...
#ifdef IDAM
    currIdx = expInfo[expIdx].idx;
    return idamGetFloat(currIdx, cpoPath, path, data);
#else
    if(isMds(expIdx, &currIdx))
...
#ifdef IDAM
    currIdx = expInfo[expIdx].idx;
    return idamGetInt(currIdx, cpoPath, path, data);
#else
    if(isMds(expIdx, &currIdx))
...
#ifdef IDAM
    currIdx = expInfo[expIdx].idx;
    return idamGetDouble(currIdx, cpoPath, path, data);
#else
    if(isMds(expIdx, &currIdx))
...
#ifdef IDAM
    currIdx = expInfo[expIdx].idx;
    return idamGetVect1DString(currIdx, cpoPath, path, data, dim);
#else
    if(isMds(expIdx, &currIdx))
...
#ifdef IDAM
    currIdx = expInfo[expIdx].idx;
    return idamGetVect1DInt(currIdx, cpoPath, path, data, dim);
#else
    if(isMds(expIdx, &currIdx))
...
#ifdef IDAM
    currIdx = expInfo[expIdx].idx;
    return idamGetVect1DFloat(currIdx, cpoPath, path, data, dim);
#else
    if(isMds(expIdx, &currIdx))
...
#ifdef IDAM
    currIdx = expInfo[expIdx].idx;
    return idamGetVect1DDouble(currIdx, cpoPath, path, data, dim);
#else
    if(isMds(expIdx, &currIdx))
...
#ifdef IDAM
    currIdx = expInfo[expIdx].idx;
    return idamGetVect2DInt(currIdx, cpoPath, path, data, dim1, dim2);
#else
    if(isMds(expIdx, &currIdx))
...
#ifdef IDAM
    currIdx = expInfo[expIdx].idx;
    return idamGetVect2DFloat(currIdx, cpoPath, path, data, dim1, dim2);
#else
    if(isMds(expIdx, &currIdx))
...
#ifdef IDAM
    currIdx = expInfo[expIdx].idx;
    return idamGetVect2DDouble(currIdx, cpoPath, path, data, dim1, dim2);
#else
    if(isMds(expIdx, &currIdx))
...
#ifdef IDAM
    currIdx = expInfo[expIdx].idx;
    return idamGetVect3DInt(currIdx, cpoPath, path, data, dim1, dim2, dim3);
#else
    if(isMds(expIdx, &currIdx))
...
#ifdef IDAM
    currIdx = expInfo[expIdx].idx;
    return idamGetVect3DFloat(currIdx, cpoPath, path, data, dim1, dim2, dim3);
#else
    if(isMds(expIdx, &currIdx))
...
#ifdef IDAM
    currIdx = expInfo[expIdx].idx;
    return idamGetVect3DDouble(currIdx, cpoPath, path, data, dim1, dim2, dim3);
#else
    if(isMds(expIdx, &currIdx))
...
#ifdef IDAM
    currIdx = expInfo[expIdx].idx;
    return idamGetVect4DDouble(currIdx, cpoPath, path, data, dim1, dim2, dim3, dim4);
#else
    if(isMds(expIdx, &currIdx))
...
#ifdef IDAM
    currIdx = expInfo[expIdx].idx;
    return idamGetVect4DFloat(currIdx, cpoPath, path, data, dim1, dim2, dim3, dim4);
#else
    if(isMds(expIdx, &currIdx))
...
#ifdef IDAM
    currIdx = expInfo[expIdx].idx;
    return idamGetVect4DInt(currIdx, cpoPath, path, data, dim1, dim2, dim3, dim4);
#else
    if(isMds(expIdx, &currIdx))
...
#ifdef IDAM
    currIdx = expInfo[expIdx].idx;
    return idamGetVect5DDouble(currIdx, cpoPath, path, data, dim1, dim2, dim3, dim4, dim5);
#else
    if(isMds(expIdx, &currIdx))
...
#ifdef IDAM
    currIdx = expInfo[expIdx].idx;
    return idamGetVect5DFloat(currIdx, cpoPath, path, data, dim1, dim2, dim3, dim4, dim5);
#else
    if(isMds(expIdx, &currIdx))
...
#ifdef IDAM
    currIdx = expInfo[expIdx].idx;
    return idamGetVect5DInt(currIdx, cpoPath, path, data, dim1, dim2, dim3, dim4, dim5);
#else
    if(isMds(expIdx, &currIdx))
...
#ifdef IDAM
    currIdx = expInfo[expIdx].idx;
    return idamGetVect6DDouble(currIdx, cpoPath, path, data, dim1, dim2, dim3, dim4, dim5, dim6);
#else
    if(isMds(expIdx, &currIdx))
...
#ifdef IDAM
    currIdx = expInfo[expIdx].idx;
    return idamGetVect6DFloat(currIdx, cpoPath, path, data, dim1, dim2, dim3, dim4, dim5, dim6);
#else
    if(isMds(expIdx, &currIdx))
...
#ifdef IDAM
    currIdx = expInfo[expIdx].idx;
    return idamGetVect6DInt(currIdx, cpoPath, path, data, dim1, dim2, dim3, dim4, dim5, dim6);
#else
    if(isMds(expIdx, &currIdx))
...
#ifdef IDAM
    currIdx = expInfo[expIdx].idx;
    return idamGetVect7DDouble(currIdx, cpoPath, path, data, dim1, dim2, dim3, dim4, dim5, dim6, dim7);
#else
    if(isMds(expIdx, &currIdx))
...
#ifdef IDAM
    currIdx = expInfo[expIdx].idx;
    return idamGetVect7DFloat(currIdx, cpoPath, path, data, dim1, dim2, dim3, dim4, dim5, dim6, dim7);
#else
    if(isMds(expIdx, &currIdx))
...
#ifdef IDAM
    currIdx = expInfo[expIdx].idx;
    return idamGetVect7DInt(currIdx, cpoPath, path, data, dim1, dim2, dim3, dim4, dim5, dim6, dim7);
#else
    if(isMds(expIdx, &currIdx))
...
#ifdef IDAM
    currIdx = expInfo[expIdx].idx;
    return idamendIdsGet(currIdx, path);
#else
    if(isMds(expIdx, &currIdx))
...
#ifdef IDAM
    currIdx = expInfo[expIdx].idx;
    return idambeginIdsGet(currIdx, path, isTimed, retSamples);
#else
    if(isMds(expIdx, &currIdx))
...
#ifdef IDAM
    currIdx = expInfo[expIdx].idx;
    return idamendIdsGetSlice(currIdx, path);
#else
    if(isMds(expIdx, &currIdx))
...
#ifdef IDAM
    currIdx = expInfo[expIdx].idx;
    return idambeginIdsGetSlice(currIdx, path, time);
#else
...
#ifdef IDAM
    currIdx = expInfo[expIdx].idx;
    return idamGetIntSlice(currIdx, cpoPath, path, data, time, retTime, interpolMode);
#else
    if(isMds(expIdx, &currIdx))
...
#ifdef IDAM
    currIdx = expInfo[expIdx].idx;
    return idamGetFloatSlice(currIdx, cpoPath, path, data, time, retTime, interpolMode);
#else
    if(isMds(expIdx, &currIdx))
...
#ifdef IDAM
    currIdx = expInfo[expIdx].idx;
    return idamGetDoubleSlice(currIdx, cpoPath, path, data, time, retTime, interpolMode);
#else
    if(isMds(expIdx, &currIdx))
...
#ifdef IDAM
    currIdx = expInfo[expIdx].idx;
    return idamGetStringSlice(currIdx, cpoPath, path, data, time, retTime, interpolMode);
#else
    if(isMds(expIdx, &currIdx))
...
#ifdef IDAM
    currIdx = expInfo[expIdx].idx;
    return idamGetVect1DIntSlice(currIdx, cpoPath, path, data, dim, time, retTime, interpolMode);
#else
    if(isMds(expIdx, &currIdx))
...
#ifdef IDAM
    currIdx = expInfo[expIdx].idx;
    return idamGetVect1DFloatSlice(currIdx, cpoPath, path, data, dim, time, retTime, interpolMode);
#else
    if(isMds(expIdx, &currIdx))
...
#ifdef IDAM
    currIdx = expInfo[expIdx].idx;
    return idamGetVect1DDoubleSlice(currIdx, cpoPath, path, data, dim, time, retTime, interpolMode);
#else
    if(isMds(expIdx, &currIdx))
...
#ifdef IDAM
    currIdx = expInfo[expIdx].idx;
    return idamGetVect2DIntSlice(currIdx, cpoPath, path, data, dim1, dim2, time, retTime, interpolMode);
#else
    if(isMds(expIdx, &currIdx))
...
#ifdef IDAM
    currIdx = expInfo[expIdx].idx;
    return idamGetVect2DFloatSlice(currIdx, cpoPath, path, data, dim1, dim2, time, retTime, interpolMode);
#else
    if(isMds(expIdx, &currIdx))
...
#ifdef IDAM
    currIdx = expInfo[expIdx].idx;
    return idamGetVect2DDoubleSlice(currIdx, cpoPath, path, data, dim1, dim2, time, retTime, interpolMode);
#else
    if(isMds(expIdx, &currIdx))
...
#ifdef IDAM
    currIdx = expInfo[expIdx].idx;
    return idamGetVect3DIntSlice(currIdx, cpoPath, path, data, dim1, dim2, dim3, time, retTime, interpolMode);
#else
    if(isMds(expIdx, &currIdx))
...
#ifdef IDAM
    currIdx = expInfo[expIdx].idx;
    return idamGetVect3DDoubleSlice(currIdx, cpoPath, path, data, dim1, dim2, dim3, time, retTime, interpolMode);
#else
    if(isMds(expIdx, &currIdx))
...
#ifdef IDAM
    currIdx = expInfo[expIdx].idx;
    return idamGetVect4DIntSlice(currIdx, cpoPath, path, data, dim1, dim2, dim3, dim4, time, retTime, interpolMode);
#else
    if(isMds(expIdx, &currIdx))
...
#ifdef IDAM
    currIdx = expInfo[expIdx].idx;
    return idamGetVect4DFloatSlice(currIdx, cpoPath, path, data, dim1, dim2, dim3, dim4, time, retTime, interpolMode);
#else
    if(isMds(expIdx, &currIdx))
...
#ifdef IDAM
    currIdx = expInfo[expIdx].idx;
    return idamGetVect4DDoubleSlice(currIdx, cpoPath, path, data, dim1, dim2, dim3, dim4, time, retTime, interpolMode);
#else
    if(isMds(expIdx, &currIdx))
...
#ifdef IDAM
    currIdx = expInfo[expIdx].idx;
    return idamGetVect5DIntSlice(currIdx, cpoPath, path, data, dim1, dim2, dim3, dim4, dim5, time, retTime, interpolMode);
#else
    if(isMds(expIdx, &currIdx))
...
#ifdef IDAM
    currIdx = expInfo[expIdx].idx;
    return idamGetVect5DFloatSlice(currIdx, cpoPath, path, data, dim1, dim2, dim3, dim4, dim5, time, retTime, interpolMode);
#else
    if(isMds(expIdx, &currIdx))
...
#ifdef IDAM
    currIdx = expInfo[expIdx].idx;
    return idamGetVect5DDoubleSlice(currIdx, cpoPath, path, data, dim1, dim2, dim3, dim4, dim5, time, retTime, interpolMode);
#else
    if(isMds(expIdx, &currIdx))
...
#ifdef IDAM
    currIdx = expInfo[expIdx].idx;
    return idamGetVect6DIntSlice(currIdx, cpoPath, path, data, dim1, dim2, dim3, dim4, dim5, dim6, time, retTime, interpolMode);
#else
    if(isMds(expIdx, &currIdx))
...
#ifdef IDAM
    currIdx = expInfo[expIdx].idx;
    return idamGetVect6DFloatSlice(currIdx, cpoPath, path, data, dim1, dim2, dim3, dim4, dim5, dim6, time, retTime, interpolMode);
#else
    if(isMds(expIdx, &currIdx))
...
#ifdef IDAM
    currIdx = expInfo[expIdx].idx;
    return idamGetVect6DDoubleSlice(currIdx, cpoPath, path, data, dim1, dim2, dim3, dim4, dim5, dim6, time, retTime, interpolMode);
#else
    if(isMds(expIdx, &currIdx))
...
#ifdef IDAM
    currIdx = expInfo[expIdx].idx;
    return idamPutObject(currIdx, cpoPath, path, obj, isTimed);
#else
    if(isMds(expIdx, &currIdx))
...
#ifdef IDAM
    currIdx = expInfo[expIdx].idx;
    return idamGetDimensionFromObject(currIdx, obj, path, idx, numDims, dim1, dim2, dim3, dim4, dim5, dim6, dim7);
#else
    if(isMds(expIdx, &currIdx))
