...
#include "imas.h"
   ...
   static USERDEFINEDTYPELIST *userdefinedtypelist = NULL;	// User Defined Structure Types from Data Files
   #ifdef ITERSERVER
      #include "/home/ITER/muird1/IDAM/source/clientserver/TrimString.c"
      ...
      #include "/home/ITER/muird1/IDAM/source/clientserver/compressDim.c"   
   #else
      ...
      #include "/home/dgm/IDAM/source/clientserver/compressDim.c"
   #endif
#endif

static IDAMPLUGINFILELIST PluginFileList;	 
IDAMPLUGINFILELIST *pluginFileList = &PluginFileList;	// Private list of open data file handles - Owned by the libualidamhdf5 library 
#include "managePluginFiles.c"					// These should be available from the server library
#include "common.c"			        		// plugin utilities dependent on build (hdf5, netcdf4)
...
#include "./ccfe_idam_hdf5_plugin.c"
...
// These are not object pointers - both are set to NULL
   ...
   for(i=0;i<MAXOBJECTCOUNT;i++)localObjs[i] = 0;
   initLocalObjs = 0; 
...
void putImasIdsVersion(char *version){ strcpy(imasIdsVersion, version);}
char *getImasIdsVersion(){ return imasIdsVersion;}
void putImasIdsDevice(char *device){ strcpy(imasIdsDevice, device);}
char *getImasIdsDevice(){ return imasIdsDevice;}

static int imasIdsShot = 0;
static int imasIdsRun = 0;
static void putImasIdsShot(int idx, int shot){ imasIdsShot = shot; }
static void putImasIdsRun(int idx, int run){ imasIdsRun = run; }
static int getImasIdsShot(int idx){ return imasIdsShot; }
static int getImasIdsRun(int idx){ return imasIdsRun; }
...
// Standard v1 Plugin Interface
   ...
   if(idam_plugin_interface->interfaceVersion > THISPLUGIN_MAX_INTERFACE_VERSION){
      err = 999;
      if(verbose) fprintf(errout, "ERROR imas: Plugin Interface Version Unknown to this plugin: Unable to execute the request!\n");      
      addIdamError(&idamerrorstack, CODEERRORTYPE, "imas", err, "Plugin Interface Version Unknown to this plugin: Unable to execute the request!");
...
// Arguments and keywords 
...
#ifndef MDSSKIP            
            if(!strcasecmp(request_block->nameValueList.nameValue[i].name, "imas_mds")){	// *** redirect to the mdsplus imas plugin 
	       return imas_mds(idam_plugin_interface);
	       continue;
	    }
#endif	    
	    if(!strcasecmp(request_block->nameValueList.nameValue[i].name, "group") || !strcasecmp(request_block->nameValueList.nameValue[i].name, "cpoPath") || 
	       !strcasecmp(request_block->nameValueList.nameValue[i].name, "cpo")){
	       isCPOPath = 1;
	       CPOPath = request_block->nameValueList.nameValue[i].value;	
	       continue;
	    }
            if(!strcasecmp(request_block->nameValueList.nameValue[i].name, "variable") || !strcasecmp(request_block->nameValueList.nameValue[i].name, "path")){
	       isPath = 1;
	       ...
	       continue;
            ...
            if(!strcasecmp(request_block->nameValueList.nameValue[i].name, "clientObjectId") || !strcasecmp(request_block->nameValueList.nameValue[i].name, "ObjectId")){
	       isClientObjectId = 1;
	       clientObjectId = atoi(request_block->nameValueList.nameValue[i].value);	
	       continue;
	    }
            if(!strcasecmp(request_block->nameValueList.nameValue[i].name, "rank") || !strcasecmp(request_block->nameValueList.nameValue[i].name, "ndims")){
	       isRank = 1;
	       rank = atoi(request_block->nameValueList.nameValue[i].value);	
	       continue;
	    }
            if(!strcasecmp(request_block->nameValueList.nameValue[i].name, "index") || !strcasecmp(request_block->nameValueList.nameValue[i].name, "index1")){
	       isIndex = 1;
	       ...
	       continue;
            ...
            if(!strcasecmp(request_block->nameValueList.nameValue[i].name, "shape") || !strcasecmp(request_block->nameValueList.nameValue[i].name, "dims")){
	       isShapeString = 1;
	    ...
	    if(!strcasecmp(request_block->nameValueList.nameValue[i].name, "singlequote")){
            ...
            if(!strcasecmp(request_block->nameValueList.nameValue[i].name, "filename") || !strcasecmp(request_block->nameValueList.nameValue[i].name, "file") ||
...
// Keywords
	       ...
	       continue;
            ...
            if(!strcasecmp(request_block->nameValueList.nameValue[i].name, "replaceLastSlice")){	// Replace the last written slice (scalar, array)
...
// IDS Version
      ...
      if(!strcasecmp(request_block->function, "putIdsVersion")){
         ...
         if(!isImasIdsVersion && !isImasIdsDevice){
            err = 999;
            if(verbose) fprintf(errout, "imas version: An IDS Version number or a Device name is required!\n");      
            addIdamError(&idamerrorstack, CODEERRORTYPE, "putCDF4", err, "An IDS Version number or a Device name is required!");
	    break;
	 }
...
//		/NoCacheData	Don't cache the measurement data after a request for the "data" using the /data keyword. Cacheing is the default with the cache cleared after time data are returned.
	 ...
	 unsigned short int isData=0, isTime=0, isNoCacheTime=0, isNoCacheData=0, isDataScaling=0, isTimeScaling=0; 
         ...
         for(i=0;i<request_block->nameValueList.pairCount;i++){ 
	    if(!strcasecmp(request_block->nameValueList.nameValue[i].name, "data")){		// Only return the data - ignore the coordinates (so do nothing as already in the DATA_BLOCK)
	       isData = 1;
	       continue;
	    }	    
	    if(!strcasecmp(request_block->nameValueList.nameValue[i].name, "time")){		// Only return the time - ignore the data
	       isTime = 1;
	       continue;
	    }	    
	    if(!strcasecmp(request_block->nameValueList.nameValue[i].name, "nocacheTime")){	// Do Not Cache the time data in preparation for a Time request. Default is to Cache with cache cleared after time data returned.
	       isNoCacheTime = 1;
	       continue;
	    }	  
	    if(!strcasecmp(request_block->nameValueList.nameValue[i].name, "nocacheData")){	// Do Not Cache the time data in preparation for a Time request. Default is to Cache with cache cleared after time data returned.
	       isNoCacheData = 1;
	       continue;
	    }	  
	    ...
	    if(!strcasecmp(request_block->nameValueList.nameValue[i].name, "timeScaling")){	// Measurement times need to scaled to comply with standards, e.g. SI Units
         ...
         if(!isSignal){
            err = 999;
            if(verbose) fprintf(errout, "imas source: No data object name (signal) has been specified!\n");      
            addIdamError(&idamerrorstack, CODEERRORTYPE,  "imas source: No data object name (signal) has been specified!", err, "");
	    break;
         }
...
// Prepare common code
	 ...
	 IDAM_PLUGIN_INTERFACE next_plugin_interface;
         ...
         if(pluginList == NULL){
            err = 999;
            if(verbose) fprintf(errout, "imas source: the specified format is not recognised!\n");      
            addIdamError(&idamerrorstack, CODEERRORTYPE,  "imas source: No plugins are available for this data request", err, "");
            break;
         }	 	 	 
...
// JET PPF sources: PPF::/$ppfname/$pulseNumber/$sequence/$owner 
         ...
         if(isFormat && !strcasecmp(format, "ppf")){			// JET PPF source naming pattern
            ...
            if(!isSource){
               err = 999;
               if(verbose) fprintf(errout, "imas source: No PPF DDA data source has been specified!\n");      
               addIdamError(&idamerrorstack, CODEERRORTYPE,  "imas source: No PPF DDA data source has been specified!", err, "");
	       break;
            }
	    ...
	    if(env == NULL)
	       sprintf(next_request_block.source, "JET%sPPF%s/%s/%d", request_block->api_delim, request_block->api_delim, source, shotNumber);
	    else   
	    ...
	    if(env == NULL)
	       sprintf(next_request_block.source, "JET%sJPF%s%d", request_block->api_delim, request_block->api_delim, shotNumber);
	    else
	 ...
	 if(isFormat && !strcasecmp(format, "MAST")){		// MAST source naming pattern
	    ...
	    if(!isServer){
               err = 999;
               if(verbose) fprintf(errout, "imas source: No data server has been specified!\n");      
	    ...
	    if(isSource){	// TDI function or tree?
		  ...
		  sprintf(p, "%d%s", shotNumber, &p[12]);   	           
            ...
            err = 999;
            if(verbose) fprintf(errout, "imas source: the specified format is not recognised!\n");      
            addIdamError(&idamerrorstack, CODEERRORTYPE, "imas source", err, "the specified format is not recognised!");
            break;              
         }
...
// Create the Request data structure
	 ...
	 if(env != NULL)
	    ...
	    sprintf(work, "IDAM::get(host=%s, port=%d, signal=\"%s\", source=\"%s\")", getIdamServerHost(), getIdamServerPort(), next_request_block.signal, next_request_block.source);
...
// Call the IDAM client via the IDAM plugin (ignore the request identified)
	 ...
	 if(env != NULL)
 	 ...
 	 if(next_request_block.request < 0){
            err = 999;
            if(verbose) fprintf(errout, "imas source: No IDAM server plugin found!\n");      
...
// Locate and Execute the IDAM plugin  
         ...
         if(!skipPlugin){
            ...
            if(id >= 0 && pluginList->plugin[id].idamPlugin != NULL){
               ...
               if(err != 0){             
                  //idamLog(LOG_ERROR, "imas source: Data Access is not available!\n");      
...
// Time Data are only cacheable if the data are rank 1 with time data! 
         ...
         if(isData){		// Ignore the coordinate data.
	        ...
	        err = 999;
                if(verbose) fprintf(errout, "imas source: Data Access is not available for this data request!\n");      
                addIdamError(&idamerrorstack, CODEERRORTYPE, "Data Access is not available for this data request!", err, "");
                break;
...
// What is the status of the data file
	    ...
	    if(fid >= 0){
	       ...
	       if(!isFileName && !isShotNumber && !isRunNumber){
                  err = 999;
                  if(verbose) fprintf(errout, "imas delete: No registered data file identified!\n");      
                  addIdamError(&idamerrorstack, CODEERRORTYPE, "imas delete", err, "No registered data file identified!");
                  break;              
	       }
	    ...
	    if(fid >= 0){
	       if(pluginFileList->files[fid].status){
	          fileStatus = FILEISOPEN;
                  if((clientIdx = findHdf5Idx(fid)) < 0){
                     err = 999;
                     if(verbose) fprintf(errout, "imas delete: Unable to Locate the necessary data file!\n");      
                     addIdamError(&idamerrorstack, CODEERRORTYPE, "imas delete", err, "Unable to Locate the necessary data file!");
	             break;              
	          }
		  isClientIdx = 1;						// Treat as if passed in a name-value pair
	       ...
	       fileStatus = FILEISNEW;
            ...
            err = 999;
            if(verbose) fprintf(errout, "imas delete: No data file identified!\n");      
...
// Open the Data file
	 ...
	 if(!isOpen){						
	    ...
	    if(isClientIdx)
               ...
               sprintf(private_request_block.signal,"imas::open(file=%s, shot=%d, run=%d)", filename, shotNumber, runNumber);	 	       
	    ...
	    if(clientIdx < 0){
               err = 999;
               if(verbose) fprintf(errout, "imas delete: Unable to Open the necessary data file!\n");      
               addIdamError(&idamerrorstack, CODEERRORTYPE, "imas delete", err, "Unable to Open the necessary data file!");
	       break;              
	    }	       
	    isClientIdx = 1;								// Treat as if passed in a name-value pair
	    isOpen = 1;									// File is Open	  
...
// What is the status of the data file
	    ...
	    if(fid >= 0){
	       ...
	       if(!isFileName && !isShotNumber && !isRunNumber){
                  err = 999;
                  if(verbose) fprintf(errout, "imas put: No registered data file identified!\n");      
                  addIdamError(&idamerrorstack, CODEERRORTYPE, "imas get", err, "No registered data file identified!");
                  break;              
	       }
	       isClientIdx = 0;		// Data is not recognised - ignore! 
	    }	    	 
	    ...
	    if(fid >= 0){
	       if(pluginFileList->files[fid].status){
	          fileStatus = FILEISOPEN;
                  if((clientIdx = findHdf5Idx(fid)) < 0){
                     err = 999;
                     if(verbose) fprintf(errout, "imas put: Unable to Locate the necessary data file!\n");      
                     addIdamError(&idamerrorstack, CODEERRORTYPE, "imas get", err, "Unable to Locate the necessary data file!");
	             break;              
	          }
		  isClientIdx = 1;						// Treat as if passed in a name-value pair
	          isOpen = 1;							// File is Open	  
	       ...
	       fileStatus = FILEISNEW;
            ...
            err = 999;
            if(verbose) fprintf(errout, "imas put: No data file identified!\n");      
...
// Open the Data file
	 ...
	 if(!isOpen){						
	    ...
	    if(clientIdx < 0){
               err = 999;
               if(verbose) fprintf(errout, "imas put: Unable to Open the necessary data file!\n");      
               addIdamError(&idamerrorstack, CODEERRORTYPE, "imas get", err, "Unable to Open the necessary data file!");
	       break;              
	    }	       
	    isClientIdx = 1;								// Treat as if passed in a name-value pair
	    isOpen = 1;									// File is Open	  
...
// Convert type string into IMAS type identifiers
	    ...
	    if(!isTypeName && !isPutData){
               err = 999;
               if(verbose) fprintf(errout, "imas get: The data's Type has not been specified!\n");      
               addIdamError(&idamerrorstack, CODEERRORTYPE, "imas get", err, "The data's Type has not been specified!");
               break;              
	    } 
 	 
	    if((type = findIMASType(typeName)) == 0){
               err = 999;
               if(verbose) fprintf(errout, "imas get: The data's Type name cannot be converted!\n");      
               addIdamError(&idamerrorstack, CODEERRORTYPE, "imas get", err, "The data's Type name cannot be converted!");
               break;              
	    }
	 ...
	 if(!isPutData && !isRank && !isGetDimension){
            err = 999;
            if(verbose) fprintf(errout, "imas get: The data's Rank has not been specified!\n");      
            addIdamError(&idamerrorstack, CODEERRORTYPE, "imas get", err, "The data's Rank has not been specified!");
            break;              
	 } 
...
// Which Data Operation?
	 ...
	 if(isGetDimension) dataOperation = GETDIMENSION_OPERATION;	 	      
...
// GET and return the data
	 ...
	 if(dataOperation == GETSLICE_OPERATION){ 
            ...
            if(!isIndex){
               err = 999;
               if(verbose) fprintf(errout, "imas get: No Time Vector Indices have been specified!\n");      
               addIdamError(&idamerrorstack, CODEERRORTYPE, "imas get", err, "No Time Vector Indices have been specified!");
               break;              
	    }
            if(!isInterpolMode){
               err = 999;
               if(verbose) fprintf(errout, "imas get: No Interpolation Mode has been specified!\n");      
               addIdamError(&idamerrorstack, CODEERRORTYPE, "imas get", err, "No Interpolation Mode has been specified!");
               break;              
	    }
	    
            if(!isPutData || putDataBlock->data_type != TYPE_DOUBLE || putDataBlock->count != 3){
               err = 999;
               if(verbose) fprintf(errout, "imas get: No Time Values have been specified!\n");      
               addIdamError(&idamerrorstack, CODEERRORTYPE, "imas get", err, "No Time Values have been specified!");
               break;              
	    }
...
// from name-value: sliceIdx1 (aka index), sliceIdx2 (aka index2)
	 ...
	 if(dataOperation == GETDIMENSION_OPERATION) 
	    rc = imas_hdf5GetDimension(clientIdx, CPOPath, path, &rank, &shape[0], &shape[1], &shape[2], &shape[3], &shape[4], &shape[5], &shape[6]);
...
// What is the status of the data file
	    ...
	    if(fid >= 0){
	       ...
	       if(!isFileName && !isShotNumber && !isRunNumber){
                  err = 999;
                  if(verbose) fprintf(errout, "imas put: No registered data file identified!\n");      
                  addIdamError(&idamerrorstack, CODEERRORTYPE, "imas put", err, "No registered data file identified!");
                  break;              
	       }
	       isClientIdx = 0;		// Data is not recognised - ignore! 
	    }	    	 
	       ...
	       if(pluginFileList->files[fid].status){
	          fileStatus = FILEISOPEN;
                  if((clientIdx = findHdf5Idx(fid)) < 0){
                     err = 999;
                     if(verbose) fprintf(errout, "imas put: Unable to Locate the necessary data file!\n");      
                     addIdamError(&idamerrorstack, CODEERRORTYPE, "imas put", err, "Unable to Locate the necessary data file!");
	             break;              
	          }
		  isClientIdx = 1;						// Treat as if passed in a name-value pair
	          isOpen = 1;							// File is Open	  
	 ...
	 if(!isOpen){						
	    ...
	    private_idam_plugin_interface.request_block = &private_request_block;
            ...
            if(fileStatus == FILEISNEW) 
               sprintf(private_request_block.signal,"imas::create(file=%s, shot=%d, run=%d)", filename, shotNumber, runNumber);		// Create the file	       
            else {
	       if(isClientIdx)
                  sprintf(private_request_block.signal,"imas::open(idx=%d)", clientIdx);						// Open the file	       
	       else
                  sprintf(private_request_block.signal,"imas::open(file=%s, shot=%d, run=%d)", filename, shotNumber, runNumber);	// Open the file	       
	    }
	    ...
	    if(clientIdx < 0){
               err = 999;
               if(verbose) fprintf(errout, "imas put: Unable to Open/Create the necessary data file!\n");      
               addIdamError(&idamerrorstack, CODEERRORTYPE, "imas put", err, "Unable to Open/Create the necessary data file!");
	       break;              
	    }	       
	    isClientIdx = 1;								// Treat as if passed in a name-value pair
	    isOpen = 1;									// File is Open	  
...
// Has a PUTDATA block been passed with a missing or a matching name?
	    ...
	    for(i=0;i<putDataBlockList->blockCount;i++){
	       ...
	       if(putDataBlockList->putDataBlock[i].blockName == NULL){
	          if(putDataBlockList->blockCount > 1){			// Multiple blocks must be named
                     err = 999;
                     if(verbose) fprintf(errout, "imas put: Multiple un-named data items - ambiguous!\n");      
                     addIdamError(&idamerrorstack, CODEERRORTYPE, "imas put", err, "Multiple un-named data items - ambiguous!");
	             break;              
		  }
		  ...
		  varDataIndex = i; 
	       ...
	       if(!strcasecmp(putDataBlockList->putDataBlock[i].blockName, "variable") ||
	    ...
	    if(varDataIndex < 0 && putDataBlockList->blockCount == 1 && 
	       (putDataBlockList->putDataBlock[0].blockName == NULL || putDataBlockList->putDataBlock[0].blockName[0] == '\0')){	       
	       varData = (char *)putDataBlockList->putDataBlock[0].data;
	       varDataIndex = 0; 
	    }      	 
	    if(varDataIndex < 0){
               err = 999;
               if(verbose) fprintf(errout, "imas put: Unable to Identify the data to PUT!\n");      
               addIdamError(&idamerrorstack, CODEERRORTYPE, "imas put", err, "Unable to Identify the data to PUT!");
               break;              
	    }
	    ...
	    if((type = findIMASType(convertIdam2StringType(putDataBlock->data_type))) == 0){		// Convert an IDAM type to an IMAS type
               err = 999;
               if(verbose) fprintf(errout, "imas put: The data's Type cannot be converted!\n");      
               addIdamError(&idamerrorstack, CODEERRORTYPE, "imas put", err, "The data's Type cannot be converted!");
               break;              
	    }
...
// Convert type string into IMAS type identifiers
	    ...
	    if(!isTypeName){
               err = 999;
               if(verbose) fprintf(errout, "imas put: The data's Type has not been specified!\n");      
               addIdamError(&idamerrorstack, CODEERRORTYPE, "imas put", err, "The data's Type has not been specified!");
               break;              
	    } 
	 
	    if((type = findIMASType(typeName)) == 0){
               err = 999;
               if(verbose) fprintf(errout, "imas put: The data's Type name cannot be converted!\n");      
               addIdamError(&idamerrorstack, CODEERRORTYPE, "imas put", err, "The data's Type name cannot be converted!");
               break;              
	    }
	 }
...
// Any Data?
         ...
         if(!isDataString && !isVarData){
            err = 999;
            if(verbose) fprintf(errout, "imas put: No data has been specified!\n");      
...
// Which Data Operation?
	 ...
	 if(isReplaceLastDataSlice) dataOperation = REPLACELASTSLICE_OPERATION;	 	      
...
// *** need new functions: convert from named type to HDF5 type
	 ...
	 if(isShapeString){
	    if((dataRank = getIdamNameValuePairVarArray(shapeString, quote, delimiter, (unsigned short) rank, TYPE_INT, (void **)&shape)) < 0){
               err = -dataRank;
               if(verbose) fprintf(errout, "imas put: Unable to convert the passed shape values!\n");      
               addIdamError(&idamerrorstack, CODEERRORTYPE, "imas put", err, "Unable to convert the passed shape value!");
               break;              
 	    }
	    if(isRank && rank != dataRank){
               err = 999;
               if(verbose) fprintf(errout, "imas put: The passed rank is inconsistent with the passed shape data!\n");      
               addIdamError(&idamerrorstack, CODEERRORTYPE, "imas put", err, "The passed rank is inconsistent with the passed shape data!");
               break;              
 	    }
	    isShape = 1;
	    isRank  = 1;
	 ...
	 if(isDataString){
	    ...
	    if((dataCount = getIdamNameValuePairVarArray(dataString, quote, delimiter, (unsigned short)shapeCount, idamType, (void **)&data)) < 0){
               err = -dataCount;
               if(verbose) fprintf(errout, "imas put: Unable to convert the passed data values!\n");      
               addIdamError(&idamerrorstack, CODEERRORTYPE, "imas put", err, "Unable to convert the passed data value!");
               break;              
 	    }
	    if(shapeCount != 0 && shapeCount != dataCount){
               err = 999;
               if(verbose) fprintf(errout, "imas put: Inconsistent count of Data items!\n");      
               addIdamError(&idamerrorstack, CODEERRORTYPE, "imas put", err, "Inconsistent count of Data items!");
               break;              
 	    }
	    isData = 1;
...
//dgm 30Jul2015         if(!isCPOPath || !isPath || !isType || !isRank || !isShape || !isData || (dataOperation == PUT_OPERATION && !isTimedArg)) 
         if(!isCPOPath || !isPath || !isType || !isRank || !isShape || !isData){
            err = 999;
            if(verbose) fprintf(errout, "imas put: Insufficient data parameters passed - put not possible!\n");      
            addIdamError(&idamerrorstack, CODEERRORTYPE, "imas put", err, "Insufficient data parameters passed - put not possible!");
	    break;
	 }	 
...
// The type passed here is the IMAS type enumeration (imas_putData is the original imas putData function)
	 ...
	 if(dataOperation == PUT_OPERATION)
	    ...
	    if(dataOperation == PUTSLICE_OPERATION){
               err = 999;
               if(verbose) fprintf(errout, "imas put: Slice Time not passed!\n");      
               addIdamError(&idamerrorstack, CODEERRORTYPE, "imas put", err, "Slice Time not passed!");
	       break;
	    } else
	       rc = imas_putDataX(clientIdx, CPOPath, path, type, rank, shape, dataOperation, (void *)data, 0.0);	// Replace Last Slice 
	 }
	 if(rc < 0) err = 999;      
...
// dgm 30Jul2015         if(!isCPOPath || !isPath || (dataOperation == PUT_OPERATION && !isTimedArg)) 
	 ...
	 if(putDataBlock->shape == NULL){
	    if(putDataBlock->rank > 1){
	       err = 999;
               if(verbose) fprintf(errout, "imas put: No shape information passed!\n");      
	 ...
	 if(dataOperation == PUT_OPERATION)
	    rc = imas_putData(clientIdx, CPOPath, path, type, putDataBlock->rank, putDataBlock->shape, isTimed, (void *)putDataBlock->data);
	 else {
	    if(dataOperation == PUTSLICE_OPERATION){
	       ...
	       if(putTimeBlock == NULL){
                  err = 999;
                  if(verbose) fprintf(errout, "imas put: No Slice Time!\n");      
	       ...
	       if(putTimeBlock->data_type != TYPE_DOUBLE || putTimeBlock->count != 1){
                  err = 999;
                  if(verbose) fprintf(errout, "imas put: Slice Time type and count are incorrect!\n");      
                  addIdamError(&idamerrorstack, CODEERRORTYPE, "imas put", err, "Slice Time type and count are incorrect!");
                  break;              
	       }
	       rc = imas_putDataX(clientIdx, CPOPath, path, type, putDataBlock->rank, putDataBlock->shape, dataOperation, (void *)putDataBlock->data, ((double *)putTimeBlock->data)[0]);
	    } else
	       rc = imas_putDataX(clientIdx, CPOPath, path, type, rank, shape, dataOperation, (void *)data, 0.0);	// Replace Last Slice 
         }
...
// Passed name-value pairs  
         ...
         if(!isFileName || !isShotNumber || !isRunNumber){
            err = 999;
            if(verbose) fprintf(errout, "imas: A Filename, Shot number and Run number are required!\n");      
            addIdamError(&idamerrorstack, CODEERRORTYPE, "imas", err, "A Filename, Shot number and Run number are required!");
	    break;
	 }
...
*/
	 ...
	 if(isCreateFromModel){
	    if((rc = imas_hdf5EuitmCreate(filename, shotNumber, runNumber, refShotNumber, refRunNumber, &idx)) < 0){      	       	 
               err = 999;      
	    ...
	    if((rc = imas_hdf5IMASCreate(filename, shotNumber, runNumber, refShotNumber, refRunNumber, &idx)) < 0){
...
// IMAS close a file
      ...
      if(!strcasecmp(request_block->function, "close")){
         ...
         if(!isClientIdx && !(isFileName && isShotNumber && isRunNumber)){
            err = 999;
            if(verbose) fprintf(errout, "imas: The file IDX or the Filename with Shot number and Run number are required!\n");      
            addIdamError(&idamerrorstack, CODEERRORTYPE, "imas", err, "The file IDX or the Filename with Shot number and Run number are required!");
	    break;
	 }
...
// IMAS create a MODEL file - groups only, no data
	 ...
	 int version = 0;            
         ...
         if(!isFileName){
            err = 999;
            if(verbose) fprintf(errout, "imas put: A Filename is required!\n");      
...
// Test all required data is available	  
         ...
         if(!isClientObjectId){
            err = 999;
            if(verbose) fprintf(errout, "imas releaseObject: Required data parameter not passed!\n");      
            addIdamError(&idamerrorstack, CODEERRORTYPE, "imas releaseObject", err, "Required data parameter not passed!");
	    break;
	 }	 
...
// Test all required data is available	  
         ...
         if(!isPutData || !isIndex || !isPath){
            err = 999;
            if(verbose) fprintf(errout, "imas getObjectObject: Insufficient data parameters passed - begin not possible!\n");      
            addIdamError(&idamerrorstack, CODEERRORTYPE, "imas getObjectObject", err, "Insufficient data parameters passed - begin not possible!");
	    break;
	 }	 
...
// Test all required data is available	  
         ...
         if(!isPutData || !isClientIdx || !isHdf5Path || !isCPOPath || putDataBlockList->blockCount != 3){
            err = 999;
            if(verbose) fprintf(errout, "imas getObjectSlice: Insufficient data parameters passed - begin not possible!\n");      
            addIdamError(&idamerrorstack, CODEERRORTYPE, "imas getObjectSlice", err, "Insufficient data parameters passed - begin not possible!");
	    break;
	 }
...
// Test all required data is available	  
         ...
         if(!isClientIdx || !isPath || !isCPOPath || !isTimedArg){
            err = 999;
            if(verbose) fprintf(errout, "imas getObjectGroup: Insufficient data parameters passed - begin not possible!\n");      
            addIdamError(&idamerrorstack, CODEERRORTYPE, "imas getObjectGroup", err, "Insufficient data parameters passed - begin not possible!");
	    break;
	 }	 
...
// Object? The data is the local object reference 
         ...
         if(putDataBlockList->putDataBlock[0].data == NULL){
            err = 999;
            if(verbose) fprintf(errout, "imas getObjectDim: No data object has been specified!\n");      
            addIdamError(&idamerrorstack, CODEERRORTYPE, "imas getObjectDim", err, "No data object has been specified!");
            break;              
	 }   	 	 	 	    	 
...
// Test all required data is available	  
         ...
         if(!isClientIdx || !isIndex || !isRelPath || !isPutData || putDataBlockList->blockCount != 2 || !isTimedArg){
            err = 999;
            if(verbose) fprintf(errout, "imas beginObject: Insufficient data parameters passed - begin not possible!\n");      
            addIdamError(&idamerrorstack, CODEERRORTYPE, "imas beginObject", err, "Insufficient data parameters passed - begin not possible!");
	    break;
	 }
...
// Object? The data is the local object reference 
         ...
         if(putDataBlockList->putDataBlock[0].data == NULL){
            err = 999;
            if(verbose) fprintf(errout, "imas beginObject: No data object has been specified!\n");      
            addIdamError(&idamerrorstack, CODEERRORTYPE, "imas beginObject", err, "No data object has been specified!");
            break;              
	 }   	 	 	 	    	 
...
// Test all required data is available	  
         ...
         if(!isPath || !isIndex || !isTypeName || !isRank || !isClientObjectId){
            err = 999;
            if(verbose) fprintf(errout, "imas getObject: Insufficient data parameters passed - gett not possible!\n");      
            addIdamError(&idamerrorstack, CODEERRORTYPE, "imas getObject", err, "Insufficient data parameters passed - get not possible!");
	    break;
	 }	 
...
// Test all required data is available	  
         ...
         if(!isPath || !isIndex || !isTypeName || !isPutData || putDataBlockList->blockCount != 2){
            err = 999;
            if(verbose) fprintf(errout, "imas getObject: Insufficient data parameters passed - put not possible!\n");      
	 ...
	 if(putDataBlockList->putDataBlock[1].data_type != TYPE_INT){		 
            err = 999;
            if(verbose) fprintf(errout, "imas getObject: The shape's Type is incorrect!\n");      
...
// Shape Data?
         ...
         if(putDataBlockList->putDataBlock[1].data == NULL){
            err = 999;
            if(verbose) fprintf(errout, "imas getObject: No shape data has been specified!\n");      
...
// Object? The data is the local object reference 
         ...
         if(putDataBlockList->putDataBlock[0].data == NULL){
            err = 999;
            if(verbose) fprintf(errout, "imas getObject: No data object has been specified!\n");      
...
// Test all required data is available	  
         ...
         if(!isPath || !isIndex || !isPutData || putDataBlockList->blockCount != 2){
            err = 999;
            if(verbose) fprintf(errout, "imas putObject: Insufficient data parameters passed - put not possible!\n");      
            addIdamError(&idamerrorstack, CODEERRORTYPE, "imas putObject", err, "Insufficient data parameters passed - put not possible!");
	    break;
	 }	 
	    
	 if((type = findIMASType(convertIdam2StringType(putDataBlockList->putDataBlock[1].data_type))) == 0){		// Convert an IDAM type to an IMAS type
            err = 999;
            if(verbose) fprintf(errout, "imas putObject: The data's Type cannot be converted!\n");      
            addIdamError(&idamerrorstack, CODEERRORTYPE, "imas putObject", err, "The data's Type cannot be converted!");
            break;              
	 }
...
// Any Data?
         ...
         if(putDataBlockList->putDataBlock[1].data == NULL){
            err = 999;
            if(verbose) fprintf(errout, "imas putObject: No data has been specified!\n");      
...
// Object?
         ...
         if(putDataBlockList->putDataBlock[0].data == NULL){
            err = 999;
            if(verbose) fprintf(errout, "imas putObject: No data object has been specified!\n");      
            addIdamError(&idamerrorstack, CODEERRORTYPE, "imas putObject", err, "No data object has been specified!");
            break;              
	 }   	 
...
// The type passed here is the IMAS type enumeration  
	 ...
	 rc = imas_putDataSliceInObject(obj, path, index, type, putDataBlockList->putDataBlock[1].rank, 
	                                putDataBlockList->putDataBlock[1].shape, putDataBlockList->putDataBlock[1].data);
...
// Test all required data is available	  
         ...
         if(!isClientObjectId){
            err = 999;
            if(verbose) fprintf(errout, "imas putObjectInObject: Insufficient data parameters passed - put not possible!\n");      
            addIdamError(&idamerrorstack, CODEERRORTYPE, "imas putObjectInObject", err, "Insufficient data parameters passed - put not possible!");
	    break;
	 }	 	    
...
// Return Success
	 ...
	 data_block->data_n = 1;
      ...
      if(!strcasecmp(request_block->function, "endIdsPut")){
