...
{
    int err = 999;
    udaAddError(UDA_CODE_ERROR_TYPE, "readHDF5", err, "Cannot Read HDF5 Files - PLUGIN NOT ENABLED");
    return err;
}
...
int readHDF5Att(hid_t file_id, char* object, hid_t att_id, char* attname, DATA_BLOCK* data_block)
{
    ...
    if ((space_id = H5Aget_space(att_id)) < 0) {
        err = 999;
        udaAddError(UDA_CODE_ERROR_TYPE, "readHDF5Att", err, "Error Querying for Attribute Space Information");
        return err;
    }
    ...
    if (size == 0) {
        err = 999;
        udaAddError(UDA_CODE_ERROR_TYPE, "readHDF5Att", err, "Attribute Size is Zero!");
        return err;
    }
    ...
    if (data_block->data_type == UDA_TYPE_UNKNOWN) {
        err = 999;
        udaAddError(UDA_CODE_ERROR_TYPE, "readHDF5Att", err, "Attribute Data Type is Unknown!");
        return err;
    }
    ...
    if (data == NULL) {
        err = 999;
        udaAddError(UDA_CODE_ERROR_TYPE, "readHDF5Att", err, "Unable to Allocate HEAP Memory for Attribute Data");
        return err;
    }
    ...
    if (rc < 0) {
        err = 999;
        udaAddError(UDA_CODE_ERROR_TYPE, "readHDF5Att", err, "Error reading Attribute Data");
        free(data);
        return err;
    }
    ...
    if (data_block->rank >= 1 && data_block->data_n > 1) {
        if ((data_block->dims = (DIMS*)malloc(data_block->rank * sizeof(DIMS))) == NULL) {
            ...
            udaAddError(UDA_CODE_ERROR_TYPE, "readHDF5", err, "Problem Allocating Dimension Heap Memory");
            return err;
        }
...
#  endif
    ...
    do {
        ...
        if ((int)file_id < 0 || serrno != 0) {
            ...
            if (serrno != 0) {
                udaAddError(UDA_SYSTEM_ERROR_TYPE, "readHDF5", serrno, "");
            }
            udaAddError(UDA_CODE_ERROR_TYPE, "readHDF5", err, "Error Opening HDF5 File");
            break;
        }
        ...
        if ((dataset_id = H5Dopen2(file_id, signal_desc.signal_name, H5P_DEFAULT)) < 0) {
            ...
            udaAddError(UDA_CODE_ERROR_TYPE, "readHDF5", err, "Error Opening the Signal Dataset");
            break;
        }
...
#  endif
            ...
            udaAddError(UDA_CODE_ERROR_TYPE, "readHDF5", err, "Error Accessing Signal Dataset Information");
            break;
        }
        ...
        if (dataset_type == H5O_TYPE_DATASET) { // Dataset Object
            if ((space_id = H5Dget_space(dataset_id)) < 0) {
                ...
                udaAddError(UDA_CODE_ERROR_TYPE, "readHDF5", err, "Error Opening the Dataspace for the Dataset");
                break;
            }
...
#  endif
            ...
            if ((space_id = H5Aget_space(dataset_id)) < 0) {
                err = HDF5_ERROR_OPENING_DATASPACE;
                udaAddError(UDA_CODE_ERROR_TYPE, "readHDF5", err, "Error Opening the Dataspace for the Attribute");
                break;
            }
            data_block->rank = (unsigned int)H5Sget_simple_extent_dims(space_id, (hsize_t*)shape, 0);
...
#  endif
        ...
        if (size == 0) {
            ...
            udaAddError(UDA_CODE_ERROR_TYPE, "readHDF5", err, "No Storage Size returned for this data item");
            break;
        }
        ...
        if (data_block->data_type == UDA_TYPE_UNKNOWN) {
            ...
            udaAddError(UDA_CODE_ERROR_TYPE, "readHDF5", err, "Unknown Data Type for this data item");
            break;
        }
...
#  endif
        ...
        if (data == NULL) {
            ...
            udaAddError(UDA_CODE_ERROR_TYPE, "readHDF5", err, "Problem Allocating Data Heap Memory");
            break;
        }

        if (status < 0) {
            err = HDF5_ERROR_READING_DATA;
            udaAddError(UDA_CODE_ERROR_TYPE, "readHDF5", err, "Problem Reading Data from the File");
            break;
        }
