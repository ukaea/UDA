  ...
  idamSetLogLevel(LOG_DEBUG);
	...
	for (i = 0; i < nRows; i++){
	  ...
	  if (!PQgetisnull(DBQuery, i, s_name_fnum)){
	    stringLength = strlen(PQgetvalue(DBQuery, i, s_name_fnum))+1;
	  ...
	  if (!PQgetisnull(DBQuery, i, s_desc_fnum)){
	    stringLength = strlen(PQgetvalue(DBQuery, i, s_desc_fnum))+1;
	  ...
	  if (!PQgetisnull(DBQuery, i, s_cat_fnum)){
	    stringLength = strlen(PQgetvalue(DBQuery, i, s_cat_fnum))+1;
	...
	char query[MAXSQL];
	sprintf(query, "SELECT t.tag_id, ts.signal_desc_id FROM tag t, signal_tags ts WHERE tree_id=(SELECT tree_id FROM tag_tree WHERE tree_name='%s') AND t.tag_id=ts.tag_id;", treename_for_query);
	...
	char query[MAXSQL];
	sprintf(query, "SELECT tag_id, tag_name, parent_tag_id FROM tag WHERE tree_id=(SELECT tree_id FROM tag_tree WHERE tree_name='%s')",
		treename_for_query);
	free(treename_for_query);
