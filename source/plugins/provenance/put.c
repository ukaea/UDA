...
int put(IDAM_PLUGIN_INTERFACE* idam_plugin_interface)
{
    ...
    idamLog(LOG_DEBUG, "Provenance: entering function 'put'\n");
...
// Provenance Data Archive location
    ...
    if (root[0] == '\0') {
        ...
        if (env != NULL) {
            ...
            root[MAXROOT - 1] = '\0';
            TrimString(root);
            idamLog(LOG_DEBUG, "Provenance: Archive root directory [%s]\n", root);
        } else {
            err = 999;
            addIdamError(&idamerrorstack, CODEERRORTYPE, "Provenance", err,
...
// Create a directory within the provenance archive to receive files and a log
        ...
        if (err == 0) idamLog(LOG_DEBUG, "Provenance Archive Directory: [%s]\n", newDir);
...
// Open a log file to record all activities and errors
        ...
        if (log == NULL || errno != 0) {
            ...
            idamLog(LOG_DEBUG, "Provenance Error: Unable to Open the Provenance Log file! [%s]\n", logName);
            free((void*) logName);
            break;
        }
...
// Copy the file
        ...
        if (ph == NULL || errno != 0) {
            ...
            idamLog(LOG_DEBUG, "Error: Unable to copy the file! [%s]\n", copyFile);
        ...
        idamLog(LOG_DEBUG, "Provenance: scp Cost %d (ms), %d (microsecs)\n", msecs, usecs);
        tv_start = tv_stop;
...
// Read file attributes
        ...
        if ((rc = stat(copyFile, &attributes)) != 0 || errno != 0) {
            ...
            idamLog(LOG_DEBUG, "Error: Unable to read the file's attributes! [%s] %s\n", copyFile, strerror(errno));
            break;
        }
...
// Check the type (regular file only)
        ...
        if (!S_ISREG(attributes.st_mode)) {
            ...
            idamLog(LOG_DEBUG, "Error: Not a Regular File! [%s]\n", copyFile);
            break;
        }
...
// Open for Copying (no native c function!)          
        ...
        if ((in = fopen(copyFile, "rb")) == NULL || errno != 0) {
            ...
            idamLog(LOG_DEBUG, "Error: Unable to Open the file for copying! [%s]\n", copyFile);
            break;
        }
        ...
        if ((out = fopen(newFile, "w")) == NULL || errno != 0) {    // Replace if the file exists
            ...
            idamLog(LOG_DEBUG, "Error: Unable to Open the file for writing! [%s]\n", newFile);
            break;
        }
        ...
        while ((count = fread(buffer, sizeof(char), BUFFERSIZE, in)) > 0 && errno == 0) {
            if ((n = fwrite(buffer, sizeof(char), count, out)) != count || errno != 0) {
                ...
                idamLog(LOG_DEBUG, "Error: Unable to Write the file! [%s]\n", newFile);
                break;
            }
        }
        if (errno != 0) {
            ...
            idamLog(LOG_DEBUG, "Error: Unable to Read and Write the file! [%s, %s]\n", copyFile, newFile);
            break;
        }
...
// Set the file copy's modification time
        ...
        if ((rc = utime(newFile, &mtime)) != 0 || errno != 0) {
            ...
            idamLog(LOG_DEBUG, "Error: Unable to Set the file copy's time stamp! [%s]\n", newFile);
            break;
        }
...
// Hash the files and compare
        ...
        if ((rc = fileSHA1(copyFile, oldHash)) != 0) {
            ...
            idamLog(LOG_DEBUG, "Error: Unable to hash the original file! [%s]\n", copyFile);
            break;
        }
        if ((rc = fileSHA1(newFile, newHash)) != 0) {
            ...
            idamLog(LOG_DEBUG, "Error: Unable to hash the file copy! [%s]\n", newFile);
            break;
        }
        ...
        if (rc != 0) {
            ...
            idamLog(LOG_DEBUG, "Error: The original file and the file copy have different hash sums!\n");
            break;
        }
