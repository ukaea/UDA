...
// Name Value pairs (Keywords have higher priority)
        ...
        UDA_LOG(UDA_LOG_DEBUG, "Provenance: passed parameters\n");
        if (uuidOK) UDA_LOG(UDA_LOG_DEBUG, "uuid = %s\n", uuid);
        if (requestedSignalOK) UDA_LOG(UDA_LOG_DEBUG, "requestedSignal = %s\n", requestedSignal);
        if (requestedSourceOK) UDA_LOG(UDA_LOG_DEBUG, " requestedSource = %s\n", requestedSource);
        if (trueSignalOK) UDA_LOG(UDA_LOG_DEBUG, "trueSignal = %s\n", trueSignal);
        if (trueSourceOK) UDA_LOG(UDA_LOG_DEBUG, "trueSource = %s\n", trueSource);
        if (trueSourceUUIDOK) UDA_LOG(UDA_LOG_DEBUG, "trueSourceUUID = %s\n", trueSourceUUID);
        if (logRecordOK) UDA_LOG(UDA_LOG_DEBUG, "logRecord = %s\n", logRecord);
...
// Login password is stored in .pgpass for POSTGRESQL database so no need to set	       
                        ...
                        UDA_LOG(UDA_LOG_DEBUG, "Provenance: update() execMethod 1 Cost A = %d (ms), %d (microsecs)\n",
                                msecs, usecs);
                        tv_start = tv_stop;
...
// Create a temporary file to collect the SQL commands
                        ...
                        usecs = (int) (tv_stop.tv_sec - tv_start.tv_sec) * 1000000 +
                                (int) (tv_stop.tv_usec - tv_start.tv_usec);
                        UDA_LOG(UDA_LOG_DEBUG, "Provenance: update() execMethod 3 Cost A = %d (ms), %d (microsecs)\n",
                                msecs, usecs);
                        tv_start = tv_stop;
                    } else {
                        err = 999;
                        UDA_LOG(UDA_LOG_ERROR, "ERROR Provenance add: Incorrect execMethod argument {1|2|3}\n");
                        addIdamError(CODEERRORTYPE, "Provenance add", err,
...
// Return the Primary Key
                    ...
                    if (nrows != 1) {
                        UDA_LOG(UDA_LOG_ERROR, "ERROR Provenance new: New signals_log record not found!\n");
                        err = 999;
...
// Complete the transaction
            ...
            if (status == 'u') {    // update an existing record using the key from the ADD step
                ...
                if (!logRecordOK) {
                    err = 999;
                    UDA_LOG(UDA_LOG_ERROR, "ERROR Provenance add: No Log record!\n");
                ...
                if (execMethodOK) {
                    if (execMethod == 1) {
                        ...
                        UDA_LOG(UDA_LOG_DEBUG, "Provenance: update() SQL\n%s\n", cmd);
                        ...
                        usecs = (int) (tv_stop.tv_sec - tv_start.tv_sec) * 1000000 +
                                (int) (tv_stop.tv_usec - tv_start.tv_usec);
                        UDA_LOG(UDA_LOG_DEBUG, "Provenance: update() execMethod 1 Cost B = %d (ms), %d (microsecs)\n",
                                msecs, usecs);
                        tv_start = tv_stop;
                        ...
                        usecs = (int) (tv_stop.tv_sec - tv_start.tv_sec) * 1000000 +
                                (int) (tv_stop.tv_usec - tv_start.tv_usec);
                        UDA_LOG(UDA_LOG_DEBUG, "Provenance: update() execMethod 2 Cost B = %d (ms), %d (microsecs)\n",
                                msecs, usecs);
                        tv_start = tv_stop;
                        ...
                        UDA_LOG(UDA_LOG_DEBUG, "Provenance: database parameters\n");
                        UDA_LOG(UDA_LOG_DEBUG, "CLI     : %s\n", work);
                        UDA_LOG(UDA_LOG_DEBUG, "db name : %s\n", dbname);
                        UDA_LOG(UDA_LOG_DEBUG, "db user : %s\n", user);
                        UDA_LOG(UDA_LOG_DEBUG, "db host : %s\n", pghost);
                        UDA_LOG(UDA_LOG_DEBUG, "db port : %s\n", pgport);
                        UDA_LOG(UDA_LOG_DEBUG, "cmd file: %s\n", tmpfile);
                        ...
                        usecs = (int) (tv_stop.tv_sec - tv_start.tv_sec) * 1000000 +
                                (int) (tv_stop.tv_usec - tv_start.tv_usec);
                        UDA_LOG(UDA_LOG_DEBUG, "Provenance: update() execMethod 2 Cost C = %d (ms), %d (microsecs)\n",
                                msecs, usecs);
                        tv_start = tv_stop;
...
// Delete the SQL collection
                    ...
                    if (execMethod == 3) {
                        ...
                        usecs = (int) (tv_stop.tv_sec - tv_start.tv_sec) * 1000000 +
                                (int) (tv_stop.tv_usec - tv_start.tv_usec);
                        UDA_LOG(UDA_LOG_DEBUG, "Provenance: update() execMethod 3 Cost B = %d (ms), %d (microsecs)\n",
                                msecs, usecs);
                        tv_start = tv_stop;
                        ...
                        usecs = (int) (tv_stop.tv_sec - tv_start.tv_sec) * 1000000 +
                                (int) (tv_stop.tv_usec - tv_start.tv_usec);
                        UDA_LOG(UDA_LOG_DEBUG, "Provenance: update() execMethod 3 Cost C = %d (ms), %d (microsecs)\n",
                                msecs, usecs);
                        tv_start = tv_stop;
                    }
...
// Execute the SQL
                    ...
                    if ((DBQuery = PQexec(DBConnect, sql)) == NULL || PQresultStatus(DBQuery) != PGRES_COMMAND_OK) {
                        PQclear(DBQuery);
                        UDA_LOG(UDA_LOG_ERROR, "ERROR Provenance add: signals_log table update failed!\n");
                        err = 999;
                        addIdamError(CODEERRORTYPE, "Provenance new", err,
                ...
                UDA_LOG(UDA_LOG_DEBUG, "Provenance: record() Close all records\n");
...
// Execute the SQL
                ...
                if ((DBQuery = PQexec(DBConnect, sql)) == NULL || PQresultStatus(DBQuery) != PGRES_COMMAND_OK) {
                    PQclear(DBQuery);
                    UDA_LOG(UDA_LOG_ERROR, "ERROR Provenance add: signals_log status update failed!\n");
                    err = 999;
                    addIdamError(CODEERRORTYPE, "Provenance new", err,
                ...
                UDA_LOG(UDA_LOG_DEBUG, "Provenance: record() Delete closed records\n");
...
// Execute the SQL
                ...
                if ((DBQuery = PQexec(DBConnect, sql)) == NULL || PQresultStatus(DBQuery) != PGRES_COMMAND_OK) {
                    PQclear(DBQuery);
                    UDA_LOG(UDA_LOG_ERROR, "ERROR Provenance add: signals_log deletion failed!\n");
                    err = 999;
...
// Return 
        ...
        UDA_LOG(UDA_LOG_DEBUG, "Provenance: exiting function record\n");
