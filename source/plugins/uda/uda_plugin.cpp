...
int uda::plugins::uda::Plugin::get(UDA_PLUGIN_INTERFACE* plugin_interface)
{
    ...
    */
    ...
    if (pathway == 1) { // Request via the Database
        ...
        debug("Calling udaGetAPI API (Database based Request)\n");
        debug("Signal: {}\n", signal.c_str());
        debug("Source: {}\n", source.c_str());
        ...
        debug("UDA Server Host for UDA Plugin {}\n", request->server);
        debug("UDA Server Port for UDA Plugin {}\n", newPort);
        debug("Calling udaGetAPI API (Device redirect or server protocol based Request)\n");
        debug("Signal: {}\n", request->signal);
        debug("Source: {}\n", source);
        ...
        debug("UDA Server Host for UDA Plugin {}\n", host);
        debug("UDA Server Port for UDA Plugin {}\n", newPort);
        debug("Calling udaGetAPI API (plugin library method based Request)\n");
        ...
        debug("UDA Server Host for UDA Plugin {}\n", request->server);
        debug("UDA Server Port for UDA Plugin {}\n", newPort);
        debug("Calling udaGetAPI API (Server protocol based Request)\n");
        debug("Signal: {}\n", request->signal);
        debug("Source: {}\n", source);
    ...
    debug("Returned from udaGetAPI API: handle = {}, error code = {}\n", handle, udaGetErrorCode(handle));

    if (handle < 0) {
        error(udaGetServerErrorStackRecordMsg(0));
