...
int checkAvailableSignals(int shot, int n_all, int** signal_ids, int** is_available){
  ...
  PGconn* DBConnect = (PGconn*) startSQL();
  ...
  for (i = 0; i < n_all; i++){
    ...
    sprintf(query_idam, "SELECT ds.exp_number FROM data_source ds, signal s, signal_desc sd WHERE ds.exp_number=%d AND ds.source_id=s.source_id AND s.signal_desc_id=sd.signal_desc_id AND sd.signal_desc_id=%d;", shot, (*signal_ids)[i]);
...
int generateGeom(char* signal, float tor_angle, PGconn* DBConnect, PGresult* DBQuery)
{
  ...
  sprintf(query_signal, "SELECT g.name FROM geomgroup g, geomgroup_geomsignal_map ggm WHERE lower(ggm.geomsignal_alias)=lower('%s') AND g.geomgroup_id=ggm.geomgroup_id;", signal);
...
int idamGeom(IDAM_PLUGIN_INTERFACE* idam_plugin_interface)
{
	...
	strcpy(data_block->data_units, "");
      ...
      } else if (! strcmp(request_block->function, "get")) {
	...
	for (i_arg = 0; i_arg < n_args; i_arg++) {
	  if (!strcasecmp(request_block->nameValueList.nameValue[i_arg].name, "signal")) {
	    ...
	    isCal = 1;
	  } else if (!strcasecmp(request_block->nameValueList.nameValue[i_arg].name, "version_cal")) {
	  ...
	  } else if (!strcasecmp(request_block->nameValueList.nameValue[i_arg].name, "version_config")) {
	    if (IsNumber(request_block->nameValueList.nameValue[i_arg].value)) {
	      versionConfig = atoi(request_block->nameValueList.nameValue[i_arg].value);
	    }
	  } else if (!strcasecmp(request_block->nameValueList.nameValue[i_arg].name, "three_d")){
	    three_d = 1;
	  } else if (!strcasecmp(request_block->nameValueList.nameValue[i_arg].name, "tor_angle")){
	    ...
	    if (IsFloat(request_block->nameValueList.nameValue[i_arg].value)) {
	      tor_angle = atof(request_block->nameValueList.nameValue[i_arg].value);
	      if (tor_angle < 0 || tor_angle > 2*M_PI) {
		idamLog(LOG_DEBUG, "geom plugin: If providing a toroidal angle, it must be in the range 0 - 2 pi. Angle given will be ignored.\n");
		tor_angle = -1;
	      }
	...
	if (isConfig == 1 || isCal == 1) {
	  ...
	  PGconn* DBConnect = PQconnectdb("dbname=idam user=idam password=idam@idam3 host=idam3.mast.ccfe.ac.uk port=60000"); // Idam3 for testing
	  ...
	  if (isConfig == 1) {
	    // configuration files
	    sprintf(query, "SELECT cds.file_name, ggm.geomsignal_alias, cds.version FROM config_data_source cds, geomgroup_geomsignal_map ggm WHERE cds.config_data_source_id=ggm.config_data_source_id AND lower(ggm.geomsignal_alias) LIKE lower('%s/%%') AND cds.start_shot<=%d AND cds.end_shot>%d", signal, shot, shot);
	    ...
	    if (three_d == 1) {
	      ...
	      sprintf(tor_angle_statement, 
		      " AND (CASE WHEN cds.tor_angle_dependent=TRUE THEN round(cds.tor_angle::numeric, 4)=round(%f, 4) ELSE TRUE END)", tor_angle);	    
	    ...
	    // calibration files 
	    sprintf(query, "SELECT cds.file_name, ggm.geomsignal_alias, cds.version FROM cal_data_source cds INNER JOIN config_data_source cods ON cods.config_data_source_id=cds.config_data_source_id INNER JOIN geomgroup_geomsignal_map ggm ON ggm.config_data_source_id=cods.config_data_source_id WHERE lower(ggm.geomsignal_alias) LIKE lower('%s/%%') AND cds.start_shot<=%d AND cds.end_shot>%d", signal, shot, shot);
	  ...
	  // Add check for toroidal angle 
	    if (three_d == 1) {
	      ...
	      char tor_angle_statement[MAXSQL];
	      sprintf(tor_angle_statement, 
		      " AND (CASE WHEN cods.tor_angle_dependent=TRUE THEN round(cds.tor_angle::numeric, 4)=round(%f, 4) ELSE TRUE END)", tor_angle);	    
	      strcat(query, tor_angle_statement);
	    } else {
	...
	DATAPLUSTYPE* data;
	int stringLength = strlen(signal_type) + 1;
	...
	break;
      } else if (! strcmp(request_block->function, "getSignalFile")) {
	...
	for (i_arg=0; i_arg < n_args; i_arg++) {
	  if (! strcasecmp(request_block->nameValueList.nameValue[i_arg].name, "signal")) {
	    signal = request_block->nameValueList.nameValue[i_arg].value;
	    idamLog(LOG_DEBUG, "configPlugin: Using signal name: %s\n", signal);
	  } else if (! strcasecmp(request_block->nameValueList.nameValue[i_arg].name, "version")) {
	    if (IsNumber(request_block->nameValueList.nameValue[i_arg].value)) {
	      version = atoi(request_block->nameValueList.nameValue[i_arg].value);
	    }
	  } else if (! strcasecmp(request_block->nameValueList.nameValue[i_arg].name, "keep_all")){
	    keep_all = 1;
	  }
	}
	...
	PGconn* DBConnect = PQconnectdb("dbname=idam user=idam password=idam@idam3 host=idam3.mast.ccfe.ac.uk port=60000"); // idam3 for testing/dev version
	PGresult* DBQuery = NULL;
	...
	int err = readCDF(*data_source, *signal_desc, *request_block, &data_block_file);
	...
	if (n_signals_available > 0){
	  ...
	  for (i = 0; i < nRows; i++){
	    if (is_available[i] == 1){
	      int strLength = strlen(all_sig_alias[i]) + 1;
	      data->signal_alias_available[i_avail] = (char*)malloc(strLength * sizeof(char));
	      addMalloc(data->signal_alias_available[i_avail], strLength, sizeof(char), "char");
	...
	break;
      ...
      } else if (! strcmp(request_block->function, "getSignalFilename")){
	...
	for (i_arg=0; i_arg < n_args; i_arg++) {
	  if (!strcasecmp(request_block->nameValueList.nameValue[i_arg].name, "geomsignal")) {
	    signal = request_block->nameValueList.nameValue[i_arg].value;
	    idamLog(LOG_DEBUG, "configPlugin: Using signal name: %s\n", signal);
	  } else if (! strcasecmp(request_block->nameValueList.nameValue[i_arg].name, "version")){
	    if (IsNumber(request_block->nameValueList.nameValue[i_arg].value)) {
	...
	idamLog(LOG_DEBUG, "geom: trying to get connection\n");
	PGconn* DBConnect = PQconnectdb("dbname=idam user=idam password=idam@idam3 host=idam3.mast.ccfe.ac.uk port=60000");
	...
	if (versionDependent >= 0) {
	  ...
	  sprintf(version_statement, " AND datasignal_geomdata_source.version=(SELECT max(datasignal_geomdata_source.version) FROM datasignal_geomdata_source WHERE datasignal_geomdata_source.start_shot<=%d AND datasignal_geomdata_source.end_shot>%d)", shot, shot);
	...
	for (i = 0; i < nRows; i++){
	  ...
	  if (!PQgetisnull(DBQuery, i, s_geom_alias)){
	    int stringLength = strlen(PQgetvalue(DBQuery, i, s_geom_alias))+1;
	  ...
	  // Signal alias
	  if (!PQgetisnull(DBQuery, i, s_sig_alias)){
	    int stringLength = strlen(PQgetvalue(DBQuery, i, s_sig_alias))+1;
	  ...
	  // Varname
	  if (!PQgetisnull(DBQuery, i, s_sig_alias)){
	    int stringLength = strlen(PQgetvalue(DBQuery, i, s_var_name))+1;
	  ...
	  // File name
	  if (!PQgetisnull(DBQuery, i, s_file)){
	    int stringLength = strlen(PQgetvalue(DBQuery, i, s_file))+strlen(file_path)+1;
	...
	for (i = 0; i < nRows; i++){
	  if (data->is_available[i] > 0) {
	    ...
	    addMalloc((void *) data_out->geom_alias[out_index], stringLength, sizeof(char), "char");
	    ...
	    addMalloc((void *) data_out->signal_alias[out_index], stringLength, sizeof(char), "char");
	    ...
	    addMalloc((void *) data_out->var_name[out_index], stringLength, sizeof(char), "char");
