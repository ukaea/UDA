...
int do_group(IDAM_PLUGIN_INTERFACE* idam_plugin_interface)
{
    ...
    UDA_LOG(UDA_LOG_DEBUG, "Testing whole group Hierarchy [%s]\n", name);
    ...
    if (!STR_EQUALS(name, "/")) {
        ...
        if ((token = strtok(work, "/")) != NULL) {
            ...
            UDA_LOG(UDA_LOG_DEBUG, "Testing group [%s]\n", token);
	    ...
	    int grpid;
            ...
            UDA_LOG(UDA_LOG_DEBUG, "Group ID %d\n", grpid);

            if (status == 1) {
                // Create all Child Nodes

                UDA_LOG(UDA_LOG_DEBUG, "Group %s did not exist - Created with all child groups\n", token);

                while ((token = strtok(NULL, "/")) != NULL) {
                    UDA_LOG(UDA_LOG_DEBUG, "Creating Group [%s]\n", token);
                ...
                UDA_LOG(UDA_LOG_DEBUG, "Group [%s] Found - Locating all child groups\n", token);
                ...
                while ((token = strtok(NULL, "/")) != NULL) {        // Loop over all Intermediate Groups
                    ...
                    if (status == 1) {                    // Create all Child Nodes
                        ...
                        UDA_LOG(UDA_LOG_DEBUG, "Group [%s] Found\n", token);
                    }
        ...
        UDA_LOG(UDA_LOG_DEBUG, "Top Level Group Selected\n");
    }
...
int testmakegroup(int locid, const char* name, int* targetid){
  ...
  *targetid = -1;     // Target Group ID
  ...
  if (!STR_EQUALS(name, "/") && !STR_EQUALS(name, "")) {
    ...
    UDA_LOG(UDA_LOG_DEBUG, "Checking if we can tokenise\n");
	...
	// Find or Define Child Nodes
	UDA_LOG(UDA_LOG_DEBUG, "Group [%s] Found - Locating all child groupsa\n", token);
...
int testgroup(int locid, const char* target, int* status, int* targetid)
{
    ...
    *targetid = -1;     // Target Group ID
    ...
    UDA_LOG(UDA_LOG_DEBUG, "How many Child Groups (ID %d) ?\n", locid);
    ...
    UDA_LOG(UDA_LOG_DEBUG, "Child Groups Count = %d\n", numgrps);

    if (numgrps > 0) {

        int* ncids = (int*) malloc(sizeof(int) * numgrps);

        UDA_LOG(UDA_LOG_DEBUG, "Listing Child Groups\n");
	...
	UDA_LOG(UDA_LOG_DEBUG, "numgrps %d\n", numgrps);

        // Test existing Groups
        int i;
	int ngrps = numgrps;
        for (i = 0; i < ngrps; i++) {
	    UDA_LOG(UDA_LOG_DEBUG, "i %d\n", i);
	    UDA_LOG(UDA_LOG_DEBUG, "Testing Child Group %d\n", ncids[i]);
	    ...
	    char* grpname = NULL;
            ...
            if (nc_inq_grpname_len(ncids[i], (size_t*) &namelength) != NC_NOERR) {
                free((void*) grpname);
                free((void*) ncids);
		UDA_LOG(UDA_LOG_DEBUG, "Unable to Obtain Group Name Length\n");
            ...
            if (nc_inq_grpname(ncids[i], grpname) != NC_NOERR) {
                free((void*) grpname);
                free((void*) ncids);
		UDA_LOG(UDA_LOG_DEBUG, "Unable to Name an existing Group\n");
                RAISE_PLUGIN_ERROR("Unable to Name an existing Group");
            }

            UDA_LOG(UDA_LOG_DEBUG, "Comparing Group Name [%s] with Target Group [%s]", grpname, target);

            if (STR_EQUALS(grpname, target)) {
                UDA_LOG(UDA_LOG_DEBUG, "Group Found %d\n", ncids[i]);
	    ...
	    UDA_LOG(UDA_LOG_DEBUG, "End of loop i %d numgrps %d\n", i, ngrps);
        }
    ...
    if (*targetid == -1) {
        ...
        UDA_LOG(UDA_LOG_DEBUG, "Creating the Group [%s]\n", target);

        if (nc_def_grp(locid, target, targetid) != NC_NOERR) {
	  UDA_LOG(UDA_LOG_DEBUG, "Unable to Create a Named Group\n");
