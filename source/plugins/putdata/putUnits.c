...
int testUnitsCompliance(const char* units)
{
  ...
  UDA_LOG(UDA_LOG_DEBUG, "Testing unit compliance\n");
    ...
    if (unitSystem == NULL) {
      UDA_LOG(UDA_LOG_DEBUG, "Unit system is NULL: Try to retrieve it\n");
        unitSystem = ut_read_xml(NULL);
        if (!setlocale(LC_CTYPE, "")) {
            UDA_LOG(UDA_LOG_DEBUG, "Can't set the specified locale! Check LANG, LC_CTYPE, LC_ALL");
            return 0;
        }
    }

    UDA_LOG(UDA_LOG_DEBUG, "Parse units %s\n", units);

    if (unitSystem == NULL){
      UDA_LOG(UDA_LOG_DEBUG, "unitSystem is NULL\n");
    }

    if ((encoded = ut_parse(unitSystem, units, encoding)) == NULL) {
        int code = (int) ut_get_status();
        UDA_LOG(UDA_LOG_ERROR, "Units [%s] are Not SI Compliant.");
        if (code == UT_SYNTAX) {
    ...
    UDA_LOG(UDA_LOG_DEBUG, "Free encoded\n");
    ut_free(encoded);
