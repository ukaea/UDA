...
int do_dimension(IDAM_PLUGIN_INTERFACE* idam_plugin_interface)
{
    ...
    UDA_LOG(UDA_LOG_DEBUG, "File id retrieved: %d \n", ncfileid);
    ...
    UDA_LOG(UDA_LOG_DEBUG, "Writing Dimension %s = %d\n", name, length);
    UDA_LOG(UDA_LOG_DEBUG, "to Group %s \n", group);
    ...
    if (unlimited || length == 0) {
        UDA_LOG(UDA_LOG_DEBUG, "Dimension is UNLIMITED");
...
int do_coordinate(IDAM_PLUGIN_INTERFACE* idam_plugin_interface)
{
    ...
    UDA_LOG(UDA_LOG_DEBUG, "Creating the Coordinate Variable %s [%d]\n", name, nctype);
    ...
    UDA_LOG(UDA_LOG_DEBUG, "Unit name is %s\n", units);
                         ...
                         int* varid, size_t* length, int chunksize, int compression, int ctype, int dctype)
{
    ...
    *length = putdata->count;
    ...
    UDA_LOG(UDA_LOG_DEBUG, "NC TYPE %d\n", nctype);
                             ...
                             int ctype, int dctype)
{
    ...
    UDA_LOG(UDA_LOG_DEBUG, "Write attribute\n");
