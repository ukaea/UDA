...
* Change History
*
* 18Jan2016	D.G.Muir	Original Version
*---------------------------------------------------------------------------------------------------------------*/
...
// Database Objects
   ...
   bson_t       *query, *command, reply, *insert;
   const bson_t *doc;
   bson_error_t  error;
   char         *str;
   bool          retval;
...
*/
         ...
         if(environment->sql_user[0] != '\0' && environment->sql_host[0] != '\0' && environment->sql_dbname[0] != '\0' && environment->sql_port > 0  && password != NULL)
            ...
            err = 999;
            addIdamError(&idamerrorstack, CODEERRORTYPE, "MongoDBPlugin", err, "Insufficient Connection and Authentication details!");
            concatIdamError(idamerrorstack, idamErrorStack);
         ...
         client = mongoc_client_new (uri); 	// Create a new client instance (Not thread safe) authenticating with the IDAM database
	    ...
	    if(debugon) fprintf(dbgout, "mongodbplugin: Private regular database connection made.\n");
         }
      } 
         
      if(!client){
         err = 999;
         addIdamError(&idamerrorstack, CODEERRORTYPE, "MongoDBPlugin", err, "No connection to Database server made!");
         concatIdamError(idamerrorstack, idamErrorStack);
      ...
      if(!database){
         err = 999;
         addIdamError(&idamerrorstack, CODEERRORTYPE, "MongoDBPlugin", err, "No connection to Database cluster made!");
         concatIdamError(idamerrorstack, idamErrorStack);
      ...
      if(!collection){
         err = 999;
         addIdamError(&idamerrorstack, CODEERRORTYPE, "MongoDBPlugin", err, "No handle to Database collection made!");
         concatIdamError(idamerrorstack, idamErrorStack);
...
// Keywords
            if(!strcasecmp(request_block->nameValueList.nameValue[i].name, "allMeta")){
	       allMeta = 1;	
	       continue;
	    }
         }
...
// Mandatory arguments
         ...
         if(!isObjectName){
            if(verbose) fprintf (errout, "MongoDBPlugin: No Data Object Name specified\n");
            if(debugon) fprintf (dbgout, "MongoDBPlugin: No Data Object Name specified\n");
            err =  999;
         ...
         if(!isExpNumber && !isObjectSource){
            ...
            if(!isExpNumber && !isObjectSource){
               if(verbose) fprintf (errout, "MongoDBPlugin: No Experiment Number or data source specified\n");
               if(debugon) fprintf (dbgout, "MongoDBPlugin: No Experiment Number or data source specified\n");
               err =  999;
               addIdamError(&idamerrorstack, CODEERRORTYPE, "MongoDBPlugin", err, "No Experiment Number or data source specified");
               break;
            }
...
// Measure performance
   ...
   if(debugon) gettimeofday(&tv_start, NULL);     
...
// Find the document
         ...
         if(!(cursor = mongoc_collection_find (collection, MONGOC_QUERY_NONE, 0, limit, 0, query, NULL, NULL))){
            if(verbose) fprintf (errout, "MongoDBPlugin: Data Object not found!\n");
            if(debugon) fprintf (dbgout, "MongoDBPlugin: Data Object not found!\n");
            err =  999;
         ...
         if(debugon){
            ...
            fprintf(dbgout,"Query Time: %d (micro sec)\n",cost);
            tv_start = tv_end;
...
// Returned Structures should be initialised prior to entry
         ...
         if(debugon){
            ...
            cost = (int)(tv_end.tv_sec-tv_start.tv_sec)*1000000 + (int)(tv_end.tv_usec - tv_start.tv_usec);
            fprintf(dbgout,"+++ mongodbPlugin +++\n");
            fprintf(dbgout,"Next Cursor Time: %d (micro sec)\n",cost);
            fprintf(dbgout,"Document available?: %d\n",docAvail);
            tv_start = tv_end;
         }         
	    ...
	    if(1 && debugon){
	       ...
	       fprintf (dbgout,"\n\nSignal Alias     : %s\n", objectName);
               fprintf (dbgout,"Experiment Number: %d\n", expNumber);
               if(isDevice) fprintf (dbgout,"Device name      : %s\n", device);
               fprintf (dbgout,"allMeta          : %d\n", allMeta);
               fprintf (dbgout,"%s\n\n", str);
 	       bson_free (str);
            }	 
            ...
            char *dbDevice = NULL;
            if (bson_iter_init (&iter, doc) && bson_iter_find_descendant (&iter, "source_device", &value) && BSON_ITER_HOLDS_UTF8 (&value)){
                uint32_t length=0;
		...
		strcpy(request_block->source, data_source->path); 
            ...
            if (bson_iter_init (&iter, doc) && bson_iter_find_descendant (&iter, "signal_name", &value) && BSON_ITER_HOLDS_UTF8 (&value)){
                uint32_t length=0;
                const char *str = bson_iter_utf8 (&value, &length);
                strcpy(signal_desc->signal_name,  str);
            }
            if (bson_iter_init (&iter, doc) && bson_iter_find_descendant (&iter, "type", &value) && BSON_ITER_HOLDS_UTF8 (&value)){
                uint32_t length=0;
                const char *str = bson_iter_utf8 (&value, &length);
                signal_desc->type = str[0];
            }
            if (bson_iter_init (&iter, doc) && bson_iter_find_descendant (&iter, "source", &value) && BSON_ITER_HOLDS_UTF8 (&value)){
                uint32_t length=0;
                const char *str = bson_iter_utf8 (&value, &length);
                if(!isExpNumber){
                   if(dbDevice != NULL)                   
      ...
      if(allMeta){
            if (bson_iter_init (&iter, doc) && bson_iter_find_descendant (&iter, "generic_name", &value) && BSON_ITER_HOLDS_UTF8 (&value)){
                uint32_t length=0;
                const char *str = bson_iter_utf8 (&value, &length);
                strcpy(signal_desc->generic_name,  str);
            }
            if (bson_iter_init (&iter, doc) && bson_iter_find_descendant (&iter, "source_alias", &value) && BSON_ITER_HOLDS_UTF8 (&value)){
                uint32_t length=0;
                const char *str = bson_iter_utf8 (&value, &length);
                strcpy(signal_desc->source_alias,  str);
            }
            if (bson_iter_init (&iter, doc) && bson_iter_find_descendant (&iter, "signal_class", &value) && BSON_ITER_HOLDS_UTF8 (&value)){
                uint32_t length=0;
                const char *str = bson_iter_utf8 (&value, &length);
                strcpy(signal_desc->signal_class,  str);
            }
            if (bson_iter_init (&iter, doc) && bson_iter_find_descendant (&iter, "description", &value) && BSON_ITER_HOLDS_UTF8 (&value)){
                uint32_t length=0;
                const char *str = bson_iter_utf8 (&value, &length);
                strcpy(signal_desc->description,  str);
            }
            if (bson_iter_init (&iter, doc) && bson_iter_find_descendant (&iter, "range_start", &value) && BSON_ITER_HOLDS_INT32 (&value))
                signal_desc->range_start = bson_iter_int32 (&value);
            if (bson_iter_init (&iter, doc) && bson_iter_find_descendant (&iter, "range_end", &value) && BSON_ITER_HOLDS_INT32 (&value))
   ...
   if(debugon){
      ...
      cost = (int)(tv_end.tv_sec-tv_start.tv_sec)*1000000 + (int)(tv_end.tv_usec - tv_start.tv_usec);
      fprintf(dbgout,"+++ mongodbPlugin +++\n");
      fprintf(dbgout,"Parse/extract Time: %d (micro sec)\n",cost);
   }
...
// Create the Returned Structure Definition
	    ...
	    defineField(&field, "objectName", "The target data object's (abstracted) (signal) name", &offset, SCALARSTRING);
            ...
            addCompoundField(&usertype, field);	
            defineField(&field, "objectSource", "The target data object's (abstracted) source name", &offset, SCALARSTRING);
            addCompoundField(&usertype, field);	      	               	 
 	    defineField(&field, "device", "The device name where the data source is located", &offset, SCALARSTRING);
            addCompoundField(&usertype, field);
