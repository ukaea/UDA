...
void udaClientInitHostList()
{
	...
	while (fgets(buffer, HOST_STRING, conf) != NULL) {
		...
		if(!strcasecmp(next, "hostName")){			// Trigger a new set of attributes		    
		 ...
		 if(newHost && !strcasecmp(next, "hostAlias")){
		    next = &split[1];
		    LeftTrimString(TrimString(next));
		    if(next[0] != '\0' && strlen(next) < HOST_STRING){ 
		 ...
		 if(newHost && !strcasecmp(next, "port")){
		    next = &split[1];
		    LeftTrimString(TrimString(next));
		    if(next[0] != '\0' && strlen(next) < HOST_STRING){ 
		 ...
		 if(newHost && !strcasecmp(next, "certificate")){
		    next = &split[1];
		    LeftTrimString(TrimString(next));
		    if(next[0] != '\0' && strlen(next) < HOST_STRING){ 
		 ...
		 if(newHost && !strcasecmp(next, "privateKey")){
		    next = &split[1];
		    LeftTrimString(TrimString(next));
		    if(next[0] != '\0' && strlen(next) < HOST_STRING){ 
		 ...
		 if(newHost && !strcasecmp(next, "CA-Certificate")){
		    next = &split[1];
		    LeftTrimString(TrimString(next));
		    if(next[0] != '\0' && strlen(next) < HOST_STRING){ 
