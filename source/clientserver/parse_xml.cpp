...
void parse_fixed_length_str_array(xmlNodePtr cur, const char* target,
                                  char array[MaxDataRank][SxmlMaxString], int* n)
{
    xmlChar* att = nullptr;
    *n = 0;
    const char* delim = ",";
...
void parse_target_value(xmlDocPtr doc, xmlNodePtr cur, const char* target, double* value)
{
    ...
    while (cur != nullptr) {
        if ((!xmlStrcmp(cur->name, (const xmlChar*)target))) {
            ...
            if (scale != nullptr) {
                *value = *value * de_scale((char*)scale);
            }
            xmlFree(key);
            xmlFree(scale);
            break;
...
void parse_time_offset(xmlDocPtr doc, xmlNodePtr cur, Actions* actions)
{
    ...
    while (cur != nullptr) {
        ...
        if ((!xmlStrcmp(cur->name, (const xmlChar*)"time_offset"))) {
            ...
            init_action(&str[n - 1]);
            str[n - 1].actionType = (int)ActionType::Offset;
            ...
            if ((att = xmlGetProp(cur, (xmlChar*)"scale")) != nullptr) {
                if (strlen((char*)att) > 0) {
                    str[n - 1].timeoffset.offset = de_scale((char*)att) * str[n - 1].timeoffset.offset;
                }
...
void parse_composite_subset(xmlDocPtr doc, xmlNodePtr cur, Composite* comp)
{
    ...
    cur = cur->xmlChildrenNode;
    while (cur != nullptr) {
        UDA_LOG(UDA_LOG_DEBUG, "parse_composite_subset: {}", (char*)cur->name)
...
void parse_dim_composite(xmlDocPtr doc, xmlNodePtr cur, Composite* comp)
{
...
void parse_composite(xmlDocPtr doc, xmlNodePtr cur, Actions* actions)
{
    ...
    cur = cur->xmlChildrenNode;
    while (cur != nullptr) {
        UDA_LOG(UDA_LOG_DEBUG, "parse_composite: {}", (char*)cur->name)
        if ((!xmlStrcmp(cur->name, (const xmlChar*)"composite"))) {
            n++;
            str = (Action*)realloc((void*)str, n * sizeof(Action));

            init_action(&str[n - 1]);
            str[n - 1].actionType = (int)ActionType::Composite;
...
void parse_dim_error_model(xmlDocPtr doc, xmlNodePtr cur, ErrorModel* mod)
{
    xmlChar* att; // General Input of tag attribute values
    float* params;
    ...
    cur = cur->xmlChildrenNode;
    while (cur != nullptr) {
        UDA_LOG(UDA_LOG_DEBUG, "parse_dim_error_model: {}", (char*)cur->name)
        if ((!xmlStrcmp(cur->name, (const xmlChar*)"dimension"))) {
            n++;
            str = (Dimension*)realloc((void*)str, n * sizeof(Dimension));

            init_dimension(&str[n - 1]);
            str[n - 1].dimType = (int)ActionDimType::ErrorModel;
...
void parse_error_model(xmlDocPtr doc, xmlNodePtr cur, Actions* actions)
{
    ...
    while (cur != nullptr) {
        ...
        if ((!xmlStrcmp(cur->name, (const xmlChar*)"errormodel"))) {
            ...
            init_action(&str[n - 1]);
            str[n - 1].actionType = (int)ActionType::ErrorModel;
            ...
            parse_dim_error_model(doc, cur, &str[n - 1].errormodel);
        }
        cur = cur->next;
    }
    actions->nactions = n; // Number of Tags Found
...
void parse_dim_documentation(xmlDocPtr doc, xmlNodePtr cur, Documentation* document)
{
    xmlChar* att; // General Input of tag attribute values
    ...
    cur = cur->xmlChildrenNode;
    while (cur != nullptr) {
        UDA_LOG(UDA_LOG_DEBUG, "parse_dim_documentation: {}", (char*)cur->name)
        if ((!xmlStrcmp(cur->name, (const xmlChar*)"dimension"))) {
            n++;
            str = (Dimension*)realloc((void*)str, n * sizeof(Dimension));

            init_dimension(&str[n - 1]);
            str[n - 1].dimType = (int)ActionDimType::Documentation;
...
void parse_documentation(xmlDocPtr doc, xmlNodePtr cur, Actions* actions)
{
    ...
    while (cur != nullptr) {
        ...
        if ((!xmlStrcmp(cur->name, (const xmlChar*)"documentation"))) {
            ...
            init_action(&str[n - 1]);
            str[n - 1].actionType = (int)ActionType::Documentation;
            ...
            parse_dim_documentation(doc, cur, &str[n - 1].documentation);
        }
        cur = cur->next;
    }
    actions->nactions = n; // Number of Tags Found
...
void parse_dim_calibration(xmlDocPtr doc, xmlNodePtr cur, Calibration* cal)
{
    xmlChar* att; // General Input of tag attribute values
    ...
    cur = cur->xmlChildrenNode;
    while (cur != nullptr) {
        UDA_LOG(UDA_LOG_DEBUG, "parse_dim_calibration: {}", (char*)cur->name)
        if ((!xmlStrcmp(cur->name, (const xmlChar*)"dimension"))) {
            n++;
            str = (Dimension*)realloc((void*)str, n * sizeof(Dimension));

            init_dimension(&str[n - 1]);
            str[n - 1].dimType = (int)ActionDimType::Calibration;
...
void parse_calibration(xmlDocPtr doc, xmlNodePtr cur, Actions* actions)
{
    ...
    while (cur != nullptr) {
        ...
        if ((!xmlStrcmp(cur->name, (const xmlChar*)"calibration"))) {
            ...
            init_action(&str[n - 1]);
            str[n - 1].actionType = (int)ActionType::Calibration;
            ...
            parse_dim_calibration(doc, cur, &str[n - 1].calibration);
        }
        cur = cur->next;
    }
    actions->nactions = n; // Number of Tags Found
...
void parse_subset(xmlDocPtr doc, xmlNodePtr cur, Actions* actions)
{
    ...
    while (cur != nullptr) {
        ...
        if ((!xmlStrcmp(cur->name, (const xmlChar*)"subset"))) {
            ...
            init_action(&str[n - 1]);
            str[n - 1].actionType = (int)ActionType::Subset;
            sub = &str[n - 1].subset;
            init_subset(sub);
...
void print_dimensions(int ndim, Dimension* dims) {
    ...
    for (int i = 0; i < ndim; i++) {
        ...
        switch ((ActionDimType)dims[i].dimType) {

            case ActionDimType::Calibration:
