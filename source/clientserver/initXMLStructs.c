...
void printFluxLoop(FILE *fh, FLUXLOOP str) {
    ...
    fprintf(fh,"rerr       : %f\n", str.rerr);
    fprintf(fh,"nco        : %d\n", str.nco);
    if(str.nco > 0) for(i=0; i<str.nco; i++)fprintf(fh,"r, z, dphi   # %d     : %f   %f   %f\n", i, str.r[i], str.z[i], str.dphi[i]);
    return;
}
...
void printPfCoils(FILE *fh, PFCOILS str) {
    ...
    fprintf(fh,"nco        : %d\n", str.nco);
    for(i=0; i<str.nco; i++) fprintf(fh,"r, z, dr, dz # %d     : %f   %f   %f   %f\n", i, str.r[i], str.z[i], str.dr[i], str.dz[i]);
    return;
}
...
void printPfPassive(FILE *fh, PFPASSIVE str) {
    ...
    fprintf(fh,"rerr       : %f\n", str.rerr);
    fprintf(fh,"model nr nr: %d  %d\n", str.modelnrnz[0], str.modelnrnz[1]);
    for(i=0; i<str.nco; i++) fprintf(fh,"r,z,dr,dz,a1,a2,res   # %d     : %f  %f  %f  %f  %f  %f  %f\n",i,str.r[i],str.z[i],
