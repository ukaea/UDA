...
// If this client/server version cannot pass/receive a specific type, then return TRUE
    ...
    idamLog(LOG_DEBUG, "protocolVersionTypeTest Version: %d, Type: %d\n", protocolVersion, type);
...
// privateFlags does not exist in the CLIENT_BLOCK structure prior to version 5
    ...
    if (protocolVersion >= 7) {
        ...
        idamLog(LOG_DEBUG, "XDRLIB: protocolVersion %d\n", protocolVersion);
    ...
    if (xdrs->x_op == XDR_DECODE && rc) {                // Test for a server crash!
        if (serverVersion == 0)
            ...
            str->version = serverVersion;        // Replace the erroneous version number
                idamLog(LOG_DEBUG, "XDRLIB: Server #1 rc = %d\n", rc);
            return rc;
        }
    }

    idamLog(LOG_DEBUG, "XDRLIB: Server #1 rc[1] = %d, version = %d\n", rc, str->version);

    rc = rc && xdr_int(xdrs, &str->error);
    idamLog(LOG_DEBUG, "XDRLIB: Server #1 rc[2] = %d, error = %d\n", rc, str->error);
    rc = rc && xdr_u_int(xdrs, &str->idamerrorstack.nerrors);
        idamLog(LOG_DEBUG, "XDRLIB: Server #1 rc[3] = %d, error = %d\n", rc, str->idamerrorstack.nerrors);

    rc = rc && WrapXDRString(xdrs, (char*) str->msg, STRING_LENGTH);

    idamLog(LOG_DEBUG, "XDRLIB: Server #1 rc[4] = %d, strlen = %zd\n", rc, strlen(str->msg));
        idamLog(LOG_DEBUG, "XDRLIB: str->msg = %p\n", str->msg);
        idamLog(LOG_DEBUG, "XDRLIB: str->msg[0] = %d\n", str->msg[0]);
        idamLog(LOG_DEBUG, "XDRLIB: maxsize = %d\n", STRING_LENGTH);
    idamLog(LOG_DEBUG, "XDRLIB: Server #1 protocolVersion %d [rc = %d]\n", protocolVersion, rc);
...
bool_t xdr_server2(XDR* xdrs, SERVER_BLOCK* str)
{
    ...
    for (i = 0; i < str->idamerrorstack.nerrors; i++) {
        ...
        idamLog(LOG_DEBUG, "xdr_server2 [%d] %s\n", i, str->idamerrorstack.idamerror[i].msg);
    }

    idamLog(LOG_DEBUG, "XDRLIB: Server #2 rc = %d\n", rc);
