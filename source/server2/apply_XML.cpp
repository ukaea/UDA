...
int uda::server_parse_signal_XML(DATA_SOURCE data_source, SIGNAL signal, SIGNAL_DESC signal_desc,
        ...
        UDA_LOG(UDA_LOG_DEBUG, "#3 %d\n", (data_source.pass = -1 ||
                                                              ((actions_desc->action[i].pass_range[0] == -1 ||
                                                               ...
                                                               (actions_desc->action[i].pass_range[1] == -1 ||
                                                                (actions_desc->action[i].pass_range[1] > -1 &&
                                                                 actions_desc->action[i].pass_range[1] >=
                                                                 data_source.pass)))));
        ...
        switch (actions.action[i].actionType) {
            ...
            case UDA_TIME_OFFSET_TYPE:
                ...
                if ((actions.action[i].timeoffset.method == 1 || actions.action[i].timeoffset.method == 2) &&
                    ...
                    // Correct the Interval Only not the Starting value: Change to the standard simple data compression model
                    ...
                    // Correct both the Starting and Interval values: Change to the standard simple data compression model
                            ...
                            if (actions.action[i].calibration.dimensions[j].dimcalibration.factor != (double)1.0E0 ||
                                ...
                                if (STR_EQUALS("data", actions.action[i].calibration.target) ||
                                    ...
                                    if (data_block->dims[dimid].compressed) {
                                        ...
                                        applyCalibration(data_block->dims[dimid].data_type,
