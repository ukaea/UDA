    ...
    if (log_level <= UDA_LOG_ACCESS) {
        if (!log_dir.empty()) {
            ...
            if (system(cmd.c_str()) != 0) {
                add_error(_error_stack, ErrorType::Code, __func__, 999, "mkdir command failed");
...
void uda::server::Server::shutdown()
{
    ...
    print_errors(_error_stack, _client_block, _request_block);
    _error_stack.clear();
...
//        udaFreeMallocLogList(_log_malloc_list);
        ...
        for (const auto& error : _error_stack) {
            _server_block.error_stack.push_back(error);
        }
        ...
        print_errors(_server_block.error_stack, _client_block, _request_block);
...
int uda::server::Server::handle_request()
{
    ...
    err = _protocol.recv_client_block(&_client_block, &_fatal_error, _server_tot_block_time,
    ...
    if (!_server_block.error_stack.empty()) {
        _server_block.error = _server_block.error_stack[0].code;
        strcpy(_server_block.msg, _server_block.error_stack[0].msg);
    }

    // Test the client version is compatible with this server version

    if (protocol_version > ServerVersion) {
        UDA_THROW_ERROR(_error_stack, 999, "Protocol Error: Client API Version is Newer than the Server Version");
    }

    if (_fatal_error) {
        if (!_server_block.error_stack.empty()) {
            err = _server_block.error_stack[0].code;
        } else {
            err = 1;
        }
    ...
    for (size_t i = 0; i < _request_block.size(); ++i) {
        ...
        if (!proxy_target.empty() && strncasecmp(request->source, work, strlen(work)) != 0) {
            ...
            if (_client_block.version < 6) {
                UDA_THROW_ERROR(_error_stack, 999,
                    "PROXY redirection: Originating Client Version not compatible with the PROXY server interface.");
            }
            ...
            if (strlen(request->source) >=
                ...
                UDA_THROW_ERROR(_error_stack, 999, "PROXY redirection: The source argument string is too long!");
            }
    ...
    print_error_stack(_error_stack);
    UDA_LOG(UDA_LOG_DEBUG,
            "======================== ******************** ==========================================\n");
    ...
    if (_client_block.get_dimdble || _client_block.get_timedble || _client_block.get_scalar) {
        for (auto& data_block : _data_blocks) {
            if (server_processing(_client_block, &data_block) != 0) {
                UDA_THROW_ERROR(_error_stack, 779, "Server-Side Processing Error");
            }
        }
    }
...
void uda::server::Server::handshake_client()
{
    ...
    err = _protocol.recv_client_block(&_client_block, &_fatal_error, _server_tot_block_time, &_server_timeout,
        _log_malloc_list, _user_defined_type_list);
