...
static int initialiseKeys(CLIENT_BLOCK* client_block, gcry_sexp_t* publickey_out, gcry_sexp_t* privatekey_out)
{
    ...
    do { // Error Trap
        ...
        if ((err = importPEMPrivateKey(serverPrivateKeyFile, &privatekey)) != 0) {
            udaAddError(UDA_CODE_ERROR_TYPE, __func__, err, "Failed to load Server's Private Key File");
            break;
        }
...
#  ifndef TESTIDAMSECURITY
    if (!xdrrec_skiprecord(serverInput)) {
        ...
        udaAddError(UDA_CODE_ERROR_TYPE, __func__, UDA_PROTOCOL_ERROR_5, "Protocol 5 Error (Client Block #2)");
    } else {
        ...
        if ((err = protocol2(serverInput, protocol_id, XDR_RECEIVE, nullptr, logmalloclist, userdefinedtypelist,
                             client_block)) != 0) {
            udaAddError(UDA_CODE_ERROR_TYPE, __func__, err, "Protocol 10 Error (Client Block #2)");
...
#  ifndef TESTIDAMSECURITY
    ...
    if ((err = protocol2(serverOutput, protocol_id, XDR_SEND, nullptr, logmalloclist, userdefinedtypelist,
                         server_block)) != 0) {
        udaAddError(UDA_CODE_ERROR_TYPE, __func__, err, "Protocol 10 Error (securityBlock #4)");
    }

    if (!xdrrec_endofrecord(serverOutput, 1)) {
        udaAddError(UDA_CODE_ERROR_TYPE, __func__, UDA_PROTOCOL_ERROR_7, "Protocol 7 Error (Server Block)");
    }
#  endif
