...
int testFilePermissions(const char* object)
{
    ...
    if (rc != 0 || errno != 0) {
        err = 999;
        if (errno != 0) {
            add_error(ErrorType::System, "testSecurityFilePermissions", errno, "");
        }
        add_error(ErrorType::Code, "clientAuthentication", err,
    ...
    if (buffer.st_mode & S_IRGRP || buffer.st_mode & S_IWGRP || buffer.st_mode & S_IXGRP || buffer.st_mode & S_IROTH ||
        ...
        err = 999;
        add_error(ErrorType::Code, "testSecurityFilePermissions", err,
