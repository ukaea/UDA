...
// Usage:
//
//	result = queryCPF(names, table, where_clause [,/verbose][,/help][,/debug])
//
...
extern IDL_VPTR IDL_CDECL queryCPF(int argc, IDL_VPTR argv[], char* argk);

extern IDL_VPTR IDL_CDECL queryStrCPF(int argc, IDL_VPTR argv[], char* argk);

extern IDL_VPTR IDL_CDECL queryArrayCPF(int argc, IDL_VPTR argv[], char* argk);

extern IDL_VPTR IDL_CDECL querySingleArrayCPF(int argc, IDL_VPTR argv[], char* argk);

extern IDL_VPTR IDL_CDECL groupCountCPF(int argc, IDL_VPTR argv[], char* argk);

extern IDL_VPTR IDL_CDECL putCPF(int argc, IDL_VPTR argv[], char* argk);

extern IDL_VPTR IDL_CDECL killCPF(int argc, IDL_VPTR argv[], char* argk);

extern IDL_VPTR IDL_CDECL closeCPF(int argc, IDL_VPTR argv[], char* argk);

extern IDL_VPTR IDL_CDECL countCPF(int argc, IDL_VPTR argv[], char* argk);

static IDL_SYSFUN_DEF2 mastCPF_functions[] = {
        {{(IDL_FUN_RET) queryCPF},            "QUERYCPF",            3, 3, IDL_SYSFUN_DEF_F_KEYWORDS, 0},
        {{(IDL_FUN_RET) queryStrCPF},         "QUERYSTRCPF",         3, 3, IDL_SYSFUN_DEF_F_KEYWORDS, 0},
        {{(IDL_FUN_RET) queryArrayCPF},       "QUERYARRAYCPF",       3, 3, IDL_SYSFUN_DEF_F_KEYWORDS, 0},
        {{(IDL_FUN_RET) querySingleArrayCPF}, "QUERYSINGLEARRAYCPF", 3, 3, IDL_SYSFUN_DEF_F_KEYWORDS, 0},
        {{(IDL_FUN_RET) groupCountCPF},       "GROUPCOUNTCPF",       3, 3, IDL_SYSFUN_DEF_F_KEYWORDS, 0},
        {{(IDL_FUN_RET) putCPF},              "PUTCPF",              4, 4, IDL_SYSFUN_DEF_F_KEYWORDS, 0},
        {{(IDL_FUN_RET) killCPF},             "KILLCPF",             2, 2, IDL_SYSFUN_DEF_F_KEYWORDS, 0},
        {{(IDL_FUN_RET) countCPF},            "COUNTCPF",            3, 3, IDL_SYSFUN_DEF_F_KEYWORDS, 0},
        {{(IDL_FUN_RET) closeCPF},            "CLOSECPF",            0, 0, 0,                         0}
};

int mastCPF_startup(void) {

    if (!IDL_SysRtnAdd(mastCPF_functions, TRUE,
                       ARRLEN(mastCPF_functions))) { return IDL_FALSE; }

// Register the exit handler

    IDL_ExitRegister(mastCPF_exit_handler);
...
IDL_VPTR IDL_CDECL closeCPF(int argc, IDL_VPTR argv[], char* argk) {
//
// IDL DLM Function to Close the Socket Connection to the CPF Database Server
//
// No Arguments:
//
...
//-------------------------------------------------------------------------
    ...
    void* psDef = NULL;
    static IDL_LONG64 ilDims[IDL_MAX_ARRAY_DIM];

    static IDL_LONG64 vdlen[] = {2, 1, 1};        // Scalar Value Array
...
//-------------------------------------------------------------------------
    ...
    IDL_STRING* pisArray;
    IDL_LONG64 lDimensions[IDL_MAX_ARRAY_DIM];
    IDL_VPTR ivReturn;
    int iStringLength;
...
//-------------------------------------------------------------------------
    ...
    static IDL_LONG64 vdlen[] = {1, 1};        // Scalar Value Array
...
// Allocate the Appropriate Return Structure Array
    ...
    switch (ngrps) {
        ...
        case 1:
            sout1 = (CSOUT1*) malloc(nrows * sizeof(CSOUT1));
            if (sout1 == NULL) err = 1;
            break;

        case 2:
            sout2 = (CSOUT2*) malloc(nrows * sizeof(CSOUT2));
            if (sout2 == NULL) err = 1;
            break;

        case 3:
            sout3 = (CSOUT3*) malloc(nrows * sizeof(CSOUT3));
            if (sout3 == NULL) err = 1;
            break;
    }
    ...
    for (i = 0; i < nrows; i++) {
        ...
        switch (ngrps) {
            case 1:
                sout1[i].count = (int) atoi(PQgetvalue(DBQuery, i, ncols - 1));
                s1 = sout1 + i;
                IDL_StrStore(&(s1->group), "");
                break;

            case 2:
                sout2[i].count = (int) atoi(PQgetvalue(DBQuery, i, ncols - 1));
                s2 = sout2 + i;
            ...
            case 3:
                sout3[i].count = (int) atoi(PQgetvalue(DBQuery, i, ncols - 1));
                s3 = sout3 + i;
            ...
            if (j < 3 && j != (ncols - 1)) {
                if (j == 0) {
                    if (strlen(value) > 0) {
                        switch (ngrps) {
                            case 1:
                                IDL_StrStore(&(s1->group), value);
                                break;
                            case 2:
                                IDL_StrStore(&(s2->group[0]), value);
                                break;
                            case 3:
                                IDL_StrStore(&(s3->group[0]), value);
                                break;
                        }
                    }
                } else {
                    if (j == 1) {
                        if (strlen(value) > 0) {
                            switch (ngrps) {
                                case 2:
                                    IDL_StrStore(&(s2->group[1]), value);
                                    break;
                                case 3:
                                    IDL_StrStore(&(s3->group[1]), value);
                                    break;
                            }
                        }
                    } else {
...
// Create the IDL Structure
    ...
    switch (ngrps) {
        case 1:
            ivReturn = IDL_ImportArray(1, ilDims, IDL_TYP_STRUCT, (UCHAR*) sout1, freeMem, psDef);
            break;
        case 2:
            ivReturn = IDL_ImportArray(1, ilDims, IDL_TYP_STRUCT, (UCHAR*) sout2, freeMem, psDef);
            break;
        case 3:
            ivReturn = IDL_ImportArray(1, ilDims, IDL_TYP_STRUCT, (UCHAR*) sout3, freeMem, psDef);
            break;
    }
...
//	verbose - Print Error Messages
//-------------------------------------------------------------------------
    int i, j, k, kk, maxcount, maxnr, maxns, nrows, ncols, is2d = 0;
    int larray, test;
    ...
    IDL_STRING* pIDLstrDesc;
    char* table, * where, * value, * token;
    char tokvalue[13];

    char sql[MAXSQL] = "select ";
    char slimit[MAXSTR];

    SOUTARR* sout = NULL;            // Pointer to the Array Oriented Returned IDL/C Structure;
    SOUTARR2* sout2;
    ...
    static IDL_LONG64 vdlen[] = {3, 1, 1, 1};    // 1-D Array Value Array
    static IDL_LONG64 vdlen2[] = {4, 1, 1, 1, 1};    // 2-D Array Value Array
...
// Returned Values (Scalar and Array)
    ...
    if ((valArray = (double*) malloc(nrows * ncols * MAXARRAYNR * MAXARRAYNS * sizeof(double))) == NULL) {
        if (kw.verbose) {
            ...
            long q = nrows * ncols * MAXARRAYNR * MAXARRAYNS;
            fprintf(stdout, "n: %ld\n", q);
            fflush(NULL);
        }
        IDL_KW_FREE;
...
//	verbose - Print Error Messages
//-------------------------------------------------------------------------
    int i, irow, k, kk, maxcount, maxnr, maxns, nrows, is2d = 0;
    ...
    static IDL_LONG64 vdlen[] = {2, 1, 1};          // 1-D Array Value Array + 1
    static IDL_LONG64 vdlen2[] = {3, 1, 1, 1};      // 2-D Array Value Array + 1
    static IDL_LONG64 vdlen3[] = {1, 1};            // 1-D Array Value Array
            ...
            for (k = 0; k < maxcount; k++) {
                ...
                if (token == NULL) {            // End of the Array; Allocate correct Size
                    if (irow == 0) {
                        ...
                        if ((valArray = (double*) realloc((void*) valArray, nrows * maxcount * sizeof(double))) ==
                            NULL) {
                            if (kw.verbose)fprintf(stdout, "queryArraySQL: ERROR - Unable to Allocate Heap!\n");
                            IDL_KW_FREE;
                            PQclear(DBQuery);
...
// Allocate the Appropriate Return Structure
    ...
    if (is2d) {
        ...
        if ((sout2 = (SOUTSARR2*) malloc(
                sizeof(SOUTSARR2) + irow * maxnr * maxns * sizeof(double) + irow * sizeof(int))) == NULL) {
...
// Create the IDL Structure
        ...
        if ((sout = (SOUTSARR*) malloc(sizeof(SOUTSARR) + irow * maxcount * sizeof(double) + irow * sizeof(int))) ==
            NULL) {
            if (kw.verbose)fprintf(stdout, "querySingleArraySQL: ERROR - Unable to Allocate Heap!\n");
            IDL_KW_FREE;
            PQclear(DBQuery);
...
// Check a String was Passed
    ...
    IDL_ENSURE_STRING(argv[1]);
    //IDL_ENSURE_SCALAR(argv[1]);
    table = IDL_STRING_STR(&(argv[1]->value.str));       // Table Name

    IDL_ENSURE_STRING(argv[2]);        // Field Name
    //IDL_ENSURE_SCALAR(argv[2]);
    field = IDL_STRING_STR(&(argv[2]->value.str));    // Field Name

    IDL_ENSURE_STRING(argv[3]);        // Value as a String
    //IDL_ENSURE_SCALAR(argv[3]);
    value = IDL_STRING_STR(&(argv[3]->value.str));    // Value
...
// Security Check
        ...
        if (checkSecurity(DBConnect, table, 'N', kw.verbose) != 1) {
            if (kw.verbose) {
...
// Check a Record Does Not Already Exist
        ...
        if (nrows >= 1) {
            if (kw.verbose)
                fprintf(stdout, "putCPF: Existing CPF.%s record for pulse %s Exists!\n", table, s_exp_number);
            IDL_KW_FREE;
            PQclear(DBQuery);
...
// Security Check
        ...
        if (checkSecurity(DBConnect, table, 'U', kw.verbose) != 1) {
            if (kw.verbose) {
...
// Execute the Command and Open a Pipe to the Output for Reading
    ...
    if ((ph = popen("whoami", "r")) == NULL) {
        ...
        return;
    } else if (!feof(ph)) fgets(uid, MAXSTR - 1, ph);
...
// Allocate the Appropriate Return Structure Array
    ...
    switch (ngrps) {
        ...
        case 1:
            sout1 = (CSOUT1*) malloc(
                    nrows * sizeof(CSOUT1));  //IDL_GetScratch(&p,(IDL_MEMINT) nrows,(IDL_MEMINT)sizeof(CSOUT1));
            if (sout1 == NULL) err = 1;
            break;

        case 2:
            sout2 = (CSOUT2*) malloc(
                    nrows * sizeof(CSOUT2));  //IDL_GetScratch(&p,(IDL_MEMINT) nrows,(IDL_MEMINT)sizeof(CSOUT2));
            if (sout2 == NULL) err = 1;
            break;

        case 3:
            sout3 = (CSOUT3*) malloc(
                    nrows * sizeof(CSOUT3));  //IDL_GetScratch(&p,(IDL_MEMINT) nrows,(IDL_MEMINT)sizeof(CSOUT3));
            if (sout3 == NULL) err = 1;
            break;
    }
