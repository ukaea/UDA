...
//{(IDL_FUN_RET) listshots,        "LISTSHOTS",   	0,0,IDL_SYSFUN_DEF_F_KEYWORDS,0},
...
PGconn *startSQL(){
   ...
   /* fprintf(stdout, "ROIDAM startSQL\n"); */
...
// Connect to the Database Server
   ...
   /* fprintf(stdout, "SQL Connection: host %s\n", environment.sql_host); */
   /* fprintf(stdout, "SQL Connection: port %s\n", pgport); */
   /* fprintf(stdout, "SQL Connection: dbname %s\n", environment.sql_dbname); */
   /* fprintf(stdout, "SQL Connection: user %s\n", environment.sql_user); */
...
// Passed Arguments:  Signal (String) 
   ...
   if((ndims = (int)argv[1]->value.arr->n_dim) != 1){			// Number of Dimensions
      if(kw.verbose)fprintf(stdout,"Error: The Pulse Number Array is Multi-Dimensional. Please change Rank to 1.\n");
      IDL_KW_FREE;
      return(IDL_GettmpLong(1));     	 
   }
...
//
   ...
   if(meta_id > 0){
      ...
      if(nrows != 1){
         if(kw.verbose)fprintf(stdout,"No Record is present in the META Table with the Key: %d"
...
// Otherwise check for Standard pattern matching characters ? and *. If found then enable pattern matching.   
   ...
   if(kw.match){		// escape any SQL pattern matching characters: _ and %
      ...
      lstr = strlen(signal);
      if(signal[0] == '*' || signal[0] == '?' || (signal[lstr-2] != '\\' && (signal[lstr-1] == '*' || signal[lstr-1] == '?'))){
         sqlIdamPatternMatch(signal, sql_signal);
...
// Extract the Resultset 
   ...
   if(nrows > 1){
      if(kw.verbose)fprintf(stdout,"Too Many Signal Description Records were Identified when only one was expected ... Please refine your Specification\n");  
      IDL_KW_FREE;
      PQclear(DBQuery);
      return(IDL_GettmpLong(1));
   }  
...
// Take the LATEST (Highest Pass No.) if no pass number was specified. 
   ...
   for(i=0;i<nshots;i++){	// Loop over all Shots 
      ...
      if(kw.is_source){
	 if(kw.is_pass){
            sprintf(sql,"SELECT source_id FROM data_Source WHERE source_alias ilike '%s' and type = '%s' and exp_number = %d and pass = %d ", 
                         source, signal_type, shotlist[i], kw.pass);
         } else {
            sprintf(sql,"SELECT source_id, pass FROM data_Source WHERE source_alias ilike '%s' and type = '%s' and exp_number = %d "  
...
// Extract the Resultset 
      ...
      if(kw.is_pass && nrows > 1){
         if(kw.verbose){
	    fprintf(stdout,"Too Many Data Source Records were Identified when only one was expected ... Please check your Specification\n");  
            if(kw.is_source)
	       fprintf(stdout,"Signal %s, Source %s, Shot %d, Pass %d, Type %s\n", signal, source, shotlist[i], kw.pass, signal_type);  
	    else
...
// Update the Specific Signal Record
	 ...
	 if((rc = checkSignalAuthorisation(1, signal_desc_id, clientname, kw.verbose, stdout)) == 1){
	    if((rc = checkAdminAuthorisation(clientname, kw.verbose, stdout)) == 1){
	       if(kw.verbose)fprintf(stdout,"ERROR - No Change made to Single Signal Meta ID Field - No Authorisation\n");
               IDL_KW_FREE;
               PQclear(DBQuery);
               return(IDL_GettmpLong(1));
	    }	  
...
//-------------------------------------------------------------------------	      
   ...
   int i, j, ndims=0, nitems=0, rc, nrows, ncols, lstr;
   int isComposite, isNew, isXML;
   int dim[IDL_MAX_ARRAY_DIM];   

   double *derr_dim_params;
   float  *err_dim_params;
            
   char sql_signal[MAXNAME], sql[MAXSQL], returnsql[MAXSQL], xml[MAXMETA];
 
   char *signal;
   char *doc_label, *doc_units, *doc_desc, *doc_dim_label, *doc_dim_units;
   char *cal_target, *cal_units;
   char *comp_new_alias, *comp_signal, *comp_error, *comp_aserror;
   char *signal_xml;
   
   const char xml_default[] = "<?xml version=\"1.0\"?><action><signal></signal></action>";
   int lxml_default = strlen(xml_default)+12;	// Empty tags with LFs and CRs  
...
// Array Keyword Descriptors 
   ...
   static IDL_KW_ARR_DESC_R doc_dim_label_Desc  = {IDL_KW_OFFSETOF(doc_dim_label), 1, 8,  IDL_KW_OFFSETOF(ndoc_dim_label)};   
   static IDL_KW_ARR_DESC_R doc_dim_units_Desc  = {IDL_KW_OFFSETOF(doc_dim_units), 1, 8,  IDL_KW_OFFSETOF(ndoc_dim_units)};   
   
   static IDL_KW_ARR_DESC_R cal_dim_id_Desc     = {IDL_KW_OFFSETOF(cal_dim_id),    1, 32, IDL_KW_OFFSETOF(ncal_dim_id)};   
   static IDL_KW_ARR_DESC_R cal_dim_factor_Desc = {IDL_KW_OFFSETOF(cal_dim_factor),1, 32, IDL_KW_OFFSETOF(ncal_dim_factor)};   
   static IDL_KW_ARR_DESC_R cal_dim_offset_Desc = {IDL_KW_OFFSETOF(cal_dim_offset),1, 32, IDL_KW_OFFSETOF(ncal_dim_offset)};   
   static IDL_KW_ARR_DESC_R cal_dim_target_Desc = {IDL_KW_OFFSETOF(cal_dim_target),1, 32, IDL_KW_OFFSETOF(ncal_dim_target)};   
   static IDL_KW_ARR_DESC_R cal_dim_units_Desc  = {IDL_KW_OFFSETOF(cal_dim_units), 1, 32, IDL_KW_OFFSETOF(ncal_dim_units)};   
   
   static IDL_KW_ARR_DESC_R err_params_Desc     = {IDL_KW_OFFSETOF(err_params),    1, MAXERRPARAMS,  IDL_KW_OFFSETOF(nerr_params)};   
   static IDL_KW_ARR_DESC_R err_dim_id_Desc     = {IDL_KW_OFFSETOF(err_dim_id),    1, 8,  IDL_KW_OFFSETOF(nerr_dim_id)};   
   static IDL_KW_ARR_DESC_R err_dim_model_Desc  = {IDL_KW_OFFSETOF(err_dim_model), 1, 8,  IDL_KW_OFFSETOF(nerr_dim_model)}; 
   
   static IDL_KW_ARR_DESC_R comp_dim_id_Desc      = {IDL_KW_OFFSETOF(comp_dim_id),     1, 8,  IDL_KW_OFFSETOF(ncomp_dim_id)};   
   static IDL_KW_ARR_DESC_R comp_dim_signal_Desc  = {IDL_KW_OFFSETOF(comp_dim_signal), 1, 8,  IDL_KW_OFFSETOF(ncomp_dim_signal)};   
   static IDL_KW_ARR_DESC_R comp_dim_error_Desc   = {IDL_KW_OFFSETOF(comp_dim_error),  1, 8,  IDL_KW_OFFSETOF(ncomp_dim_error)};   
   static IDL_KW_ARR_DESC_R comp_dim_aserror_Desc = {IDL_KW_OFFSETOF(comp_dim_aserror),1, 8,  IDL_KW_OFFSETOF(ncomp_dim_aserror)};   
   static IDL_KW_ARR_DESC_R comp_dim_from_Desc    = {IDL_KW_OFFSETOF(comp_dim_from),   1, 8,  IDL_KW_OFFSETOF(ncomp_dim_from)};   
...
// Maintain Alphabetical Order of Keywords
   ...
   { IDL_KW_FAST_SCAN,
     ...
     {"CAL_DIM_FACTOR",  IDL_TYP_DOUBLE,  1,IDL_KW_ARRAY,IDL_KW_OFFSETOF(is_cal_dim_factor),    IDL_CHARA(cal_dim_factor_Desc)},
     {"CAL_DIM_ID",      IDL_TYP_LONG,    1,IDL_KW_ARRAY,IDL_KW_OFFSETOF(is_cal_dim_id),	IDL_CHARA(cal_dim_id_Desc)},
     {"CAL_DIM_OFFSET",  IDL_TYP_DOUBLE,  1,IDL_KW_ARRAY,IDL_KW_OFFSETOF(is_cal_dim_offset),    IDL_CHARA(cal_dim_offset_Desc)},
     {"CAL_DIM_TARGET",  IDL_TYP_STRING,  1,IDL_KW_ARRAY,IDL_KW_OFFSETOF(is_cal_dim_target),    IDL_CHARA(cal_dim_target_Desc)},
     {"CAL_DIM_UNITS",   IDL_TYP_STRING,  1,IDL_KW_ARRAY,IDL_KW_OFFSETOF(is_cal_dim_units),     IDL_CHARA(cal_dim_units_Desc)},
     {"CAL_FACTOR",      IDL_TYP_DOUBLE,  1,IDL_KW_ZERO,IDL_KW_OFFSETOF(is_cal_factor),		IDL_KW_OFFSETOF(cal_factor)},
     {"CAL_OFFSET",      IDL_TYP_DOUBLE,  1,IDL_KW_ZERO,IDL_KW_OFFSETOF(is_cal_offset),		IDL_KW_OFFSETOF(cal_offset)},
     {"CAL_TARGET",      IDL_TYP_STRING,  1,IDL_KW_ZERO,IDL_KW_OFFSETOF(is_cal_target),		IDL_KW_OFFSETOF(cal_target)},
     {"CAL_UNITS",       IDL_TYP_STRING,  1,IDL_KW_ZERO,IDL_KW_OFFSETOF(is_cal_units),		IDL_KW_OFFSETOF(cal_units)},
     {"COMMIT",          IDL_TYP_LONG,    1,IDL_KW_ZERO,0,					IDL_KW_OFFSETOF(commit)},
     {"COMP_ASERROR",    IDL_TYP_STRING,  1,IDL_KW_ZERO,IDL_KW_OFFSETOF(is_comp_aserror),	IDL_KW_OFFSETOF(comp_aserror)},
     {"COMP_DIM_ASERROR",IDL_TYP_STRING,  1,IDL_KW_ARRAY,IDL_KW_OFFSETOF(is_comp_dim_aserror),	IDL_CHARA(comp_dim_aserror_Desc)},
     {"COMP_DIM_ERROR",  IDL_TYP_STRING,  1,IDL_KW_ARRAY,IDL_KW_OFFSETOF(is_comp_dim_error),	IDL_CHARA(comp_dim_error_Desc)},
     {"COMP_DIM_FROM",   IDL_TYP_LONG,    1,IDL_KW_ARRAY,IDL_KW_OFFSETOF(is_comp_dim_from),	IDL_CHARA(comp_dim_from_Desc)},
     {"COMP_DIM_ID",     IDL_TYP_LONG,    1,IDL_KW_ARRAY,IDL_KW_OFFSETOF(is_comp_dim_id),	IDL_CHARA(comp_dim_id_Desc)},
     {"COMP_DIM_SIGNAL", IDL_TYP_STRING,  1,IDL_KW_ARRAY,IDL_KW_OFFSETOF(is_comp_dim_signal),	IDL_CHARA(comp_dim_signal_Desc)},
     {"COMP_DIM_TO",     IDL_TYP_LONG,    1,IDL_KW_ARRAY,IDL_KW_OFFSETOF(is_comp_dim_to),	IDL_CHARA(comp_dim_to_Desc)},
     {"COMP_ERROR",      IDL_TYP_STRING,  1,IDL_KW_ZERO,IDL_KW_OFFSETOF(is_comp_error),		IDL_KW_OFFSETOF(comp_error)},
     {"COMP_NEW_ALIAS",  IDL_TYP_STRING,  1,IDL_KW_ZERO,IDL_KW_OFFSETOF(is_comp_new_alias),	IDL_KW_OFFSETOF(comp_new_alias)},
     {"COMP_SIGNAL",     IDL_TYP_STRING,  1,IDL_KW_ZERO,IDL_KW_OFFSETOF(is_comp_signal), 	IDL_KW_OFFSETOF(comp_signal)},          
     {"DEBUG",           IDL_TYP_LONG,    1,IDL_KW_ZERO,0,					IDL_KW_OFFSETOF(debug)},
     {"DELETE",          IDL_TYP_LONG,    1,IDL_KW_ZERO,0,					IDL_KW_OFFSETOF(delete)},
     ...
     {"DOC_DIM_LABEL",   IDL_TYP_STRING,  1,IDL_KW_ARRAY,IDL_KW_OFFSETOF(is_doc_dim_label),     IDL_CHARA(doc_dim_label_Desc)},          
     {"DOC_DIM_UNITS",   IDL_TYP_STRING,  1,IDL_KW_ARRAY,IDL_KW_OFFSETOF(is_doc_dim_units),     IDL_CHARA(doc_dim_units_Desc)},          
     ...
     {"ERR_DIM_MODEL",   IDL_TYP_LONG,    1,IDL_KW_ARRAY,IDL_KW_OFFSETOF(is_err_dim_model),	IDL_CHARA(err_dim_model_Desc)},
     {"ERR_DIM_PARAMS",  IDL_TYP_FLOAT,   1,IDL_KW_VIN,IDL_KW_OFFSETOF(is_err_dim_params),	IDL_KW_OFFSETOF(err_dim_params)},
     ...
     {"TIME_INTERVAL",   IDL_TYP_DOUBLE,  1,IDL_KW_ZERO,IDL_KW_OFFSETOF(is_time_interval),	IDL_KW_OFFSETOF(time_interval)},
     {"TIME_OFFSET",     IDL_TYP_DOUBLE,  1,IDL_KW_ZERO,IDL_KW_OFFSETOF(is_time_offset),	IDL_KW_OFFSETOF(time_offset)},
     {"TIME_START",      IDL_TYP_DOUBLE,  1,IDL_KW_ZERO,IDL_KW_OFFSETOF(is_time_start),	        IDL_KW_OFFSETOF(time_start)},
     {"VERBOSE",         IDL_TYP_LONG,    1,IDL_KW_ZERO,0,					IDL_KW_OFFSETOF(verbose)},
...
// Process String Keywords  
   ...
   if(kw.is_doc_label) doc_label  = IDL_STRING_STR(&kw.doc_label);
   if(kw.is_doc_units) doc_units  = IDL_STRING_STR(&kw.doc_units);
   if(kw.is_doc_desc)  doc_desc   = IDL_STRING_STR(&kw.doc_desc);
   if(kw.is_cal_target)cal_target = IDL_STRING_STR(&kw.cal_target);
   if(kw.is_cal_units) cal_units  = IDL_STRING_STR(&kw.cal_units);
   if(kw.is_comp_new_alias) comp_new_alias = IDL_STRING_STR(&kw.comp_new_alias);
   if(kw.is_comp_signal)    comp_signal    = IDL_STRING_STR(&kw.comp_signal);
   if(kw.is_comp_error)     comp_error     = IDL_STRING_STR(&kw.comp_error);
   if(kw.is_comp_aserror)   comp_aserror   = IDL_STRING_STR(&kw.comp_aserror);
...
// Process Keyword Arrays  
   ...
   if(kw.is_err_dim_params){
	 ...
	 nitems = (int)kw.err_dim_params->value.arr->n_elts;				// Number of Elements
	 if(kw.err_dim_params->type == IDL_TYP_FLOAT){         
	    if(kw.debug)fprintf(stdout,"Array is of Type: FLOAT\n");
	    err_dim_params  = (float *)kw.err_dim_params->value.arr->data;		// Float Array
	    derr_dim_params = NULL;
	 } else {
...
/*   
   if(kw.debug){
      fprintf(stdout,"Build Date: %s\n", __DATE__);     
      if(kw.is_xml)           fprintf(stdout,"XML Passed: %s\n", IDL_STRING_STR(&kw.xml));
      if(kw.alias )           fprintf(stdout,"Alias Keyword Passed\n");
      if(kw.match)            fprintf(stdout,"Match Keyword Passed\n");
      if(kw.is_time_offset)   fprintf(stdout,"Time Offset Value Passed: %f\n", kw.time_offset);
      if(kw.is_time_start)    fprintf(stdout,"Time Start Value Passed: %f\n", kw.time_start);
      if(kw.is_time_interval) fprintf(stdout,"Time Interval Value Passed: %f\n", kw.time_interval);
      if(kw.is_passrange){
         fprintf(stdout,"No. Pass Range Values Passed: %d\n", (int)kw.npassrange);
	 for(i=0;i<kw.npassrange;i++)
	    fprintf(stdout,"PassRange[%d] = %d\n", i, kw.passrange[i]);
      }
      if(kw.is_shotrange){
         fprintf(stdout,"No. Shot Range Values Passed: %d\n", (int)kw.nshotrange);
	 for(i=0;i<kw.nshotrange;i++)
	    fprintf(stdout,"ShotRange[%d] = %d\n", i, kw.shotrange[i]);
      }	
      
      if(kw.is_meta_id) fprintf(stdout,"Meta ID of Existing XML Passed: %d\n", (int)kw.meta_id);
	
      if(kw.is_doc_label) fprintf(stdout,"Data Label      : %s\n", doc_label);
      if(kw.is_doc_units) fprintf(stdout,"Data Units      : %s\n", doc_units);
      if(kw.is_doc_desc)  fprintf(stdout,"Data Description: %s\n", doc_desc);
      
      if(kw.is_doc_dim_id){
         fprintf(stdout,"No. Document Dimension Id Values Passed: %d\n", (int)kw.ndoc_dim_id);
	 for(i=0;i<kw.ndoc_dim_id;i++)
	    fprintf(stdout,"Dim_id[%d] = %d\n", i, kw.doc_dim_id[i]);
      }
      
      if(kw.is_doc_dim_label){
         for(i=0;i<kw.ndoc_dim_label;i++){
            doc_dim_label = IDL_STRING_STR(&kw.doc_dim_label[i]);
	    fprintf(stdout,"Dim[%d] Label      : %s\n", i, doc_dim_label);
	 }   
      }
      
      if(kw.is_doc_dim_units){
         for(i=0;i<kw.ndoc_dim_units;i++){
            doc_dim_units = IDL_STRING_STR(&kw.doc_dim_units[i]);
	    fprintf(stdout,"Dim[%d] Units      : %s\n", i, doc_dim_units);
	 }   
      }
      
      if(kw.is_cal_factor) fprintf(stdout,"Calibration Factor Value Passed: %f\n", kw.cal_factor);		
      if(kw.is_cal_offset) fprintf(stdout,"Calibration Offset Value Passed: %f\n", kw.cal_offset);
      if(kw.is_cal_target) fprintf(stdout,"Calibration Target Value Passed: %s\n", cal_target);
      if(kw.is_cal_units ) fprintf(stdout,"Calibration Units  Value Passed: %s\n", cal_units);
      if(kw.is_cal_dim_id){
         fprintf(stdout,"No. Calibration Dimension Id Values Passed: %d\n", (int)kw.ncal_dim_id);
	 for(i=0;i<kw.ncal_dim_id;i++) fprintf(stdout,"Dim_id[%d] = %d\n", i, kw.cal_dim_id[i]);
	 if(kw.is_cal_dim_factor)for(i=0;i<kw.ncal_dim_factor;i++) fprintf(stdout,"Dim_factor[%d] = %f\n", i, kw.cal_dim_factor[i]); 
	 if(kw.is_cal_dim_offset)for(i=0;i<kw.ncal_dim_offset;i++) fprintf(stdout,"Dim_offset[%d] = %f\n", i, kw.cal_dim_offset[i]); 
	 if(kw.is_cal_dim_target)for(i=0;i<kw.ncal_dim_target;i++) fprintf(stdout,"Dim_target[%d] = %s\n", i, IDL_STRING_STR(&kw.cal_dim_target[i])); 
	 if(kw.is_cal_dim_units)for(i=0;i<kw.ncal_dim_units;i++)   fprintf(stdout,"Dim_units[%d]  = %s\n", i, IDL_STRING_STR(&kw.cal_dim_units[i])); 
      }
      
      if(kw.is_err_model)  fprintf(stdout,"Error Model Id Value Passed: %d\n", (int)kw.err_model);
      if(kw.is_err_params) for(i=0;i<kw.nerr_params;i++) fprintf(stdout,"Param[%d] = %f\n", i, kw.err_params[i]); 		
      if(kw.is_err_dim_id){
         int offset = 0;
         fprintf(stdout,"No. Error Model Dimension Id Values Passed: %d\n", (int)kw.nerr_dim_id);
	 for(i=0;i<kw.nerr_dim_id;i++) fprintf(stdout,"Dim_id[%d] = %d\n", i, kw.err_dim_id[i]);
	 if(kw.is_err_dim_model)for(i=0;i<kw.nerr_dim_model;i++)  fprintf(stdout,"Model[%d] = %d\n", i, kw.err_dim_model[i]); 	 
	 fprintf(stdout,"No. Parameter Dimensions = %d\n", (int)kw.err_dim_params->value.arr->n_dim);
	 fprintf(stdout,"No Array Items = %d\n", (int)kw.err_dim_params->value.arr->n_elts);
	 
	 //for(i=0;i<(int)kw.err_dim_params->value.arr->n_dim;i++){
	 //  fprintf(stdout,"Dimensions # %d\n", i);
	 //   if(err_dim_params != NULL)
	//	for(j=0;j<(int)kw.err_dim_params->value.arr->n_elts;j++)
	//	   fprintf(stdout,"Param[%d] = %f\n", j, *((float *)kw.err_dim_params->value.arr->data + offset++));
	 //   else 	
	 //       for(j=0;j<(int)kw.err_dim_params->value.arr->n_elts;j++)
	//	   fprintf(stdout,"Param[%d] = %f\n", j, *((double *)kw.err_dim_params->value.arr->data + offset++));
	 //}     
      }
      
      if(kw.is_comp_new_alias)  fprintf(stdout,"New Composite Signal: %s\n", comp_new_alias);
      if(kw.is_comp_signal)     fprintf(stdout,"Composite Data Signal: %s\n", comp_signal); 
      if(kw.is_comp_error)      fprintf(stdout,"Composite Error Signal: %s\n", comp_error);
      if(kw.is_comp_aserror)    fprintf(stdout,"Composite Asymmetric Error Signal: %s\n", comp_aserror);  
      if(kw.is_comp_dim_id){
         fprintf(stdout,"No. Composite Dimension Id Values Passed: %d\n", (int)kw.ncomp_dim_id);
	 for(i=0;i<kw.ncomp_dim_id;i++) fprintf(stdout,"Dim_id[%d] = %d\n", i, kw.comp_dim_id[i]);
	 if(kw.is_comp_dim_signal)for(i=0;i<kw.ncomp_dim_signal;i++) fprintf(stdout,"Dim_signal[%d] = %s\n", i, IDL_STRING_STR(&kw.comp_dim_signal[i]));
	 if(kw.is_comp_dim_error)for(i=0;i<kw.ncomp_dim_error;i++) fprintf(stdout,"Dim_error[%d] = %s\n", i, IDL_STRING_STR(&kw.comp_dim_error[i]));
	 if(kw.is_comp_dim_aserror)for(i=0;i<kw.ncomp_dim_aserror;i++) fprintf(stdout,"Dim_aserror[%d] = %s\n", i, IDL_STRING_STR(&kw.comp_dim_aserror[i]));
	 if(kw.is_comp_dim_to)for(i=0;i<kw.ncomp_dim_to;i++) fprintf(stdout,"Dim_to[%d] = %d\n", i, kw.comp_dim_to[i]); 
	 if(kw.is_comp_dim_from)for(i=0;i<kw.ncomp_dim_from;i++) fprintf(stdout,"Dim_from[%d] = %d\n", i, kw.comp_dim_from[i]); 
      }
...
// Error Model: 
   ...
   if(kw.is_err_dim_id){
      ...
      ndims  = (int)kw.err_dim_params->value.arr->n_dim;			// Number of Dimensions
      nitems = (int)kw.err_dim_params->value.arr->n_elts;
      //for(i=0;i<ndims;i++) dim[i] = (int)kw.err_dim_params->value.arr->dim[i];	// Dimension Lengths
      if(!(ndims == 2 && dim[1] <= kw.nerr_dim_id)){
         if(kw.verbose) fprintf(stdout,"Dimensional Error Model Parameter Array is Not 2D or have Inconsistent Lengths!\n");
         IDL_KW_FREE;
         return(IDL_GettmpLong(1));			 
      }
   }
...
// Extract the Resultset 
   ...
   if(kw.is_comp_new_alias && nrows > 0){
      if(kw.verbose)fprintf(stdout,"The New Composite Signal Specified Already Exists ... Please change your Specification\n");
      IDL_KW_FREE;
      PQclear(DBQuery);
      return(IDL_GettmpLong(1));
   }
...
// Are All Records Composite Records?
   ...
   isComposite = kw.is_comp_new_alias;
   if(!kw.is_comp_new_alias && (kw.is_comp_signal || kw.is_comp_error || kw.is_comp_aserror)){
      isComposite = 1;
      for(i=0;i<nrows;i++){
         if(strcasecmp(PQgetvalue(DBQuery,i,4),"C") != 0) {
	    if(kw.verbose)fprintf(stdout,"ERROR - Not all Records Identified are Composite Signal Records ... Please refine your Specification\n");
            IDL_KW_FREE;
            PQclear(DBQuery);
            return(IDL_GettmpLong(1));
	 }
...
// Add a new Signal Desc Record 
   ...
   if(kw.is_comp_new_alias){		// Only 1 New record to be created
      ...
      if(strlen(xml) > 0){      
         ...
         strcat(sql,xml);
         strcat(sql,"'); INSERT INTO Signal_Desc (meta_id,signal_alias,signal_name,generic_name,type,source_alias,signal_owner) VALUES(");
         strcat(sql,"currval('meta_id_seq'),upper('");
      } else {
         strcpy(sql, "BEGIN; INSERT INTO Signal_Desc (signal_alias,signal_name,generic_name,type,source_alias,signal_owner) VALUES(upper('");
      }	 
      strcat(sql,signal);
      strcat(sql,"'),upper('");
      strcat(sql,signal);
...
// Create Data Structures from new XML
	 ...
	 if(kw.debug){
	    ...
	    fprintf(stdout, "length xml[%d]   = %d (%d)\n",i, strlen(xml), lxml_default);
	 }
	 ...
	 if(!isNew && !isXML && !kw.delete) continue;		// No New XML and No Delete of Original XML Requested
	    ...
	    initActions(&newactions);		  
            parseDoc(signal_xml, &actions);		// Create an Array of action blocks from the current Signal_desc record	    
...
// Update or Write a New XML Document
	    ...
	    if(isXML){
		  ...
		  strcpy(sql, "; UPDATE Signal_Desc SET modified = current_date WHERE signal_desc_id = ");
	          ...
	          if(kw.debug) fprintf(stdout, "Do Nothing ...\n"); 
	    }     
         }
	 ...
	 if(kw.commit && strlen(sql)>0){
	    if((rc = checkSignalAuthorisation(1, PQgetvalue(DBQuery,i,0), clientname, kw.verbose, stdout)) == 1){
	       if((rc = checkAdminAuthorisation(clientname, kw.verbose, stdout)) == 1){
...
// Documentation
   ...
   if(kw.is_doc_label || kw.is_doc_units || kw.is_doc_desc || kw.is_doc_dim_id){
      sprintf(xml_str, "    <documentation id=\"1\" exp_number_start=\"%d\" exp_number_end=\"%d\" pass_start=\"%d\" pass_end=\"%d\">", 
   ...
   if(kw.is_time_offset || kw.is_time_start || kw.is_time_interval){
      if(kw.is_time_offset){
         ...
         sprintf(xml_str, "    <time_offset id=\"1\" method=\"1\" start=\"%f\" interval=\"%f\" "
	         "exp_number_start=\"%d\" exp_number_end=\"%d\" pass_start=\"%d\" pass_end=\"%d\" />", 
                 kw.time_start, kw.time_interval, kw.shotrange[0], kw.shotrange[1], kw.passrange[0], kw.passrange[1]); 
...
// Calibration
   ...
   if(kw.is_cal_factor || kw.is_cal_offset || kw.is_cal_units || kw.is_cal_dim_id){
      if(kw.is_cal_target){
         sprintf(xml_str, "    <calibration id=\"1\" target=\"%s\" exp_number_start=\"%d\" exp_number_end=\"%d\" pass_start=\"%d\" pass_end=\"%d\">", 
                 IDL_STRING_STR(&kw.cal_target), kw.shotrange[0], kw.shotrange[1], kw.passrange[0], kw.passrange[1]); 
      } else {
         sprintf(xml_str, "    <calibration id=\"1\" exp_number_start=\"%d\" exp_number_end=\"%d\" pass_start=\"%d\" pass_end=\"%d\">", 
                 kw.shotrange[0], kw.shotrange[1], kw.passrange[0], kw.passrange[1]); 
      }		 
      strcat(xml, xml_str);
      strcat(xml, lfcr);
      ...
      if(kw.is_cal_dim_id && (kw.is_cal_dim_factor || kw.is_cal_dim_offset || kw.is_cal_dim_units)){
	    ...
	    if(kw.is_cal_dim_target && i < kw.ncal_dim_target){
	       if(strlen(IDL_STRING_STR(&kw.cal_dim_target[i]))>0)
	          sprintf(xml_str,"      <dimension dimid=\"%d\" target=\"%s\">",kw.cal_dim_id[i], IDL_STRING_STR(&kw.cal_dim_target[i]));
	       else
...
// Error Model
   ...
   if(kw.is_err_model || kw.is_err_dim_id){
      if(kw.is_err_model){
         sprintf(xml_str, "    <errormodel id=\"1\" model=\"%d\" exp_number_start=\"%d\" exp_number_end=\"%d\" pass_start=\"%d\" pass_end=\"%d\">", 
                 (int)kw.err_model, kw.shotrange[0], kw.shotrange[1], (int)kw.passrange[0], (int)kw.passrange[1]);
      } else {
         sprintf(xml_str, "    <errormodel id=\"1\" exp_number_start=\"%d\" exp_number_end=\"%d\" pass_start=\"%d\" pass_end=\"%d\">", 
...
// dim[1] == Number of Parameter Arrays
	 ...
	 if(ndims == 2 && dim[1] <= kw.nerr_dim_id){   
            for(i=0;i<dim[1];i++){		
	       sprintf(xml_str,"      <dimension dimid=\"%d\" model=\"%d\">",kw.err_dim_id[i], kw.err_dim_model[i]);
	       strcat(xml, xml_str);
	       strcat(xml, lfcr);
	       strcat(xml, "        <params>"); 	    
	       if(kw.err_dim_params->type == IDL_TYP_FLOAT){         
...
// Maintain Alphabetical Order of Keywords
   ...
   { IDL_KW_FAST_SCAN,
     {"COMMIT",          IDL_TYP_LONG,    1,IDL_KW_ZERO,0,					IDL_KW_OFFSETOF(commit)},
     {"DEBUG",           IDL_TYP_LONG,    1,IDL_KW_ZERO,0,					IDL_KW_OFFSETOF(debug)},
     {"DESCRIPTION",     IDL_TYP_STRING,  1,IDL_KW_ZERO,IDL_KW_OFFSETOF(is_description),	IDL_KW_OFFSETOF(description)},
   ...
   if(argv[0]->type == IDL_TYP_STRING){
      ...
      if(argv[1]->type == IDL_TYP_LONG || argv[1]->type == IDL_TYP_INT){	// Exp_number & Range
	    ...
	    if(kw.range && nitems != 2){
               if(kw.verbose) fprintf(stdout,"The List Array is a Range but the Element Count is Incorrect: Check Argument #2\n");
               IDL_KW_FREE;
               return(IDL_GettmpLong(1));
            } 	    
	    if((list = malloc(nitems*sizeof(int))) == NULL){
	       if(kw.verbose) fprintf(stdout,"System Problem - Unable to aquire Heap Memory - Check Resources and Try Again!\n");
               IDL_KW_FREE;
               return(IDL_GettmpLong(1));
	    }	    
	    ...
	    if(kw.debug){
         ...
         if(kw.verbose) fprintf(stdout,"Data Source Alias Name Not Followed by an Integer Experiment Number or List Array - Check Argument #2\n");
	 IDL_KW_FREE;
	 return(IDL_GettmpLong(1));
      }	 
   } else {
...
// Passed Arguments: Integer (Scalar or Array) followed by a String
	 ...
	 if(argv[0]->type == IDL_TYP_LONG || argv[0]->type == IDL_TYP_INT){	// Exp_number & Range
	       ...
	       if(ndims != 1){
                  if(kw.verbose) fprintf(stdout,"The List Array should have dimensionality 1: Check Argument #1\n");
                  IDL_KW_FREE;
                  return(IDL_GettmpLong(1));
               } 
	       if(kw.range && nitems != 2){
	       ...
	       if((list = malloc(nitems*sizeof(int))) == NULL){
	          if(kw.verbose) fprintf(stdout,"System Problem - Unable to acquire Heap Memory - Check Resources and Try Again!\n");
                  IDL_KW_FREE;
                  return(IDL_GettmpLong(1));
	       }	    
	       ...
	       if(kw.debug){
            ...
            if(kw.verbose) fprintf(stdout,"Data Source Alias Name Not Followed by an Integer Experiment Number or List - Check Argument #1\n");
	    IDL_KW_FREE;
	    return(IDL_GettmpLong(1));
         }	 
      } 
...
// Debug Dump of Keywords  
   ...
   if(kw.debug){
      ...
      if(kw.is_description) fprintf(stdout,"Status Change Description Passed: %s\n", IDL_STRING_STR(&kw.description));
...
// Valid Status Value Range
   ...
   if(kw.is_status && (kw.status < MINSTATUS || kw.status > MAXSTATUS)){
      if(kw.verbose) fprintf(stdout,"Status Values are Invalid. They must be in the Range: %d - %d \n",MINSTATUS,MAXSTATUS);
      free((void *)list);
      IDL_KW_FREE;
      return(IDL_GettmpLong(1));			 
   }	
...
// Reason/Impact Description
   ...
   if( kw.is_description && strlen(IDL_STRING_STR(&kw.description)) == 0 && (!kw.is_reason || (kw.is_reason && (int)kw.reason != 0))){
...
// Otherwise check for Standard pattern matching characters ? and *. If found then enable pattern matching.   
   ...
   if(kw.match){		// escape any SQL pattern matching characters: _ and %
      ...
      lstr = strlen(source);
      if(source[0] == '*' || source[0] == '?' || (source[lstr-2] != '\\' && (source[lstr-1] == '*' || source[lstr-1] == '?'))){
         sqlIdamPatternMatch(source, sql_source);
	 kw.match = 1;					// Flag Pattern Matching to be used 
      } else {    
...
// Check there is NO Ambiguity in the Source_Alias
   ...
   if(kw.latest || (!kw.is_passrange && kw.is_pass && (int)kw.pass == -1)){
      strcpy(sql, "SELECT A.source_alias, count(*) as count FROM (SELECT source_alias, max(pass) as mpass FROM data_source WHERE ");
      strcat(sql, whr);
   ...
   if((nrows = PQntuples(DBQuery)) > 1){
      if(kw.verbose) fprintf(stdout,"The Data Source File Alias Name (%s) is Ambiguous ... Please Clarify\n", source);
      PQclear(DBQuery);
      IDL_KW_FREE;
      return(IDL_GettmpLong(1)); 
   }
   ...
   if((nrows = PQntuples(DBQuery)) == 0){
      if(kw.verbose) fprintf(stdout,"No Data Source Files with the Alias Name (%s) were Found ... Please Clarify\n", source);
      PQclear(DBQuery);
      IDL_KW_FREE;
      return(IDL_GettmpLong(1)); 
   }
   
   if(kw.debug)fprintf(stdout,"Number of Source Files with Alias Name (%s) Selected: %d\n", PQgetvalue(DBQuery,0,0), nrows);
...
// Validate Reason & Impact Codes
   ...
   if(kw.is_reason){
      ...
      if((rc = countIdamTable(sql_str, kw.verbose, stdout)) == 0){
         if(kw.verbose) fprintf(stdout,"The Status Change Reason Code is Invalid (%d) ... Please Correct\n", (int)kw.reason);
         PQclear(DBQuery);
         IDL_KW_FREE;
         return(IDL_GettmpLong(1));
      }	  
   ...
   if(kw.is_impact){
      ...
      if((rc = countIdamTable(sql_str, kw.verbose, stdout)) == 0){
         if(kw.verbose) fprintf(stdout,"The Status Change Impact Code is Invalid (%d) ... Please Correct\n", (int)kw.impact);
	 if(kw.debug) fprintf(stdout,"%s\n", sql_str);
         PQclear(DBQuery);
         IDL_KW_FREE;
         return(IDL_GettmpLong(1));
...
// Check User Authorisation (All Records should have the same Source_Alias name)
   ...
   if(kw.commit && (rc = checkSourceAuthorisation(PQgetvalue(DBQuery,0,0), clientname, kw.verbose, stdout)) == 1){
      if(kw.verbose)fprintf(stdout,"ERROR - You are Not Authorised to Update Status Values of Source Files %s\n", PQgetvalue(DBQuery,0,0) );
      kw.commit = 0;
   }     
...
// Status Change Description
	    ...
	    if(strlen(PQgetvalue(DBQuery,i,8)) == 0){	// No Previous Description? - Create a New Record
	       ...
	       strcat(sql, " INSERT INTO Status_Desc (status_desc_id,description) VALUES (nextval('status_desc_id_seq'),'");
	       strcat(sql, IDL_STRING_STR(&kw.description));
               strcat(sql, "'); UPDATE Data_Source SET status_desc_id = currval('status_desc_id_seq') WHERE source_id = "); 
	       strcat(sql, PQgetvalue(DBQuery,i,0));
	       strcat(sql, "; UPDATE Data_Source SET modified = current_date WHERE source_id = "); 
	       strcat(sql, PQgetvalue(DBQuery,i,0)); 	  
	       strcat(sql,"; INSERT INTO Change_Log (username,dbtable,key1,dbfield,prior_value,rollback_sql) VALUES ('");
	       ...
	       sprintf(sql_str,"SELECT description FROM Status_Desc WHERE status_desc_id = %s", PQgetvalue(DBQuery,i,8));
	       if(kw.debug)fprintf(stdout,"\n%s\n\n", sql_str);
               if((rc = queryIdamTable(sql_str,kw.verbose,stdout,old_description)) != 0){
                  if(kw.verbose) fprintf(stdout,"ERROR ... Unable to Read the Previous Status Change Description for Database!\n");
                  PQclear(DBQuery);
                  IDL_KW_FREE;
                  return(IDL_GettmpLong(1));
               }		      
	 ...
	 if(kw.commit){
	    if((rc = executeIdamSQL(sql, kw.verbose, stdout)) == 1){
               if(kw.verbose)fprintf(stdout,"ERROR - Data_Source Status NOT Updated (record %s)\n", PQgetvalue(DBQuery,i,0));
               PQclear(DBQuery);
               IDL_KW_FREE;
               return(IDL_GettmpLong(1));
	    }
   ...
   if(argv[0]->type == IDL_TYP_STRING){
      ...
      if(argv[1]->type == IDL_TYP_LONG || argv[1]->type == IDL_TYP_INT){	// Exp_number & Range
	    ...
	    if(kw.debug){
         ...
         if(kw.verbose) fprintf(stdout,"Signal Alias Name Not Followed by an Integer Experiment Number or List Array - Check Argument #2\n");
	 IDL_KW_FREE;
	 return(IDL_GettmpLong(1));
      }	 
   } else {
...
// Passed Arguments: Integer (Scalar or Array) followed by a String
	 ...
	 if(argv[0]->type == IDL_TYP_LONG || argv[0]->type == IDL_TYP_INT){	// Exp_number & Range
	       ...
	       if(kw.debug){
            ...
            if(kw.verbose) fprintf(stdout,"Signal Alias Name Not Followed by an Integer Experiment Number or List - Check Argument #1\n");
	    IDL_KW_FREE;
	    return(IDL_GettmpLong(1));
         }	 
      } 
...
// Check there is NO Ambiguity in the Source_Alias if Specified  
   ...
   if(kw.is_source){
      ...
      if((nrows = PQntuples(DBQuery)) > 1){
         if(kw.verbose) fprintf(stdout,"The Specified Data Source File Alias Name (%s) is Ambiguous ... Please Clarify\n",
	                        IDL_STRING_STR(&kw.source));
         PQclear(DBQuery);
         IDL_KW_FREE;
         return(IDL_GettmpLong(1)); 
      ...
      if((nrows = PQntuples(DBQuery)) == 0){
         if(kw.verbose) fprintf(stdout,"No Data Source Files with the Specified Alias Name (%s) were Found ... Please Clarify\n", 
	                        IDL_STRING_STR(&kw.source));
         PQclear(DBQuery);
         IDL_KW_FREE;
         return(IDL_GettmpLong(1)); 
      ...
      if(kw.debug)fprintf(stdout,"Number of Source Files with Specified Alias Name (%s) Selected: %d\n", source, nrows);
...
// Check there is NO Ambiguity in the Source of Signals 
   ...
   if(!kw.is_source){ 
      ...
      strcat(sql, whr_sig);
      if(kw.latest){
         strcat(sql, ") as A, (SELECT source_alias, source_id as src_id, max(pass) as mpass FROM data_source WHERE ");
         strcat(sql, whr);
	 strcat(sql, " GROUP BY source_alias, src_id");
      ...
      strcat(sql, ") as B WHERE Signal.source_id=B.src_id AND Signal.signal_desc_id=A.desc_id) as C GROUP BY source_alias");
      ...
      if((nrows = PQntuples(DBQuery)) > 1){
         if(kw.verbose) fprintf(stdout,"The Data Source associated with the Signal (%s) is Ambiguous ... Please Clarify\n", signal);
         PQclear(DBQuery);
         IDL_KW_FREE;
         return(IDL_GettmpLong(1)); 
      }
      ...
      if((nrows = PQntuples(DBQuery)) == 0){
         if(kw.verbose) fprintf(stdout,"No Data Source Files associated with the Signal (%s) were Found ... Please Clarify\n", signal);
         PQclear(DBQuery);
         IDL_KW_FREE;
         return(IDL_GettmpLong(1)); 
      }
...
// Query Signal Table for Current Status Values 
   ...
   if(kw.is_query && strlen(IDL_STRING_STR(&kw.query)) > 0){		// Additional Query used against Signal Table
      ...
      strcat(sql,") as B WHERE Signal.source_id=B.src_id AND Signal.signal_desc_id=A.desc_id) as C ORDER BY signal_name;");   
   }
...
// Check User Authorisation: Continue Looping if Not Authorised
         ...
         if(kw.commit && (rc = checkSignalAuthorisation(0, PQgetvalue(DBQuery,i,8), clientname, kw.verbose, stdout)) == 1){
            if(kw.verbose)fprintf(stdout,"ERROR - You are Not Authorised to Update Status Values of the Signal %s\n", PQgetvalue(DBQuery,i,8));
            continue;
         }      
...
// Status 
	    ...
	    strcat(sql,PQgetvalue(DBQuery,i,1));  		 
	    strcat(sql,"; INSERT INTO Change_Log (username,dbtable,key1,key2,dbfield,prior_value,rollback_sql) VALUES ('");
	    strcat(sql,clientname);
	    strcat(sql, "','Signal',");
	    strcat(sql,PQgetvalue(DBQuery,i,0));
	    strcat(sql, ",");
...
// Status Change Reason Code
	    ...
	    code = 100*kw.reason + kw.impact;
            ...
            sprintf(sql_str, " UPDATE Signal SET status_reason_impact_code = %d WHERE source_id = %s AND signal_desc_id = %s", 
...
// Status Change Description
	    ...
	    if(strlen(PQgetvalue(DBQuery,i,12)) == 0){	// No Previous Description? - Create a New Record
	       ...
	       strcat(sql, " INSERT INTO Status_Desc (status_desc_id,description) VALUES (nextval('status_desc_id_seq'),'");
	       strcat(sql, IDL_STRING_STR(&kw.description));
               strcat(sql, "'); UPDATE Signal SET status_desc_id = currval('status_desc_id_seq') WHERE source_id = "); 
	       strcat(sql, PQgetvalue(DBQuery,i,0));
	       strcat(sql," AND signal_desc_id = ");
	       strcat(sql, PQgetvalue(DBQuery,i,1));
	       strcat(sql, "; UPDATE Signal SET modified = current_date WHERE source_id = "); 
	       ...
	       sprintf(sql_str,"SELECT description FROM Status_Desc WHERE status_desc_id = %s", PQgetvalue(DBQuery,i,12));
	       if(kw.debug)fprintf(stdout,"\n%s\n\n", sql_str);
               if((rc = queryIdamTable(sql_str,kw.verbose,stdout,old_description)) != 0){
                  if(kw.verbose) fprintf(stdout,"ERROR ... Unable to Read the Previous Status Change Description for Database!\n");
                  PQclear(DBQuery);
	 ...
	 if(kw.commit){
	    if((rc = executeIdamSQL(sql, kw.verbose, stdout)) == 1){
               if(kw.verbose)fprintf(stdout,"ERROR - Signal Status NOT Updated (record %s %s)\n", 
...
//-------------------------------------------------------------------------	      
   ...
   int i, rc, nrows, wherenum=0, lstr;
...
// Check User Authorisation  
   ...
   if(kw.commit && (rc = checkSignalAuthorisation(1, PQgetvalue(DBQuery,0,0), clientname, kw.verbose, stdout)) == 1){
      if(kw.verbose)fprintf(stdout,"ERROR - You are Not Authorised to Update Descriptions of Signals %s\n", PQgetvalue(DBQuery,0,2) );
      kw.commit = 0;
   } 
...
// Update the Description
   ...
   sprintf(sql, "BEGIN; UPDATE Signal_desc SET description = '%s' WHERE signal_desc_id = %s", IDL_STRING_STR(&kw.description), 
           PQgetvalue(DBQuery,0,0));	         		 
   strcat(sql,"; UPDATE Signal_desc SET modified = current_date WHERE signal_desc_id = ");
   ...
   if(kw.commit){
      if((rc = executeIdamSQL(sql, kw.verbose, stdout)) == 1){
         if(kw.verbose)fprintf(stdout,"ERROR - Signal Status NOT Updated (record %s)\n", PQgetvalue(DBQuery,i,0));
         PQclear(DBQuery);
         IDL_KW_FREE;
         return(IDL_GettmpLong(1));
      }	 
...
// Check User Authorisation  
   ...
   if(kw.commit && kw.is_class && (rc = checkSignalAuthorisation(1, PQgetvalue(DBQuery,0,0), clientname, kw.verbose, stdout)) == 1){
      if(kw.verbose)fprintf(stdout,"ERROR - You are Not Authorised to Update the Classification of the Signal %s\n", PQgetvalue(DBQuery,0,2));
      kw.is_class = 0;
   }
   
   if(kw.commit && kw.is_owner && (rc = checkAdminAuthorisation(clientname, kw.verbose, stdout)) == 1){
      if(kw.verbose)fprintf(stdout,"ERROR - You are Not Authorised to Update Ownership of the Signal %s\n", PQgetvalue(DBQuery,0,2));
      kw.is_owner = 0;
   } 
...
// Range Limit
   ...
   if(kw.is_shotrange && !kw.huge){
      if((i = abs(kw.shotrange[1] - kw.shotrange[0])) > LISTRANGELIMIT){
         if(kw.verbose) fprintf(stdout,"Range is Too Large: The limit is %d shots. Please respecify!\n", LISTRANGELIMIT);
         IDL_KW_FREE;
         return(IDL_GettmpLong(1));
      } 
   }
 ...
 // Exp_Number 
   ...
   if(kw.is_exp_number && !kw.is_shotrange){
      if(kw.exp_number == 0){
	    ...
	    if(kw.shot > 0) kw.exp_number = kw.shot;
      }	          
   }  
   
   if(!kw.all && !kw.distinct && !kw.is_shotrange && !kw.is_exp_number){
      if(kw.verbose) fprintf(stdout,"Warning - No Exp. Numbers passed and No ALL or DISTINCT Keyword Passed - Assuming ALL.\n");
      kw.all = 1;			 
   }
   
   if(!kw.all && kw.distinct && !kw.is_shotrange && !kw.is_exp_number){
      if(kw.verbose) fprintf(stdout,"No Exp. Numbers passed with DISTINCT Keyword - Please specify an Exp. Number or Range \n");
      IDL_KW_FREE;
      return(IDL_GettmpLong(1));			 
   }
...
// Query the Database
   ...
   if(!kw.all && !kw.distinct){
      ...
      if(kw.latest || (!kw.is_passrange && kw.is_pass && (int)kw.pass == -1)){
         strcat(sql,") as A, ");
	 strcat(sql,"(SELECT source_id as src_key, exp_number, pass, source_status, source_alias, type FROM data_source, ");
         strcat(sql,"(SELECT src_id FROM (SELECT source_id as src_id, filename as fname, pass as passno FROM Data_Source WHERE ");
         strcat(sql, whr);
         strcat(sql,") as X, (SELECT filename, max(pass) as mpass FROM Data_Source WHERE ");
         strcat(sql, whr);
         strcat(sql," GROUP BY filename) as Y WHERE X.fname = Y.filename AND X.passno = Y.mpass) as Z WHERE Z.src_id = data_source.source_id");	 	 	 
	 ...
	 strcat(sql,") as B WHERE signal.source_id=B.src_id AND Signal.signal_desc_id=A.desc_id) as C "
      ...
      if(kw.distinct && !kw.all){
	    ...
	    strcat(sql,") as B WHERE signal.source_id=B.src_id AND Signal.signal_desc_id=A.desc_id) as C "
         ...
         strcpy(sql,"SELECT signal_alias, generic_name, type, description, source_alias, signal_class, signal_owner, mds_name FROM signal_desc");
         if(wherenum > 0){
	    strcat(sql," WHERE ");
	    strcat(sql,whr);
	 }   
...
// Extract the Resultset 
   ...
   if(!kw.huge && nrows > LISTROWLIMIT){
      if(kw.verbose) fprintf(stdout,"ERROR - Too many Records (%d) Returned by the Query. The limit is %d", nrows, LISTROWLIMIT);
      IDL_KW_FREE; 
      PQclear(DBQuery);
      return(IDL_GettmpLong(1));
   }   
...
// Query the Database
   ...
   strcpy(sql,"SELECT source_id as src_key, exp_number, pass, source_status, source_alias, type, format, path, filename, "
              "server, run_id FROM data_source ");        
   
   if(kw.latest){
      strcat(sql,",(SELECT src_id FROM (SELECT source_id as src_id, exp_number as enumber, pass as passno FROM Data_Source ");
      strcat(sql, whr);
      strcat(sql,") as X, (SELECT exp_number, max(pass) as mpass FROM Data_Source ");
      strcat(sql, whr);
      strcat(sql," GROUP BY exp_number) as Y WHERE X.enumber = Y.exp_number AND X.passno = Y.mpass) as Z WHERE Z.src_id = data_source.source_id");
   } else
      strcat(sql,whr); 
...
// Build SQL  
   ...
   if(kw.is_type){
      sprintf(sql,"SELECT DISTINCT source_alias, type FROM data_source WHERE type='%s' ORDER BY type, source_alias;", IDL_STRING_STR(&kw.type));        
   } else {
...
// Extract the Resultset 
   ...
   if(kw.is_search && nrows > 1){
      if(kw.verbose)IDL_Message(IDL_M_NAMED_GENERIC, IDL_MSG_INFO, "ERROR - Too Many Reason Codes Found. Please Clarify"); 
      IDL_KW_FREE; 
      PQclear(DBQuery);
      return(IDL_GettmpLong(1));
   }
...
// Extract the Resultset 
   ...
   if(kw.is_search && nrows > 1){
      if(kw.verbose)IDL_Message(IDL_M_NAMED_GENERIC, IDL_MSG_INFO, "ERROR - Too Many Impact Codes Found. Please Clarify"); 
      IDL_KW_FREE; 
      PQclear(DBQuery);
      return(IDL_GettmpLong(1));
   }
...
//=========================================================================================   
/*
IDL_VPTR IDL_CDECL listshots(int argc, IDL_VPTR argv[], char *argk){
//
// List ALL shots available for a specific pass, source, signal, class and type
//-------------------------------------------------------------------------	      

   int i, nrows, ncols, wherenum, lstr;
   int ndims, ilDims[1];
   
   char *type, *source, *signal;      
   char sql[MAXSQL];
   char whr[MAXSQL], whr_str[256];
   sql_signal_desc[MAXSQL];
   sql_data_source[MAXSQL];
   char sql_source[MAXNAME];
   char sql_signal[MAXNAME];
...
// Maintain Alphabetical Order of Keywords
   ...
   { IDL_KW_FAST_SCAN,
     ...
     {"PASS",            IDL_TYP_LONG,    1,IDL_KW_ZERO,IDL_KW_OFFSETOF(is_pass),               IDL_KW_OFFSETOF(pass)},
     {"PRANGE",          IDL_TYP_LONG,    1,IDL_KW_ARRAY,IDL_KW_OFFSETOF(is_passrange),         IDL_CHARA(passDesc)},
     {"SIGNAL",          IDL_TYP_STRING,  1,IDL_KW_ZERO,IDL_KW_OFFSETOF(is_signal),		IDL_KW_OFFSETOF(signal)}, 
     {"SOURCE",          IDL_TYP_STRING,  1,IDL_KW_ZERO,IDL_KW_OFFSETOF(is_source),		IDL_KW_OFFSETOF(source)},          
     {"SRANGE",          IDL_TYP_LONG,    1,IDL_KW_ARRAY,IDL_KW_OFFSETOF(is_shotrange),         IDL_CHARA(expDesc)},
     {"TYPE",            IDL_TYP_STRING,  1,IDL_KW_ZERO,IDL_KW_OFFSETOF(is_type),		IDL_KW_OFFSETOF(type)},          
     {"VERBOSE",         IDL_TYP_LONG,    1,IDL_KW_ZERO,0,					IDL_KW_OFFSETOF(verbose)},
...
// Signal Name
   ...
   if(kw.is_signal && strlen(IDL_STRING_STR(&kw.signal)) == 0){
      if(kw.verbose) fprintf(stdout,"No Passed Signal Name. Please check!\n");
      IDL_KW_FREE;
      return(IDL_GettmpLong(1));			 
   }
...
//-----------------------------------------------------------------------------------------------------------------------------
// Build SQL WHERE Clause to Identify Source Files
	 ...
	 wherenum++;
      ...
      if(wherenum > 0) strcat(whr," AND ");
      strcat(whr,"source_status > -1");   
      wherenum++;
            	                      
//-----------------------------------------------------------------------------------------------------------------------------
// Query the Database

      if(kw.is_type || kw.is_signal){
         strcpy(sql_signal_desc, "SELECT signal_desc_id as desc_id FROM signal_desc WHERE");
         if(kw.is_type){
            strcat(sql_signal_desc,"type='");
            strcat(sql_signal_desc,IDL_STRING_STR(&kw.type));
            strcat(sql_signal_desc,"'");
         }
	 if(kw.is_type && kw.is_signal) strcat(sql_signal_desc," AND ");
	 if(kw.is_signal){
	    if(kw.match){  
               strcat(whr, "signal_alias ILIKE '");
	       strcat(whr, sql_signal);
	       strcat(whr, "' OR generic_name ILIKE '");
	       strcat(whr, sql_signal);
	       strcat(whr, "');
	    } else { 
	       strcat(whr, "upper(signal_alias) = '");
	       strcat(whr, sql_signal);
	       strcat(whr, "' OR upper(generic_name) = '");
	       strcat(whr, sql_signal);
	       strcat(whr, "');	       
	    }
	    if(kw.is_prange){
	       if(kw.nshotrange == 2){
	          sprintf(sql_whr,"range_start <= %d AND (range_stop=0 OR range_stop>= %d", kw.shotrange[0]);
	       } else {
	          sprintf(sql_whr,"range_start <= %d AND (range_stop=0 OR range_stop>= %d", kw.shotrange[0]);
	       }
...
// *************** Add tests for generic/Rank/Shot range .....
   strcat(sql,"')) AND (signal_desc.range_start=0 OR signal_desc.range_start<=");
   strcat(sql,expno);
   strcat(sql,") AND (signal_desc.range_stop=0 OR signal_desc.range_stop>=");
   strcat(sql,expno);
   strcat(sql,") ORDER BY signal_desc.generic_name, signal_desc.rank, data_source.pass DESC LIMIT 1;");	      


	      
      if(kw.latest){
         strcat(sql_data_source,"SELECT src_id, exp_number FROM (SELECT source_id as src_id, exp_number FROM Data_Source WHERE ");
	 strcat(sql_data_source, whr);
	 strcat(sql_data_source," as X, (SELECT source_id, max(pass) as mpass FROM Data_Source WHERE ");
	 strcat(sql_data_source, whr);
	 strcat(sql_data_source," GROUP BY source_id) as Y WHERE X.src_id = Y.source_id) as Z");
      } else {
	 strcat(sql_data_source,"SELECT source_id as src_id, exp_number FROM Data_Source WHERE ");
	 strcat(sql_data_source, whr);
      }
      	 

      strcat(sql, "SELECT DISTINCT exp_number FROM Signal, (");
      strcat(sql, sql_signal_desc);
      strcat(sql,") as A, ("      
      strcat(sql, sql_data_source);
      strcat(sql, ") as B WHERE Signal.signal_desc_id=A.desc_id AND Signal.source_id=B.src_d "
                  "AND signal.signal_status > -1 ORDER BY exp_number DESC");		 
...
// Allocate the Return Array
   ...
   dvec = (int *)IDL_MakeTempArray((int)IDL_TYP_LONG, ndims, (IDL_MEMINT *)ilDims, 
	                            IDL_ARR_INI_ZERO, &idlArray);

   for(i=0;i<nrows;i++) dvec[i] = atoi(PQgetvalue(DBQuery,i,0))
...
IDL_VPTR IDL_CDECL getidampath(int argc, IDL_VPTR argv[], char *argk){
   ...
   int nrows, pulno, iscase = 1, length;
   char *source;   
   char path[MAXPATH];
   char type[2] = " ";
   char *ptype; 
