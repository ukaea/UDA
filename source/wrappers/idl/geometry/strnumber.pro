...
;
; EXPLANATION:
;      A string is considered a valid numeric value if IDL can convert
;      it
...
;
; OPTIONAL OUTPUT:
;      val - (optional) value of the string. double precision unless
;            /L64 is set
;
; OPTIONAL INPUT KEYWORD:
;       /HEX - If present and nonzero, the string is treated as a
;              hexadecimal
;             longword integer.
;       /L64 - If present and nonzero, the val output variable is
;              returned
;              as a 64 bit integer.    This to ensure that precision
;              is not       
;              lost when returning a large 64 bit integer as double
;              precision.
;              This keyword has no effect on the function result.
;       /NAN - if set, then the value of an empty string is returned
;              as NaN,
;              by default the returned value is 0.0d.     In either
;              case,
...
;              
; NOTES:
;      (1) STRNUMBER was modified in August 2006 so that an empty
;      string is 
;      considered a valid number.   Earlier versions of strnumber.pro
;      did not 
;      do this because in very early (pre-V4.0) versions of IDL
;      this could corrupt the IDL session.
;
;       (2) STRNUMBER will return a string such as '23.45uyrg' as a
;       valid 
;      number (=23.45) since this is how IDL performs the type
;      conversion.  If
;      you want a stricter definition of valid number then use the
;      VALID_NUM()
;      function.
; HISTORY:
