...
# get rid of abc. treat as mixin to add common methods
...
def auto_implement_properties(cls):
    import inspect
    params = inspect.signature(cls.__init__).parameters.keys()
    props = set(cls.__abstractmethods__)
    for name in getattr(cls, '__abstractmethods__', []):
        ...
        if name in params:
            ...
            setattr(cls, name, property(fget=make_fget(private_name),
                                        fset=make_fset(private_name)))
            props.remove(name)
    cls.__abstractmethods__ = frozenset(props)
    return cls


@auto_implement_properties
class DataOwningSignal(SignalBase):
    """
    ...
    def __init__(self, data=None, errors=None, label='', units='', description='',
                 rank=None, dims=None, time=None, shape=None, time_index=None, meta=None):
        self._data = data
        self._errors = errors
        self._label = label
        self._units = units
...
class BadSignal(SignalBase):
    ...
    def __init__(self, data=None, errors=None, label='', units='', description='',
                 rank=None, dims=None, time=None, shape=None, time_index=None):
        self._data = data
        self._errors = errors
        self._label = label
        self._units = units
