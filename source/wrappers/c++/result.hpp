...
public:
    ...
    dim_type rank() const { return rank_; }
    const std::map<std::string, std::string>& meta() { return meta_; }

    Dim dim(dim_type ndim) const;

    bool isTree() const { return istree_; }
    TreeNode tree() const;
...
private:
    ...
    template <typename T> T as() { return boost::any_cast<T>(data_); }
    const int handle_;
    std::string label_;
    std::string units_;
    std::string desc_;
