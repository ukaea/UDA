...
//   UDA_SERVER_OIDC_JWKS_URI         Direct JWKS endpoint (optional; if empty, discovers via issuer)
...
//   UDA_SERVER_OIDC_VERIFY_EXPIRY=1|0       (default: 1; note: jwt-cpp always checks exp if present)
//   UDA_SERVER_OIDC_VERIFY_NBF=1|0          (default: 1; note: jwt-cpp always checks nbf if present)
//   UDA_SERVER_OIDC_VERIFY_IAT=1|0          (default: 0; jwt-cpp may check if present)
...
struct OidcConfig {
    ...
    std::string audience;
    std::string jwks_uri;
    std::vector<std::string> allowed_algs; // empty means ["RS256"]
