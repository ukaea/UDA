...
int extractIdamX509SExpKey(ksba_cert_t cert, gcry_sexp_t* key_sexp)
{
    ...
    if ((p = ksba_cert_get_public_key(cert)) == NULL) {
        err = 999;
        addIdamError(ErrorType::Code, "extractIdamX509SExpKey", err, "Failure to get the Public key!");
        return err;
    }
    ...
    if ((n = gcry_sexp_canon_len(p, 0, NULL, NULL)) == 0) {
        err = 999;
        addIdamError(ErrorType::Code, "extractIdamX509SExpKey", err, "did not return a proper S-Exp!");
        ksba_free(p);
        return err;
    }
    ...
    if ((errCode = gcry_sexp_sscan(key_sexp, NULL, (char*)p, n)) != 0) {
        err = 999;
        addIdamError(ErrorType::Code, "extractIdamX509SExpKey", err, "S-Exp creation failed!");
        ksba_free(p);
        return err;
    }
...
int main()
{
    ...
    do {        // Start of Error Trap
        ...
        if ((err = clientAuthentication(&client_block, &server_block, &logmalloclist, &userdefinedtypelist, authenticationStep)) != 0) {
            addIdamError(ErrorType::Code, "idamClient", err, "Client or Server Authentication Failed #1");
            break;
        }
        ...
        if ((err = serverAuthentication(&client_block, &server_block, &logmalloclist, &userdefinedtypelist, authenticationStep)) != 0) {
            addIdamError(ErrorType::Code, __func__, err, "Client or Server Authentication Failed #2");
            break;
        }
        ...
        if ((err = serverAuthentication(&client_block, &server_block, &logmalloclist, &userdefinedtypelist, authenticationStep)) != 0) {
            addIdamError(ErrorType::Code, __func__, err, "Client or Server Authentication Failed #3");
            break;
        }
        ...
        if ((err = serverAuthentication(&client_block, &server_block, &logmalloclist, &userdefinedtypelist, authenticationStep)) != 0) {
            addIdamError(ErrorType::Code, __func__, err, "Client or Server Authentication Failed #4");
            break;
        }
        ...
        if ((err = clientAuthentication(&client_block, &server_block, &logmalloclist, &userdefinedtypelist, authenticationStep)) != 0) {
            addIdamError(ErrorType::Code, "idamClient", err, "Client or Server Authentication Failed #5");
            break;
        }
        ...
        if ((err = clientAuthentication(&client_block, &server_block, &logmalloclist, &userdefinedtypelist, authenticationStep)) != 0) {
            addIdamError(ErrorType::Code, "idamClient", err, "Client or Server Authentication Failed #6");
            break;
        }
        ...
        if ((err = serverAuthentication(&client_block, &server_block, &logmalloclist, &userdefinedtypelist, authenticationStep)) != 0) {
            addIdamError(ErrorType::Code, __func__, err, "Client or Server Authentication Failed #7");
            break;
        }
        ...
        //            addIdamError(ErrorType::Code, "idamClient", err,
