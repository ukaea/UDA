#if 0
#!/bin/bash
g++ test3.cpp -g -O0 -gdwarf-3 -o test \
...
int main() {
    ...
    const Idam::Result& result = client.get("ts::read(element=/bpol_probe/110/name, shot=43970)", "");
    ...
    std::vector<char> values = arr->as<char>();
    for (int i = 0; i < arr->as<char>().size(); ++i) {
