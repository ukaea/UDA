#if 0
#!/bin/bash
g++ test_west_ece_develop.cpp -g -std=c++11 -O0 -gdwarf-3 -o test -DHOME=$HOME -I$HOME/iter/uda/source -I$HOME/iter/uda/source/wrappers \
...
#define SHOT_NUM_51826 "51827" // WEST
...
int main() {
	...
	const uda::Result& shape_of = client.get("imas::get(idx=0, group='ece', variable='channel/Shape_of', expName='WEST', type=int, rank=0, shot=" SHOT_NUM ", )", "");
	...
	/*const uda::Result& name2 = client.get("imas::get(idx=0, group='ece', variable='channel/10/name', expName='WEST', type=string, rank=0, shot=" SHOT_NUM_51826 ", )", "");
	const uda::Data * data2 = name2.data();
	const uda::String* s2 = dynamic_cast<const uda::String*>(data2);

	std::cout << "channel/10/name : " << s2->str() << "\n";*/

	const uda::Result& sigfreq = client.get("imas::get(idx=0, group='ece', variable='channel/1/frequency/data', expName='WEST', type=double, rank=1, shot=" SHOT_NUM ", )", "");
	...
	/*const uda::Result& sigfreq2 = client.get("imas::get(idx=0, group='ece', variable='channel/1/frequency/time', expName='WEST', type=double, rank=1, shot=" SHOT_NUM ", )", "");
	const uda::Data * data5 = sigfreq2.data();
	const uda::Array* arrfreq2 = dynamic_cast<const uda::Array*>(data5);

	std::cout << "first values for channel/1/frequency/time : ";
	for (int j = 0; j < 10; ++j) {
	...
	const uda::Result& sigtime = client.get("imas::get(idx=0, group='ece', variable='channel/1/t_e_voltage/time', expName='WEST', type=double, rank=1, shot=" SHOT_NUM ", )", "");
	...
	const uda::Result& sig2 = client.get("imas::get(idx=0, group='ece', variable='channel/1/t_e_voltage/data', expName='WEST', type=double, rank=1, shot=" SHOT_NUM ", )", "");
	const uda::Data *voltage_data = sig2.data();
	const uda::Array* arr2 = dynamic_cast<const uda::Array*>(voltage_data);

	std::cout << "first values for channel/1/t_e_voltage/data: ";
	for (int j = 0; j < 10; ++j) {
		std::cout << arr2->as<double>().at(j) << " ";
	}
	std::cout << "..." << std::endl;

	const uda::Result& sigharm = client.get("imas::get(idx=0, group='ece', variable='channel/1/harmonic/data', expName='WEST', type=double, rank=1, shot=" SHOT_NUM ", )", "");
	const uda::Data * data = sigharm.data();
	...
	/*const uda::Result& sigharm_time = client.get("imas::get(idx=0, group='ece', variable='channel/1/harmonic/time', expName='WEST', type=double, rank=1, shot=" SHOT_NUM ", )", "");
    ...
    const uda::Result& time = client.get("imas::get(idx=0, group='ece', variable='time', expName='WEST', type=double, rank=1, shot=" SHOT_NUM ", )", "");
	const uda::Data * time_data = time.data();
	const uda::Array* arr_time_data = dynamic_cast<const uda::Array*>(time_data);

	std::cout << "first values for time : ";
	for (int j = 0; j < 10; ++j) {
		std::cout << arr_time_data->as<double>().at(j) << " ";
	}
	std::cout << "..." << std::endl;
