#if 0
#!/bin/bash
g++ test_west_andromede1.cpp -g -std=c++11 -O0 -gdwarf-3 -o test -DHOME=$HOME -I$HOME/uda/source -I$HOME/uda/source/wrappers \
-L$HOME/uda/lib -Wl,-rpath,$HOME/uda/lib  -luda_cpp -lssl -lcrypto -lxml2
exit 0
#endif
...
int main() {
	setenv("UDA_PLUGIN_DIR", QUOTE(HOME) "/uda/etc/plugins", 1);
	setenv("UDA_PLUGIN_CONFIG", QUOTE(HOME) "/uda/test/idamTest.conf", 1);
	setenv("UDA_SARRAY_CONFIG", QUOTE(HOME) "/uda/bin/plugins/idamgenstruct.conf", 1);
	setenv("UDA_WEST_MAPPING_FILE", QUOTE(HOME) "/uda/source/plugins/west/WEST_mappings/IDAM_mapping.xml", 1);
	setenv("UDA_WEST_MAPPING_FILE_DIRECTORY", QUOTE(HOME) "/uda/source/plugins/west/WEST_mappings", 1);
	setenv("UDA_LOG", QUOTE(HOME) "/uda/", 1);
