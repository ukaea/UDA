#if 0
#!/bin/bash
g++ test1.cpp -g -O0 -gdwarf-3 -o test \
...
int main() {
    ...
    //const Idam::Result& result = client.get("XDAM::get(signal=ip)", "18299");
    //const Idam::Result& result = client.get("ts::mapToIDS(index=1)", "");
    //const Idam::Result& result = client.get("ts_xml::read(file=/u/lfleury/itmwork/test.xml, xpath=/top/topinfo/description)", "");
    ...
    const Idam::Result& result = client.get("ts::read(element=/bpol_probe/1/field, shot=43970)", "");
    
    //const Idam::Result& result = client.get("ts::read(element=/bpol_probe/Size_of, shot=43970)", "");
    
    //std::cout << result.label() << std::endl;1
